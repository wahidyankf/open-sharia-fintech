...
- **Colors**: Claude Code agents use named colors (`blue`, `green`, `yellow`, `purple`, etc.) written by hand in `.claude/agents/*.md`. `rhino-cli harness bindings generate` translates these to OpenCode theme tokens (`primary`, `success`, `warning`, `secondary`, etc.) when generating `.opencode/agents/*.md` — current OpenCode rejects named colors. See [Platform Binding Color Translation](./repo-governance/development/agents/ai-agents/17-agent-color-categorization.md#platform-binding-color-translation) for the full mapping.
