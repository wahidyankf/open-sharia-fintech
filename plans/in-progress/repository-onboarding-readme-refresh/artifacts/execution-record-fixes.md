...
One imprecision is recorded rather than fixed, because it is in the pushed commit message for
`ab0dc7b4c` and not worth a force-push to amend: that message says the false "mandated" claim sat
"150 lines before the caveat". The actual distances at the parent commit were 175 lines for
`P5B-005` and 148 for `P5B-005A`, so the figure rounds one of the two rather than describing both.
