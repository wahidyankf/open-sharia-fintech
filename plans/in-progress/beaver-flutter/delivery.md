...
- [ ] [AI] Verify the hard predecessor before any BeaverNest implementation, branch creation, or baseline test: run `beavernest_env_dependency_tree=$(mktemp) && gh api 'repos/wahidyankf/ose-private/git/trees/main?recursive=1' --jq '.tree[].path' > "$beavernest_env_dependency_tree" && rg -q '^plans/done/[0-9]{4}-[0-9]{2}-[0-9]{2}__restrict-env-access-to-prod-and-stag/README\.md$' "$beavernest_env_dependency_tree" && ! rg -q '^plans/in-progress/restrict-env-access-to-prod-and-stag/' "$beavernest_env_dependency_tree" && beavernest_env_dependency_sha=$(gh api 'repos/wahidyankf/ose-private/commits/main' --jq .sha) && { printf 'upstream-main=%s\n' "$beavernest_env_dependency_sha"; rg '^plans/done/[0-9]{4}-[0-9]{2}-[0-9]{2}__restrict-env-access-to-prod-and-stag/README\.md$' "$beavernest_env_dependency_tree"; } > plans/in-progress/beaver-flutter/evidence/phase-0-env-access-predecessor.md && rm -f -- "$beavernest_env_dependency_tree"` — acceptance: the `ose-private` plan is archived on its `main`, absent from `in-progress`, and its exact archive path/SHA are recorded; otherwise stop this plan without creating a branch or changing BeaverNest code.
...
- [ ] [AI] All checks must pass before starting Phase 1: verify `plans/in-progress/beaver-flutter/evidence/phase-0-env-access-predecessor.md` exists and run `npm run doctor -- --fix` and `git status --short` — acceptance: the archived predecessor evidence is present, the toolchain is green, and only plan/evidence ledger files exist.
