...
| `env staged-guard validate`                               | — (hook step 1) | —                | —                             | —         | Staged-only by design; declare as carve-out |
...
| `carve-out` | no       | `formatter` marks a gate exempt from the composition rule, per ratified rule 2.                        |

Surface names are `pre-commit`, `pre-push`, `ci`, and `cron`. Scope values are the five already
ratified in the SDLC Gate Standard — `affected-file-type`, `all-file-type`, `affected-projects`,
`all-projects`, `other` — plus `path-gated`, which is the qualifier the standard already applies in
prose to the governance validators. No new scope vocabulary is introduced.
...
Three new leaf commands under a `gate` domain, following the ratified verb-last naming
(`{domain} {sub-domain} {verb}`):
...
| `rhino-cli gate run --surface=<name>`                          | Execute every gate on that surface in declaration order, stopping at first failure. Path-gated entries are skipped when their triggers miss the changed set. |
...
`gate validate` performs four checks:

1. **Composition rule** — every gate declaring `pre-commit` or `pre-push` must also declare `ci`,
   unless it carries `carve-out: formatter`.
...
3. **CI derivation** — `pr-quality-gate.yml` builds its check matrix from `gate list`, and runs no
   check command that the registry does not declare.
4. **No orphan surfaces** — no surface file invokes a gate id the registry does not carry.

Checks 2 through 4 are deliberately narrow: they assert _that the surface derives from the registry_,
...
One gate, one job, same parallelism as today, and the job list can no longer drift from the registry
because it is computed from it. The per-language `test:quick` jobs stay hand-written for their
toolchain setup; `gate validate` check 3 asserts their presence rather than emitting them.
...
place. This plan keeps that exactly, and adds a verify pass on the `ci` surface only, marked
`carve-out: formatter` so the composition rule does not demand the inverse (a pre-commit `--check`
that would fight the auto-fix).
