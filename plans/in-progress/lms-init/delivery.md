...
- [ ] [AI] Verify exact-current-head/base `Quality gate` and one clean current-head
      `pr-leak-review`
- [x] [AI] If any check fails, fix at the root cause and push a follow-up commit; never bypass
      <!-- Implementation notes (DU4-PP-204): one check failed on the first head, `Specs structure validation (all affected)`, and it was a real defect rather than noise. Two tasks died with `error : The file '.../RhinoCli.Program/obj/project.assets.json' already exists`. The job fans 31 Nx targets out at the default parallelism and every one of them shells into the SAME rhino-cli project with `dotnet run`, which restores and builds implicitly; from a cold checkout the first wave restores that one project concurrently and collides on its `obj/`. Not caused by this unit's code, but this unit's new project widened the fan-out that lost the race. The remedy is `--parallel=1`, which is not invented here: the `dotnet` job in the same workflow already records the identical reasoning for its own serial run, and the `ose-private` sibling already runs both of its `specs:structure-validation` invocations with `--parallel=1` — the public repository was the outlier, so this closes a drift rather than opening one, and creates no sibling obligation. Verified locally with `--skipNxCache`: 31 projects, `TERMINAL_EXIT=0`. `actionlint` exits 0 on the edited workflow. -->
