...
| K-8 | Terminal-Bench and ARC Prize republication terms — not stated by either _(amended, see note below)_ | No terms stated by either operator. The page's Sources and Licences section records "no terms stated" rather than implying permission; figures are cited (with source URLs) under standard scholarly attribution, not re-licensed.                                                                                                                | n/a — disclosure only.                                             |

**K-8 amendment (recorded during PR review, cycle 3, finding F8; see DD-23 in `tech-docs.md`)**:
this row's original framing was itself based on a planning-time DD-21 table that listed `ARC Prize`
as an operator. Implementation later shipped a single row named `"ARC Prize (GPQA)"` that
conflated the unrelated ARC Prize Foundation with GPQA — no ARC Prize figure was ever in the
roster (ARC-AGI-2/3 were both rejected), while GPQA (`gpqa-diamond`, weight 30) very much is. The
shipped Sources and Licences section now names `GPQA` — which **does** state terms (the benchmark
repository is MIT-licensed) — in place of `ARC Prize`. Terminal-Bench's "no terms stated" finding
in this row stands unchanged; the ARC Prize half of the original K-8 framing does not apply to the
shipped page.
