...
**In Progress (`PRIV-ADMISSION` is the sole live delivery unit, after PR #290 merges and its
PR-native terminal-handoff receipt is read back; its sole successor is `PRIV-A1`, followed by
`PUB-A2`).**
...
Those historical receipts supersede the former future idea-delivery forecast. Public PR #289 merged at `539cda50e6aa48079d347ae6131b81901120cd84`,
admitting the exact A-wave paths and completing direct PUB-A1 size-policy edits. Its sole successor
is the private-safe `PRIV-ADMISSION` obligation; GitHub remains the durable review and receipt
record. PR #290's two zero-finding validations are draft evidence only: they satisfy neither the
post-merge prerequisite nor release a frozen unit until the merged PR's native terminal-handoff
comment is posted and read back. Private rule/code work remains frozen until that receipt then
authorizes `PRIV-ADMISSION`.
...
[delivery.md](./delivery.md#historical-sequential-plan-assembly-receipt). The only executable order
is PR #290's merged and read-back terminal handoff → `PRIV-ADMISSION` → `PRIV-A1` → `PUB-A2`; later
waves remain frozen.
