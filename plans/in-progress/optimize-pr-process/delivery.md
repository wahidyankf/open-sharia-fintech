...
- [ ] `[CLOSURE:P6.21][AI]` Read-only validation: from the already-merged closure branch, run `gh pr view --repo wahidyankf/ose-public --json url,baseRefOid,headRefOid,body,additions,deletions,changedFiles,mergeCommit,state` and copy the result into the closure evidence record in the merged PR. Acceptance: its immutable URL, base/head, readable AI-marked body, statistics, state, and merge SHA equal the Phase 3 and Phase 5 records; this step creates no route, review, or PR mutation.
- [ ] `[CLOSURE:P6.22][AI]` Read-only validation: run `gh pr view --repo wahidyankf/ose-public --json reviews,comments` for that already-merged closure PR and compare the returned review/thread URLs with the Phase 4 evidence record. Acceptance: every recorded review and same-thread disposition is present and no new review or reply is posted in Phase 6.
- [ ] `[CLOSURE:P6.23][AI]` Read-only validation: run `gh pr checks --repo wahidyankf/ose-public --required` for the already-merged closure branch and copy the returned check summary into its evidence record. Acceptance: the recorded current-head CI result is green and names the reviewed head; Phase 6 does not poll, repair, or restart CI.
- [ ] `[CLOSURE:P6.24][AI]` Read-only validation: run `gh pr view --repo wahidyankf/ose-public --json isDraft,mergeStateStatus,statusCheckRollup` and compare it with the Phase 4 ready-state read-back. Acceptance: `isDraft` is `false`, the recorded readiness preconditions were met before merge, and this step does not transition readiness.
- [ ] `[CLOSURE:P6.25][AI]` Read-only validation: run `gh pr view --repo wahidyankf/ose-public --comments` and compare the two raw-patch hashes already recorded by Phase 5. Acceptance: the reviewed and landed fingerprints are both present, identical, and attached to the closure PR artifact; Phase 6 calculates no replacement fingerprint.
...
- [ ] `[CLOSURE:P6.29a][AI]` Read `plans/in-progress/optimize-pr-process/learnings.md` with `sed -n '1,$p' plans/in-progress/optimize-pr-process/learnings.md`; for each entry, write its `terminal-state:` and `durable-destination:` or `discard-reason:` fields in that same file. Apply the durable-surface litmus test: retain only an entry for which the named destination could prevent or detect the issue next time. Acceptance: `rg -n '^(terminal-state|durable-destination|discard-reason):' plans/in-progress/optimize-pr-process/learnings.md` accounts for every entry.
- [ ] `[CLOSURE:P6.29b][AI]` Read the same full target and run `rg -ni --glob '*.md' '(secret|credential|token|private hostname)' plans/in-progress/optimize-pr-process/learnings.md`; replace sensitive values with `<placeholder>` or discard an unsanitizable entry, and mark private-only infrastructure `terminal-state: private-only` without copying it into public evidence. Acceptance: each match has either `<placeholder>` or a discard/private-only record, and every public destination is named in the full `learnings.md` target.
- [ ] `[CLOSURE:P6.29c][AI]` Route each safe survivor in `plans/in-progress/optimize-pr-process/learnings.md` to exactly one named owner surface. For a code/test survivor, first record `backlog-plan: plans/backlog/<kebab-case-slug>/` in that file, then create exactly `README.md`, `brd.md`, `prd.md`, `tech-docs.md`, and `delivery.md` under that resolved directory using the plan template; verify with `test -f "$backlog_plan/README.md" -a -f "$backlog_plan/brd.md" -a -f "$backlog_plan/prd.md" -a -f "$backlog_plan/tech-docs.md" -a -f "$backlog_plan/delivery.md"`, where `backlog_plan` is read from the recorded `backlog-plan:` field. Rules/workflows/docs use their exact named owner surface; transient observations receive `discard-reason:`. Acceptance: `rg -n '^(durable-destination|backlog-plan|discard-reason):' plans/in-progress/optimize-pr-process/learnings.md` shows one, and only one, terminal home per survivor; no inline out-of-scope code change exists.
- [ ] `[CLOSURE:P6.29d][AI]` Before any `plans/ideas/` destination, record `idea-search:` with the exact issue keywords in `plans/in-progress/optimize-pr-process/learnings.md`, then run `rg -ni --glob '*.md' "$idea_search" plans/ideas/README.md plans/ideas/q1-urgent-important plans/ideas/q2-not-urgent-important`. Copy the matching paths (or `no-match`) into that same entry; fold into an existing brief when there is a match. Acceptance: every ideas-routed entry has a read-back search query and result plus either the reused brief path or a unique-new-brief rationale.
- [ ] `[CLOSURE:P6.29e][AI]` If no generalizable learning survives, record `none-reason:` in `plans/in-progress/optimize-pr-process/learnings.md`, then run `printf 'No generalizable learnings — %s\\n' "$none_reason" >> plans/in-progress/optimize-pr-process/learnings.md`. Acceptance: `rg -n '^No generalizable learnings — ' plans/in-progress/optimize-pr-process/learnings.md` returns exactly one line, or P6.29a–d record every entry as terminal before archive.
