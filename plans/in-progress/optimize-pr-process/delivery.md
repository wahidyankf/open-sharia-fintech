...
> PR #289 is provenance only; PR #290's terminal-handoff receipt creates the sole actionable
> public-to-private obligation after its reader-state successor has landed and been read back. Its two
> zero-finding plan-quality validations are draft evidence only; they do not satisfy the post-merge
> prerequisite or release `PRIV-ADMISSION` before that receipt is posted and read back.
...
- [ ] `[PLAN-QUALITY-290:P5.HANDOFF][AI]` After PR #290 merges, fetch `origin/main` and prove it
      ...
      a new private PR starts at Cycle 1 (only a resumed same PR inherits its count). **Acceptance:** read
      back the comment URL, #290 and reader PR SHAs, fetched-main proof, both zero results, fields, and
      `Generated by AI`. Until this passes, draft evidence releases no live unit.
...
| 1–5      | PRIV-ADMISSION   | private/PRIV-WT | `optimize-pr-process-priv-plan-amendment-a-rules-admission` | worktree-to-pr | Phase 3 | #290 handoff + merged reader receipt; #289 provenance                                         | exact private adaptation admission pin   |
...
- [ ] `[PRIV-ADMISSION:P1.01][AI]` From PR #290's terminal handoff, retaining #289 as provenance, record the exact private predecessor, then use `AR-ENTRY` to create the declared private plan-amendment branch from fetched private current main. Acceptance: the worktree is clean; no public artifact receives private path or content detail.
...
- [ ] `[PRIV-ADMISSION:P1.P][AI]` Publish the private admission pin and authorize `PRIV-A1` only when #290's handoff, reader receipt, #289 provenance pin, and one paired obligation are read back.
