...
| ---------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| `predecessor` and `unit-base`      | The exact same-repository landed `origin/main` SHA named by the immediately preceding terminal obligation; record it before `AR-ENTRY`.         |
...
| `pr-body-file` and `route-payload` | The exact local files under `local-tmp/optimize-pr-process/`, written and read back after the cohesive commit and before `AR-DRAFT`/`AR-ROUTE`. |
| `current-main`                     | The immutable `origin/main` SHA read by `AR-ENTRY` before edit.                                                                                 |
...
The variables in the references below are only shorthand for these fields: `$PREDECESSOR` and
`$UNIT_BASE` are `predecessor`/`unit-base`; `$HAND_AUTHORED_PATHS`, `$ADMITTED_PATHS`,
`$PR_BODY_FILE`, `$ROUTE_PAYLOAD`, `$CURRENT_MAIN`, `$REVIEWED_HEAD`, and `$MERGE_SHA` are their
same-named record fields. A field is passed exactly as recorded; it is never inferred from a branch
name, a directory, or a previous PR.

| Reference        | Exact input, command, and output record                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Pass/fail condition                                                                                                                                                                                                                                                                                        |
| ---------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AR-ENTRY`       | Input: literal `REPO`, `BRANCH`, `RECORD`, and the pre-entry fields `predecessor`, `unit-base`, `amendment-url`, `admitted-paths`, and `cap-forecast`. Run `git fetch origin main && test -z "$(git status --porcelain)" && git worktree list --porcelain && if git show-ref --verify --quiet "refs/heads/$BRANCH"; then git switch "$BRANCH"; else git switch -c "$BRANCH" "$PREDECESSOR"; fi && test "$(git rev-parse HEAD)" = "$PREDECESSOR" && test "$UNIT_BASE" = "$PREDECESSOR" && git rev-parse origin/main`; record the selected branch, resolved `current-main`, and command output in `RECORD`. | The declared branch is created or safely reused from the declared predecessor and `HEAD` exactly equals it before edit. `RECORD` names one already-merged admission, one exact source set, and one cap forecast; a missing field, dirty tree, mismatched branch base, or nonzero command freezes the unit. |
...
| `AR-DRAFT`       | Input: literal AI-marked `pr-body-file`, plus `REPO`/`BRANCH` and the immediately preceding `AR-PUSH` remote/local SHA equality in `RECORD`. Run `gh pr create --repo "$REPO" --draft --base main --head "$BRANCH" --title "<resolved-title>" --body-file "$PR_BODY_FILE"` then `gh pr view --repo "$REPO" --json url,isDraft,baseRefOid,headRefOid,body,additions,deletions,changedFiles`; copy read-back JSON and its `headRefOid` as `reviewed-head` into `RECORD`.                                                                                                                                    | Exactly one draft has the resolved base/head, readable body ending `Generated by AI`, separately recorded whole-PR totals, and a preceding `AR-PUSH` equality. Missing push evidence fails before PR creation.                                                                                             |
...
- [ ] `[PRIV-ADMISSION:P4.05][AI]` Run only `AR-READY`. Acceptance: readiness is read back only after all current-head preconditions hold.
