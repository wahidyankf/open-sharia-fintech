...
| ----------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `predecessor`                                   | The exact same-repository landed `origin/main` SHA named by the immediately preceding terminal obligation; record it before `AR-ENTRY` as authorization evidence.                                                                            |
...
| `current-main` and `unit-base`                  | The same immutable `origin/main` SHA read by `AR-ENTRY` after it proves `predecessor` is an ancestor and before edit.                                                                                                                        |
...
The variables in the references below are only shorthand for these fields: `$PREDECESSOR`,
`$UNIT_BASE`, `$PR_TITLE`, `$HAND_AUTHORED_PATHS`, `$ADMITTED_PATHS`, `$PR_BODY_FILE`,
`$ROUTE_PAYLOAD`, `$CURRENT_MAIN`, `$REVIEWED_HEAD`, and `$MERGE_SHA` are their same-named record
fields. A field is passed exactly as recorded; it is never inferred from a branch name, a directory,
or a previous PR.

| Reference        | Exact input, command, and output record                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | Pass/fail condition                                                                                                                                                                                                                                                                                                                                                                                                 |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AR-ENTRY`       | Input: literal `REPO`, `BRANCH`, `RECORD`, and the pre-entry fields `predecessor`, `amendment-url`, `admitted-paths`, and `cap-forecast`. Run `git fetch origin main && test -z "$(git status --porcelain)" && git worktree list --porcelain && git merge-base --is-ancestor "$PREDECESSOR" origin/main && CURRENT_MAIN="$(git rev-parse origin/main)" && if git show-ref --verify --quiet "refs/heads/$BRANCH"; then git switch "$BRANCH"; else git switch -c "$BRANCH" "$CURRENT_MAIN"; fi && test "$(git rev-parse HEAD)" = "$CURRENT_MAIN"`; record the selected branch, resolved `current-main`/`unit-base`, and command output in `RECORD`. | `predecessor` is authorization evidence that is an ancestor of fetched current main. The declared branch is created or safely reused from that current main and `HEAD`, `current-main`, and `unit-base` exactly equal it before edit. `RECORD` names one already-merged admission, one exact source set, and one cap forecast; a missing field, dirty tree, stale branch base, or nonzero command freezes the unit. |
