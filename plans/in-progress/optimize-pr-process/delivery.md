...
`FINDING_COMMENT_ID`, `DISPOSITION`, and `REPLY_BODY_FILE` from `RECORD`. `FINDING_COMMENT_ID` is
the leading review comment's `databaseId`, not a top-level PR comment or a prior reply. Enumerate
all threads first and append the output to `RECORD`; it proves which threads were unresolved at the
fixer-pass boundary and supplies the exact native IDs.
...
gh api graphql -f query="$THREADS_QUERY" -f owner="$OWNER" -f repo="$REPO" -F pr="$PR" \
  --jq '.data.repository.pullRequest.reviewThreads.nodes[] | {thread_id: .id, is_resolved: .isResolved, comments: [.comments.nodes[] | {comment_id: .databaseId, url: .url, body: .body}]}'
```
...
by the stored `THREAD_ID`. The query output is appended to `RECORD` with `THREAD_ID`,
`FINDING_COMMENT_ID`, reply `databaseId`/URL, `REVIEWED_HEAD`, disposition, and `isResolved`.
...
gh api graphql -f query="$THREAD_QUERY" -f thread="$THREAD_ID" \
  --jq '.data.node | {thread_id: .id, is_resolved: .isResolved, comments: [.comments.nodes[] | {comment_id: .databaseId, url: .url, body: .body}]}'
```

The fixer reads this response and records the newly posted reply's `databaseId`/URL plus the
matching `ose-pr-review-disposition:v2` block and `Generated by AI` footer before considering
resolution. Resolution is not an acknowledgement button: `clarify`, an unaddressed thread, a
defer without its already-posted follow-up link, a rejected finding without a well-founded cited
reason, or a fix not committed and pushed all remain unresolved. For a fixed thread, first prove
the commit is on the current PR head and appears in the PR diff; a changed head returns to
`AR-REVIEW`, rather than resolving against stale evidence.

```bash
git fetch origin "$PR_BRANCH"
test -z "$(git status --porcelain)"
git merge-base --is-ancestor "$FIX_COMMIT" "origin/$PR_BRANCH"
test "$(git rev-parse "origin/$PR_BRANCH")" = "$REVIEWED_HEAD"
