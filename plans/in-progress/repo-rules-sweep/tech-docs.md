...
├── apps/rhino-cli/src/application/governance/readme_index.rs [E] — order-preserving generate,
│                                                                   rewrite-paths mode
...
flowchart LR
    ...
    P2 --> P3["Phase 3<br/>ose-public sweep"]
    P3 --> P4["Phase 4<br/>Knowledge Capture"]
    P4 --> P5["Phase 5<br/>Archival, then<br/>one PR"]
    P5 --> P6["Phase 6<br/>ose-private<br/>sweep and PR"]
    ...
    style P4 fill:#029E73,stroke:#000000,color:#FFFFFF
    style P5 fill:#029E73,stroke:#000000,color:#FFFFFF
    style P6 fill:#CC78BC,stroke:#000000,color:#000000
```

_Accessibility_: a single serial chain. Phase 0 baselines in the existing worktree; the blue phases
land the convention, the machinery, and the tooling; the orange phase performs the `ose-public`
sweep; the green phases capture knowledge and archive the plan; all of it merges as one PR before the
purple `ose-private` phase repeats the work there as its own single PR.

## 8. Dependencies

- Phase 2 must land before Phase 3: renaming before the generator is order-preserving would reorder
  every index.
- Phase 3 depends on Phase 1 for the rule the rename decisions are made against.
- Phase 6 depends on the `ose-public` PR having merged, so the `rhino-cli` change it copies is final.
- `ose-private` must be reachable and on a clean `main` before Phase 6.
- The worktree already exists and is already checked out; no provisioning step is required.
...
| 3 | Revert the sweep commits. Renames are `git mv` plus a `rewrite-paths` pass in the same commits, so a revert restores both sides. Because the PR is atomic, reverting it returns `ose-public` wholly to its pre-sweep state. |
| 4–5 | Revert the capture and archival commits; the plan folder returns to `plans/in-progress/`. |
| 6 | Revert in `ose-private` only. `ose-public` is unaffected, and the `parity-manifest` gate then reports the drift it exists to report. |
