...
- **Every URL segment under `paths/` renders, none 404 (observable, amendment A3)**: all six
  ...
  carries a title and a `description` frontmatter sentence — not body prose, which `generate-indexes`
  would overwrite in a childless section (DD-49) — so each renders a titled page, never a 404; the
  visible empty-state itself is plan-03's, verified before any of the three downstream
...
| Search traffic to the six relocated domains (~1,148 pages, ~67% of the `en/learn/` corpus) collapses — **accepted, not mitigated**, under the 2026-07-23 Q-D ruling.                                                                | The maintainer overturned Q-D's recommended default and chose **Option C — `noindex` the whole bucket** (see [Q-D](./tech-docs.md#q-d--seo-treatment-of-legacy) and the [resolution table](./tech-docs.md#open-questions--learn-section-scope-extension-resolved-2026-07-23)). Search de-indexing of the ~1,148 pages is therefore a **deliberately accepted cost**, not a mitigated risk: the 308s still preserve in-site nav and direct-link reachability, but the search surface is intentionally surrendered ahead of the replacement catalog. Reversible to A/B in one metadata change once the courses exist. |
| Legacy material and canonical courses cover the same subject, and a reader studies the superseded one.                                                                                                                              | Under Q-D Option C there is **no per-page banner**; supersession is signalled by `noindex` (the page is dropped from search, the primary discovery path) plus recording the supersession in the surviving course's `overview.md` ([Q-A](./tech-docs.md#q-a--is-legacy-a-staging-pen-or-a-permanent-archive)); the bucket is a staging pen expected to shrink, not a permanent parallel library.                                                                                                                                                                                                                     |
...
| A missing structural index under `paths/` (`paths/careers/`, an arc, or `paths/skills/`) is a dead URL segment (R7) — `paths/careers/_index.md` and `paths/skills/_index.md` sit on a shared seam three sibling plans build toward. | All five deeper indexes are created **in the same Phase 1 step** as `paths/_index.md` (amendment A3, DD-49), each with a title and a `description` frontmatter sentence — not body prose, which `generate-indexes` would overwrite in a childless section — so it renders a titled page, never a 404 or a synthesized `weight: 0` node; Phase 1 gate asserts all six exist. The visible empty-state **design** is `ayokoding-learning-path-03-navigation-ui`'s, not this plan's — see [tech-docs.md DD-49](./tech-docs.md#design-decisions).                                                                        |
