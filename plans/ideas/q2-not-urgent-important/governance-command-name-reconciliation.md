...
**Scripts invoking a removed subcommand — in ose-private only.** There, `npm run sync:agents`,
...
claim above was first written against ose-private's `package.json` and asserted of both repos without
...
`CLAUDE.md` in ose-private hits an unrecognized-subcommand error and has no signal about what
