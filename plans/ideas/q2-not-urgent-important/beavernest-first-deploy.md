...
One-line summary: provision the first real deploy targets for `beavernest-app-web`/`beavernest-be` —
...
Now that BeaverNest lives in `ose-public` as `beavernest-app-web`/`beavernest-be`
(`apps-beavernest-app-web-deployer`, `apps-beavernest-be-deployer`), the same gap persists here:
pushing to a `prod-beavernest-app-web`/`stag-beavernest-be` branch today reaches nothing, because no
such branch, Vercel project, or GHCR consumer exists in `ose-public` either.
...
- `apps/beavernest-app-web/README.md` and `apps/beavernest-be/README.md` document the intended
...
- For `beavernest-app-web`: provision a Vercel project building from `prod-beavernest-app-web`.
...
In scope: the first real `prod-beavernest-app-web` Vercel project and the first real
...
- Is a Vercel project for `beavernest-app-web` provisioned manually (human `[HUMAN]` step) or can it
  be automated? Account/billing implications likely make this a human step.
...
Success: a real, working `prod-beavernest-app-web` URL and a real running `beavernest-be` staging
