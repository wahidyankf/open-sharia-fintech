...
- **No rendered surface regresses** (observable): after `content-url.ts` gains the optional `pathId`
  param and the canonical `/en/learn/courses/<course-id>` shape, existing learn pages in **both**
...
- **Owning, authoring, or scoping the `skills/` URL category.** Both `skills/` path landings, both
  skills manifests, and the entire ERP + accounting course corpus belong end-to-end to a separate,
  not-yet-created plan (R4). This plan's schema and resolvers are built category-agnostic (R2) so that
  plan can consume them without a rewrite, but this plan authors none of the skills content itself.
...
| **The `pathId` schema hardcodes a fixed segment count (e.g. always 3), silently breaking the future `skills/` category (2-segment ids) or forcing a rewrite when it lands.**                                                                                              | `.refine()` validates only the first segment (`careers`/`skills`) and manifest resolvability — never segment count (R2). Proven by a unit-test fixture pair (2-segment skills + 3-segment careers, both valid) with no fixture asserting a specific depth (R2/R8, Phase 1.2).                                                                                                                                                                                                                                                                                             |
