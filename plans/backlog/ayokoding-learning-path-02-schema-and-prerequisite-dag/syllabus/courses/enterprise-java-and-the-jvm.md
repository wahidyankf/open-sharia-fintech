# Enterprise Java and the JVM (By Example, Java)
...
memory model). The primer [`84-just-enough-java`](./just-enough-java.md) gives you the language;
...
- **Prior topics**: [topic 84 Just Enough Java](./just-enough-java.md) (the language, records, streams,
  the memory model at a glance) and [topic 42 Software Architecture](./software-architecture.md) (layering,
...
- Enterprise Java's shape is a reaction to its own past. J2EE/EJB tried to standardize enterprise concerns
  ...
  hands forward: the DI + layered-service discipline reinforces [`42-software-architecture`](./software-architecture.md),
...
> _Content originated in the now-closed FS-SE plan (topic 85); it now lives here in
