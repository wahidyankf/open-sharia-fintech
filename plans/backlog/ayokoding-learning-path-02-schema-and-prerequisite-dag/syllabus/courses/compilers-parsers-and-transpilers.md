# Compilers, Parsers, and Transpilers (By Example, F#)
...
[`94-site-reliability-engineering`](./site-reliability-engineering.md)).
...
- **Prior topics**: [topic 87 Just Enough F#](./just-enough-fsharp.md) (the implementation language —
  discriminated unions, records, pattern matching, pipelines),
  [topic 19 Computer Science Foundations](./computer-science-foundations.md) (trees, recursion, grammars),
  and [topic 88 Type Systems](./type-systems.md) (ADTs + pattern matching make an AST + evaluator natural
...
- Early parser generators (yacc/bison-style LALR tools, and their F# analogue FsLexYacc) won for decades
  ...
  type-checker in [`88-type-systems`](./type-systems.md) reuses, which is the guardrail lens (DD-16)
...
> _Content originated in the now-closed FS-SE plan (topic 89); it now lives here in
> full — this course block is self-contained._

## In which paths — `capstone-concurrency-showdown` (DD-20)

Its only prerequisites (`csp-style-concurrency`, `actor-model-concurrency`) are already satisfied by
the end of this course's own section, so it is placed immediately after
`compilers-parsers-and-transpilers`:

- `interview-ready/software-engineer` — Go deeper · Concurrency, JVM & languages, right after `compilers-parsers-and-transpilers`.
- `immediately-effective/software-engineer` — Deepening band · Concurrency & language breadth, right after `compilers-parsers-and-transpilers`.
- `fundamentally-strong/software-engineer` — Stage 3 · Concurrency & language breadth, right after `compilers-parsers-and-transpilers`.

## In which paths — `capstone-concurrency-and-systems` (DD-20)

Its ordered steps require `site-reliability-engineering` (golden-signals instrumentation), which sits
in a **later** section than this course in all three manifests, so it is placed after that course
instead of here:

- `interview-ready/software-engineer` — Go deeper · Ops, platform, quality & product, right after `site-reliability-engineering`.
- `immediately-effective/software-engineer` — Deepening band · Scale, cloud & platform ops, right after `site-reliability-engineering`.
- `fundamentally-strong/software-engineer` — Stage 10 · Scale, cloud & platform ops, right after `site-reliability-engineering`.
