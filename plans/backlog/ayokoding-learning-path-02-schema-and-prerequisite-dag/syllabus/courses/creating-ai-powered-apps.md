# Creating AI-Powered Apps (By Example, Python)
...
[`37-data-engineering`](./data-engineering.md); it is served over a backend from
...
- **Prior topics**: [topic 11 Backend Essentials](./backend-essentials.md) (serving the app + calling
  an API), [topic 4 Just Enough Python](./just-enough-python.md), and
  [topic 15 Software Testing](./software-testing.md) (the eval/testing mindset for non-deterministic
  output).
...
- Before RAG (Lewis et al. 2020) and tool/function calling matured, LLM apps either fine-tuned a model
  ...
  frozen weights — which is exactly why [`39-backend-at-scale`](./backend-at-scale.md) (serving the
...
> _Content originated in the now-closed FS-SE plan (topic 56); it now lives here in
