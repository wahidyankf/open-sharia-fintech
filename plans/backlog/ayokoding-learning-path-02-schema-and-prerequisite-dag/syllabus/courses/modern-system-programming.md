# Modern System Programming (By Example, Rust)
...
counterpart to the C systems topic ([`81-system-programming`](./system-programming.md)): the same
...
[`82-just-enough-rust`](./just-enough-rust.md). `†`: Rust, driving `cargo` and building native binaries.
...
- **Prior topics**: [topic 82 Just Enough Rust](./just-enough-rust.md) (ownership/borrowing intuition,
  the type system, `Result`/`Option`, traits, pattern matching) and [topic 80 Windows OS](./windows-os.md)
...
- Systems programming lived on C and C++ for decades: unmatched control and performance, paid for with a
  ...
  manual-discipline model of [`81-system-programming`](./system-programming.md).
...
> _Content originated in the now-closed FS-SE plan (topic 83); it now lives here in
