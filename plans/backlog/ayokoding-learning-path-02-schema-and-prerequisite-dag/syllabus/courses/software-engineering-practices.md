# Software Engineering Practices (Annotated-concept, Python)
...
[`15-software-testing`](./software-testing.md); this topic is the surrounding workflow, driven from the
...
- **Prior topics**: [topic 5 Just Enough Bash](./just-enough-bash.md) (terminal + `git` CLI),
  [topic 15 Software Testing](./software-testing.md) (the testing discipline this workflow wraps), and a
  working app from Pass 1 (e.g. [topic 11 Backend Essentials](./backend-essentials.md)) to practice on.
...
- These practices are scar tissue from specific, expensive failures. Version control (SCCS → CVS → SVN → Git)
  ...
  is the ground the repo's own conventions and [`32-software-product-engineering`](./software-product-engineering.md) /
  [`09-project-management`](./project-management.md) build on.
...
> _Content originated in the now-closed FS-SE plan (topic 30); it now lives here in
