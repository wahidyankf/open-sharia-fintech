...
[`76-linux-app-development`](./linux-app-development.md); cross-OS contrast is
[`80-windows-os`](./windows-os.md).
...
- **Prior topics**: [topic 78 Just Enough C](./just-enough-c.md) (the language for syscalls) and
  [topic 5 Just Enough Bash](./just-enough-bash.md) (`/proc`, `ps`,
  `strace`).
...
- Early Unix systems ran cooperative, single-tasking programs with no memory protection between them — a
  ...
  same tradeoff, still visible today in [`76-linux-app-development`](./linux-app-development.md)'s use
...
> _Content originated in the now-closed FS-SE plan (topic 79); it now lives here in
