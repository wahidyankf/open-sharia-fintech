# Windows OS (By Example, C + PowerShell)
...
- **Prior topics**: [topic 78 Just Enough C](./just-enough-c.md) (the language for Win32 calls),
  [topic 79 Linux OS](./linux-os.md) (the OS-concept baseline to contrast), and
  [topic 5 Just Enough Bash](./just-enough-bash.md) (shell/PowerShell
  fluency).
...
- **Object/handle model** — `[Verified]` Windows is _object-based_: files, processes, threads, mutexes,
  ...
  to Linux fds (which are a VFS abstraction) — [`79-linux-os`](./linux-os.md).
...
- **co-30 · os-theory** — Win32's handles/objects/working-sets are one implementation of universal OS concepts also seen in [`79-linux-os`](./linux-os.md).
...
> _Content originated in the now-closed FS-SE plan (topic 80); it now lives here in
