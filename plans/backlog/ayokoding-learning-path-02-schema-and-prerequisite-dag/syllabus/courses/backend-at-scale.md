# Backend at Scale (By Example, Python)
...
testing. The usable slice is the prerequisite [`11-backend-essentials`](./backend-essentials.md);
system-level scaling is [`44-system-design`](./system-design.md).
...
- **Prior topics**: [topic 11 Backend Essentials](./backend-essentials.md) (routing, validation, DB
  access), [topic 10 SQL Essentials](./sql-essentials.md), [topic 17 Security Essentials](./security-essentials.md)
  (auth to deepen), and [topic 15 Software Testing](./software-testing.md) (integration/contract tests).
...
- These patterns are the industry's answer to the shift from single-server apps to always-on internet-scale
  ...
  failure is on your path. This scales up to systems in [`44-system-design`](./system-design.md) and to
  async workflows in [`45-event-driven-architecture`](./event-driven-architecture.md).
...
> _Content originated in the now-closed FS-SE plan (topic 39); it now lives here in
