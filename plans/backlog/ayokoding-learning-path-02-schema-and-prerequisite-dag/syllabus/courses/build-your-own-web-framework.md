# Build Your Own Web Framework (By Example, Python)
...
[`39-backend-at-scale`](./backend-at-scale.md), it makes the "magic" of `@app.route` and
...
- **Prior topics**: [topic 11 Backend Essentials](./backend-essentials.md) (routing, request
  handling, status codes as a _user_ of a framework) and [topic 39 Backend at Scale](./backend-at-scale.md)
...
> _Content originated in the now-closed FS-SE plan (topic 40); it now lives here in
