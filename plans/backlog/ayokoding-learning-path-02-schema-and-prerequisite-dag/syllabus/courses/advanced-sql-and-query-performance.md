# Advanced SQL and Query Performance (By Example, SQL + Python (PostgreSQL))
...
trade-offs. Basics are the prerequisite [`10-sql-essentials`](./sql-essentials.md); PostgreSQL is the
...
- **Prior topics**: [topic 10 SQL Essentials](./sql-essentials.md) (schema, CRUD, joins, transactions,
  parameterized queries) and [topic 4 Just Enough Python](./just-enough-python.md) for the DAL side;
  [topic 11 Backend Essentials](./backend-essentials.md) provides the N+1 scenario.
...
This topic **extends** [topic 10 SQL Essentials](./sql-essentials.md) — where topic 10 taught `SELECT`/`JOIN`/
...
> _Content originated in the now-closed FS-SE plan (topic 26); it now lives here in
