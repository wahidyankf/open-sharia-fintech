# Just Enough Nvim (Primer, Neovim (ex-commands))
...
of scope here** and belong to [`03-extending-neovim`](./extending-neovim.md). Neovim is Apache-2.0
(Tier-1, DD-21); this primer precedes [`02-just-enough-lua`](./just-enough-lua.md), so configuration
...
- **Big ideas touched**: `mechanism-vs-policy` — vanilla Neovim is pure **mechanism**; the **policy** (your
  config, plugins, LSP) is deliberately deferred to [`02`](./just-enough-lua.md) and
  [`03`](./extending-neovim.md).
...
> _Content originated in the now-closed FS-SE plan (topic 1); it now lives here in
