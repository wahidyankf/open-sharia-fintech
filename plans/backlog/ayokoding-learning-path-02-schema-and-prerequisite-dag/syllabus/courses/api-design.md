...
[`40-build-your-own-web-framework`](./build-your-own-web-framework.md). `†`: Python, fully
...
- **Prior topics**: [topic 11 Backend Essentials](./backend-essentials.md) (HTTP verbs, status
  codes, routing, JSON handling) and [topic 39 Backend at Scale](./backend-at-scale.md)
...
- API design consolidated as web services replaced hand-rolled RPC and SOAP: Fielding's REST
  ...
  This hands stable interfaces up to [topic 42 Software Architecture](./software-architecture.md)
  and its scaling/failure concerns back to [topic 39 Backend at Scale](./backend-at-scale.md).
...
> _Content originated in the now-closed FS-SE plan (topic 41); it now lives here in
