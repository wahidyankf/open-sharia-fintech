# Cloud and IaC (Annotated-concept, HCL/YAML)
...
no paid cloud account is required (DD-20). Containers/K8s are [`50-containers-and-orchestration`](./containers-and-orchestration.md).
...
- **Prior topics**: [topic 50 Containers & Orchestration](./containers-and-orchestration.md) (the
  workload to run), [topic 5 Just Enough Bash](./just-enough-bash.md)
  (CLI + env), and [topic 11 Backend Essentials](./backend-essentials.md) (the service being deployed).
...
- IaC answered the "works in prod, nobody knows why" era of hand-configured servers (snowflakes) —
  ...
  as immutable images in [`50-containers-and-orchestration`](./containers-and-orchestration.md).
...
> _Content originated in the now-closed FS-SE plan (topic 51); it now lives here in
