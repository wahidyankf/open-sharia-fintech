...
| Proceeding before `vercel-function-cost-reduction` merges compounds the site's dynamic-render cost problem with 7 more pages, or collides with its in-flight layout/middleware rewrite. | Phase 0 hard-gates on that plan's concrete file-based signal (root layout `headers()` call removed; `middleware.ts` deleted) before any authoring begins.                                                                                                                           |
