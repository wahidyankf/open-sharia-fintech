...
proceed on its own worktree, its own PR cadence, and its own review cycle.
...
| `ayokoding-learning-path-10-...`'s `build-your-own-raft` is blocked because `just-enough-go` lands late or incompletely. | Band 4 is authored as its own phase with its own delivery boundary and its own band-completion signal; `just-enough-go` is checked for its own structural completeness before that phase's gate passes, and the band signal names it explicitly among the four landed IDs. (Corrected from an earlier draft of this risk that cited `csp-style-concurrency`/`actor-model-concurrency` as the needed courses — the verified DAG edge, confirmed against `ayokoding-learning-path-10-...`'s own dependency table, is `just-enough-go` only.) |
