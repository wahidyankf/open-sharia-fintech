...
| Deploying this plan's PRs before `vercel-function-cost-reduction` lands compounds that plan's cost defect                                                 | Medium     | Medium | Phase 0's precondition check reads `apps/ayokoding-www/.next/prerender-manifest.json`'s route count before the first deploy; the check is re-run at each delivery boundary, not only once at plan start.             |
