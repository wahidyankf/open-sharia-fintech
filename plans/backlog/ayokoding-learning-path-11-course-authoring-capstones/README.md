...
| **blockedBy** | `ayokoding-learning-path-04-course-authoring`                                                                                                                                                            | **hard, baseline + Band 1/2 content** — carves Band 8 out of plan 04; needs `data-engineering`, `containers-and-orchestration`, `cloud-and-iac`, `cicd-and-release-engineering`, `async-python-and-fastapi-services`, `backend-at-scale` merged                                                                                                                                                                                                                                                                                                                                                                 |
| **blockedBy** | `ayokoding-learning-path-05-course-authoring-platform-and-concurrency`                                                                                                                                   | **hard, Band 4** — `capstone-concurrency-and-systems` and `capstone-concurrency-showdown` both declare `csp-style-concurrency` and `actor-model-concurrency` as direct prerequisites (confirmed — see tech-docs)                                                                                                                                                                                                                                                                                                                                                                                                |
...
to remove Band 8's scope from its checklist and hand it to this folder instead — the two edits are
independent (this plan touches only its own new folder; plan 04's trim touches only plan 04's
folder). Five other sibling agents are concurrently authoring the remaining band splits
(`05` Bands 3+4, `06` Band 5, `07` Band 6a, `08` Band 7, `09` Band 9, `10` Band 6b) in the same
swarm event; this plan's dependency claims on those plans were verified directly against each
sibling's own `README.md` where it already existed at this plan's authoring time (05, 06, 08 all
