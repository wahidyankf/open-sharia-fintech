# 76 · Just Enough C++ (Primer, C++)

**Mapping row** (frozen [tech-docs §Canonical Mapping Table](../tech-docs.md#canonical-mapping-table)):
N=76 · Phase 3 · Deepening (low-level systems) · Primer · C++ · folder weight 860 / learn 176 / drill 276. **NEW (Addition 4)** — systems-language principle on-ramp (manual memory / RAII / templates).
**Prereq: [N=75 `just-enough-c`](./README.md). Difficulty: intermediate** (per DN-14). Wazuh's C++ core
is one illustration, not the subject.
...
builds directly on [N=75 Just Enough C](./README.md) (pointers, structs, the compile/link loop) and
keeps that a pure C ramp. Proof-of-transfer target: `wazuh/wazuh` (C++ manager/agent core), not a
subject.
...
- **Prior topics**: [N=75 Just Enough C](./README.md) (pointers, structs, `stdio`, the compile/link
  loop, `make`) — a hard prerequisite; [N=4 Just Enough Python](./README.md) for a high-level contrast.
...
- C++ began as "C with classes" and grew RAII, templates, the STL, and (later) move semantics and smart
  ...
  unacceptable. This primer builds on [N=75 Just Enough C](./README.md) and equips the reader for the
  low-level systems topics and C++ codebases.

## Worked examples

Colocated under `just-enough-cpp/learning/code/`; each built via `g++`/`clang++` + `cmake`
(DD-20/DD-30), compiled warning-clean. Contiguous `ex-01..ex-72`. Every example cites the `co-NN` it
...
← Previous: N=75 `just-enough-c` ([index](./README.md)) · Next: N=77 `linux-os` ([index](./README.md)) →
