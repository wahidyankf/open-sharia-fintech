# 71 · Agent Tools & MCP (By Example, Python)

**Mapping row** (frozen [tech-docs §Canonical Mapping Table](../tech-docs.md#canonical-mapping-table)):
N=71 · Phase 3 · Deepening (AI & harness engineering — harness cluster) · By Example · Python · folder
weight 810 / learn 171 / drill 271. **NEW (Addition 2)** — the same MCP `remotebrowser` exposes. Cluster
language **Python** (DN-12).
...
loop consumes. Builds directly on [N=70 The Agent Loop](./70-the-agent-loop.md) (which dispatches tools)
and connects to [N=69 Browser Automation with CDP](./69-browser-automation-with-cdp.md) (the browser
...
- **Prior topics**: [N=70 The Agent Loop](./70-the-agent-loop.md) (tool dispatch), [N=20 Async Python &
  FastAPI Services](./20-async-python-and-fastapi-services.md) (JSON-RPC-shaped servers, async),
  [N=22 API Design](./README.md) (contract design), [N=4 Just Enough Python](./README.md).
- **Tools & environment**: a macOS/Linux terminal; Python 3.x under `uv`; an MCP SDK/library (e.g.
  ...
  [N=70](./70-the-agent-loop.md); `pytest`; Neovim/VSCode.
...
19. **co-19 · security-at-the-tool-boundary** — a tool executes real actions, so its inputs are untrusted
    and must be validated and constrained (links forward to [N=73](./73-agent-permissions-and-sandboxing.md)).
...
22. **co-22 · exposing-a-service-as-mcp** — wrapping an existing service (e.g. the
    [browser service](./69-browser-automation-with-cdp.md)) as an MCP server makes it usable by any agent
...
- **Tool power vs safety**: the more a tool can do (run shell, write files, hit the network), the more
  ...
  [N=73](./73-agent-permissions-and-sandboxing.md) — do not ship a raw shell tool ungated.
...
- Early agent tools were hard-coded per agent, so every integration was bespoke and non-portable. MCP
  ...
  loop](./70-the-agent-loop.md) its capabilities; [N=73 Permissions &
  Sandboxing](./73-agent-permissions-and-sandboxing.md) constrains them and
  [N=74 Orchestration](./74-agent-orchestration-subagents-and-observability.md) composes them.

## Worked examples

Colocated under `agent-tools-and-mcp/learning/code/`. Tools + MCP servers/clients in Python, wired to
the [N=70 loop](./70-the-agent-loop.md); a fake model keeps tests deterministic. Contiguous
...
14. **ex-14 · mcp-prompt-template** — expose a parameterized prompt — verify the client fetches it.
    (co-12)
15. **ex-15 · connect-mcp-to-loop** — the [agent loop](./70-the-agent-loop.md) discovers + calls an MCP
...
1. **ex-35 · browser-mcp-server** — wrap the [CDP browser service](./69-browser-automation-with-cdp.md)
   as an MCP server exposing navigate/evaluate/screenshot tools — verify a client drives the browser via
   MCP. (co-22, co-14)
2. **ex-36 · agent-drives-browser-over-mcp** — the [agent loop](./70-the-agent-loop.md) uses the browser
...
11. **ex-45 · sandboxed-tool-execution** — run a tool inside a sandbox boundary (preview of
    [N=73](./73-agent-permissions-and-sandboxing.md)) — verify isolation. (co-19)
...
- **Goal**: build a complete **MCP tool provider** — a server exposing well-designed tools (with typed
  ...
  it into the [agent loop](./70-the-agent-loop.md) so the agent discovers and uses it to complete a task,
  including (optionally) the [browser service](./69-browser-automation-with-cdp.md) exposed as MCP.
...
← Previous: [N=70 · The Agent Loop](./70-the-agent-loop.md) · Next:
