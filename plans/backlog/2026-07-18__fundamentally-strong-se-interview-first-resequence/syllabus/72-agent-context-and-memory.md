# 72 · Agent Context & Memory (Annotated-concept, Python)

**Mapping row** (frozen [tech-docs §Canonical Mapping Table](../tech-docs.md#canonical-mapping-table)):
N=72 · Phase 3 · Deepening (AI & harness engineering — harness cluster) · Annotated-concept · Python ·
folder weight 820 / learn 172 / drill 272. **NEW (Addition 2)**. Cluster language **Python** (DN-12).
...
where architecture is the point. Extends [N=70 The Agent Loop](./70-the-agent-loop.md) (whose message
history is the raw context) and draws on [N=67 Creating AI-Powered Apps](./README.md) (embeddings/RAG).
...
- **Prior topics**: [N=70 The Agent Loop](./70-the-agent-loop.md) (message history),
  [N=67 Creating AI-Powered Apps](./README.md) (embeddings, vector search, RAG),
  [N=57 Search & Information Retrieval](./README.md) (retrieval fundamentals),
...
- **Tools & environment**: a macOS/Linux terminal; Python 3.x under `uv`; a tokenizer library; an
  ...
  [N=70](./70-the-agent-loop.md); Neovim/VSCode.
...
- Early agents simply appended every message until they hit the window and broke. As tasks grew, three
  ...
  loop](./70-the-agent-loop.md) needs to run long tasks, and it draws its retrieval machinery from
  [N=57 Search & IR](./README.md) and [N=67 AI-Powered Apps](./README.md). It feeds
  [N=74 Orchestration](./74-agent-orchestration-subagents-and-observability.md), where subagents are a
...
8. **ex-20 · compaction-in-the-loop** — integrate compaction into the [agent loop](./70-the-agent-loop.md)
...
- **Goal**: extend the [agent loop](./70-the-agent-loop.md) with a full **context-management layer** —
...
← Previous: [N=71 · Agent Tools & MCP](./71-agent-tools-and-mcp.md) · Next:
