# 20 · Async Python & FastAPI Services (By Example, Python)

**Mapping row** (frozen [tech-docs §Canonical Mapping Table](../tech-docs.md#canonical-mapping-table)):
N=20 · Phase 2 · Multi-Platform Productivity (web sub-phase) · By Example · Python · folder weight 300 /
learn 120 / drill 220. **NEW (Addition 1)** — productivity module; the async-Python / FastAPI stack the
`remotebrowser` target codebase is built on.
...
[N=19 Backend Essentials](./README.md) (which teaches HTTP backends with persistence generally); this
module goes deeper into async and the FastAPI/Pydantic idiom for **production services**. It keeps
[N=4 Just Enough Python](./README.md) a lean interview refresh — the depth lives here, where web/backend
productivity lives. Proof-of-transfer target: `remotebrowser` (async Python + FastAPI + `fastmcp`), not
a subject.
...
- **Prior topics**: [N=4 Just Enough Python](./README.md), [N=19 Backend Essentials](./README.md)
  (HTTP, routing, persistence), and [N=13 SQL Essentials](./README.md) for the data layer.
...
> Pre-authoring `web-researcher` sweep pending (DD-28 convention). This stack moves fast — pin every
...
- Python services moved from synchronous WSGI (one worker per concurrent request) to ASGI + async
  ...
  stack of slower tools. This module deepens [N=19 Backend Essentials](./README.md) toward production
  services and feeds [N=69 Browser Automation with CDP](./69-browser-automation-with-cdp.md) and the
  harness cluster.
...
← Previous: N=19 `backend-essentials` ([index](./README.md)) · Next: N=21 `networking-essentials`
([index](./README.md)) →
