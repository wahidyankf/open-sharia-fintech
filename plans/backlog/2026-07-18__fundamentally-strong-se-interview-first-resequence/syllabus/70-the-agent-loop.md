# 70 · The Agent Loop (By Example, Python)

**Mapping row** (frozen [tech-docs §Canonical Mapping Table](../tech-docs.md#canonical-mapping-table)):
N=70 · Phase 3 · Deepening (AI & harness engineering — harness cluster) · By Example · Python · folder
weight 800 / learn 170 / drill 270. **NEW (Addition 2)** — first module of the build-your-own-coding-
agent cluster. Cluster language is **Python** (DN-12: matches the series primary, `remotebrowser`, and
the async-Python module).
...
the cluster ([N=71–74](./71-agent-tools-and-mcp.md)) extends, and the deep counterpart to the user-side
[N=66 Agentic Coding](./README.md) and [N=68 Agentic AI](./README.md).
...
- **Prior topics**: [N=67 Creating AI-Powered Apps](./README.md) and [N=68 Agentic AI](./README.md)
  (LLM APIs, tool-use concepts), [N=20 Async Python & FastAPI Services](./20-async-python-and-fastapi-services.md)
  (async + streaming), [N=4 Just Enough Python](./README.md).
...
- Agents grew out of the observation that a model which can _request actions_ and _read their results_
  ...
  observability). This module builds that core; [N=71 Agent Tools & MCP](./71-agent-tools-and-mcp.md)
  gives it capabilities, [N=72 Agent Context & Memory](./72-agent-context-and-memory.md) manages its
  state, [N=73 Agent Permissions & Sandboxing](./73-agent-permissions-and-sandboxing.md) constrains it,
  and [N=74 Orchestration & Observability](./74-agent-orchestration-subagents-and-observability.md)
  scales it — culminating in the [coding-agent capstone](./74c-capstone-build-your-own-coding-agent.md).
...
13. **ex-47 · agent-with-browser-tool** — wire the [CDP browser service](./69-browser-automation-with-cdp.md)
...
← Previous: [N=69 · Browser Automation with CDP](./69-browser-automation-with-cdp.md) · Next:
[N=71 · Agent Tools & MCP](./71-agent-tools-and-mcp.md) →
