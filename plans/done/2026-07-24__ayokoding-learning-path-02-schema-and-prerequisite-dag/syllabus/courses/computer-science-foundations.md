# Computer Science Foundations (Annotated-concept, Python)
...
Depth in specific areas is spread across [`25-advanced-algorithms`](./advanced-algorithms.md) and
[`88-type-systems`](./type-systems.md); this topic builds the mental model they hang on.
...
- **Prior topics**: [topic 4 Just Enough Python](./just-enough-python.md) (the small demonstrations
  are Python); [topic 7 Data Structures & Algorithms Essentials](./data-structures-and-algorithms-essentials.md)
...
- **co-16 · memory-hierarchy-intuition** — registers → cache → RAM → disk trade capacity for latency
  (survey depth here; full treatment in [`20-computer-architecture`](./computer-architecture.md)).
...
> _Content originated in the now-closed FS-SE plan (topic 19); it now lives here in
