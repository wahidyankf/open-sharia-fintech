# Build Your Own Reactive UI (By Example, TypeScript)
...
[`47-advanced-frontend`](./advanced-frontend.md), it turns "the framework re-renders" into a
...
- **Prior topics**: [topic 13 Just Enough TypeScript](./just-enough-typescript.md) (generics,
  discriminated unions, strict typing) and [topic 47 Advanced Frontend](./advanced-frontend.md)
...
- **Compiled fine-grained (Solid)** — "Instead of using a Virtual DOM, it compiles its templates to real
  ...
  that depends on it will rerun." Source: [solidjs/solid README](https://github.com/solidjs/solid) (fetched). `docs.solidjs.com` / `solidjs.com` remained Cloudflare-blocked (HTTP 403) on re-attempt 2026-07-22 — verbatim SolidJS-docs quotes stay `[Unverified]`; the compile-to-real-DOM / fine-grained claim is however sourced to the fetched solidjs/solid README above.
- **Observer pattern** — ReactiveX "extends the observer pattern to support sequences of data and/or events"
  and is "a 'push' equivalent to Iterable, which is a 'pull'." Source: [ReactiveX Introduction](https://reactivex.io/intro.html) (fetched). The GoF 1994 "Observer" definition is bibliographically solid but `[Unverified]` as a fetched primary quote — the source is a print book (Gamma, Helm, Johnson & Vlissides, _Design Patterns_, 1994) with no fetchable primary URL, so no verbatim web citation this pass (2026-07-22).
- **Hooks call-order rule** — "you can only call [Hooks] at the top level of your component … You can't call
  [them] inside loops or conditions." Source: [react.dev, useState](https://react.dev/reference/react/useState) + [Rules of Hooks](https://react.dev/reference/rules/rules-of-hooks) (fetched). The _why_ (per-fiber call-index state) is community/Fiber knowledge, `[Unverified]` as an official quote — no official React doc states the per-fiber call-index mechanism verbatim (searched 2026-07-22); teach it as widely-understood implementation lore, not a cited spec.
...
- **Reactivity glitches / diamond** — MobX's creator: "All reactions happen synchronously and … glitch-free,"
  ...
  dependency] has become stable." Source: [Weststrate, "Becoming fully reactive"](https://medium.com/hackernoon/becoming-fully-reactive-an-in-depth-explanation-of-mobservable-55995262a254) (2015, author = MobX creator; personal blog, not official docs). Note: the claim that Knockout's docs coin "diamond dependency problem" / expose `ko.atomically()` was **not** confirmed — re-searched Knockout's docs/repo 2026-07-22 and found `ko.dependencyDetection` and a "cascade disposals" GitHub issue, but neither a documented "diamond dependency problem" term nor a `ko.atomically()` API; treat both namings as `[Unverified]` and do not restate either as fact.
- **Disposal / cleanup** — Vue's `effectScope()` "can capture the reactive effects … so that these effects
  ...
  stopped." Source: [Vue — Reactivity Advanced](https://vuejs.org/api/reactivity-advanced.html) (fetched). SolidJS `onCleanup` "runs when that scope is disposed or refreshed" — `[Unverified]` (docs.solidjs.com remained Cloudflare-blocked / HTTP 403 on re-attempt 2026-07-22).
...
> _Content originated in the now-closed FS-SE plan (topic 48); it now lives here in
