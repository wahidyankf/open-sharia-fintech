# Building Production CLI Tools (By Example, Go + Rust)
...
The telemetry instincts from [`63-analytics-and-experimentation`](./analytics-and-experimentation.md)
inform how a tool reports itself, and the shell fluency from
[`05-just-enough-bash`](./just-enough-bash.md) is what your tool has to compose with. `†`: Go and Rust
...
- **Prior topics**: [topic 63 Analytics & Experimentation](./analytics-and-experimentation.md)
  (honest measurement and self-reporting) and [topic 5 Just Enough Bash](./just-enough-bash.md) (pipes,
...
- CLI conventions were forged in early Unix: small single-purpose tools composed through pipes, with stdout
  ...
  carry straight into the lower-level tooling of [`78-just-enough-c`](./just-enough-c.md) and every
...
> _Content originated in the now-closed FS-SE plan (topic 77); it now lives here in
