# Self-Managed Kubernetes and GitOps (By Example, YAML/CLI)
...
and core K8s objects are [`50-containers-and-orchestration`](./containers-and-orchestration.md);
cloud-provider provisioning is [`51-cloud-and-iac`](./cloud-and-iac.md); the VMs underneath are
[`52-bare-metal-virtualization`](./bare-metal-virtualization.md). Here the cluster **you** operate is
the unit.
...
- **Big ideas touched**: `mechanism-vs-policy` (you declare cluster + app desired state — the _policy_;
  ...
  survival) — see prd Cross-Cutting Big Ideas.

## Prerequisites

- **Prior topics**: [topic 50 Containers & Orchestration](./containers-and-orchestration.md) (images,
  ...
  [topic 52 Bare-Metal Virtualization](./bare-metal-virtualization.md) (the VMs/hosts the cluster
  nodes run on), and [topic 46 Distributed Systems](./distributed-systems.md) (consensus, quorum,
...
> _Content originated in the now-closed FS-SE plan (topic 53); it now lives here in
