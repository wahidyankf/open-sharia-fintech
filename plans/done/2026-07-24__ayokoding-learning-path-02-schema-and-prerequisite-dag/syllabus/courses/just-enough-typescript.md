# Just Enough TypeScript (Primer, TypeScript)
...
[`14-frontend-essentials`](./frontend-essentials.md) / [`47-advanced-frontend`](./advanced-frontend.md)
and the TS side of [`15-software-testing`](./software-testing.md). Node/TS are Tier-1 OSS (DD-21).
...
- **Prior topics**: [topic 1 Just Enough Nvim](./just-enough-nvim.md) (to edit/run); prior
  programming maturity from [topic 4 Just Enough Python](./just-enough-python.md) helps but is not
  required.
...
> _Content originated in the now-closed FS-SE plan (topic 13); it now lives here in
