# CSP-Style Concurrency (By Example, Go)
...
[`67-actor-model-concurrency`](./actor-model-concurrency.md). Deepens the concepts from
[`24-concurrency-and-parallelism`](./concurrency-and-parallelism.md).
...
- **Prior topics**: [topic 64 Just Enough Go](./just-enough-go.md) (the language + a channel preview) and
  [topic 24 Concurrency & Parallelism](./concurrency-and-parallelism.md) (races, deadlocks, the shared-
...
- **CSP vs actor contrast** — framed against Elixir/BEAM in
  [`67-actor-model-concurrency`](./actor-model-concurrency.md): CSP = synchronous rendezvous over
...
- **co-26 · csp-vs-actor** — CSP (Go channels: anonymous processes, synchronous rendezvous, the channel is
  ...
  set up for [`67-actor-model-concurrency`](./actor-model-concurrency.md).
...
> _Content originated in the now-closed FS-SE plan (topic 65); it now lives here in
