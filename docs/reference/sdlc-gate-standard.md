...
| #   | Command                                                                      | Scope              | What it does                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| --- | ---------------------------------------------------------------------------- | ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1   | `nx affected -t test:quick`                                                  | affected projects  | Runs typecheck → lint → test:unit → test:coverage (≥90%) → test:specs (all `specs:*` validators) per affected project. Projects with a real `compat:min-version` (F#, TypeScript) also run `nx affected -t compat:min-version` (cacheable). `deps:audit` is not here — it is uncacheable (network advisory DB) and runs CRON-only.                                                                                                                                       |
...
byte/behavior-identical across both; ⚠️ = tracked, non-blocking divergence with a linked follow-up;
...
| lint-staged full set (emoji, gherkin-cardinality, docker-compose)                                     | ✅     | Identical entries in both `package.json` (only per-language formatter entries differ, an allowed divergence).                                                                                                                                                                                                                                                                          |
...
| specs-gate set (structure + behavior-coverage + domain-coverage + gherkin-cardinality)                | ✅     | Identical job/target composition in both.                                                                                                                                                                                                                                                                                                                                              |
...
| rhino-cli command set, verb-last                                                                      | ✅     | `specs counts validate` and the `behavior-coverage`/`domain-coverage` verbs are now identical in both; infra's former standalone `bc`/`ul` leaves (already-duplicate logic vs. `structure validate`) are removed.                                                                                                                                                                      |
...
| Mandatory targets on every project                                                                    | ✅     | `typecheck`/`lint`/`test:unit`/`test:integration`/`test:e2e`/`test:quick` present on every project in both (0 `MISSING` in the jq sweep).                                                                                                                                                                                                                                              |
...
| specs/ C4 structure (every app + every lib)                                                           | ✅     | Every spec area across both repos has `product/`, `system-context/`, `containers/`, `components/`, `behavior/gherkin/` — including all app-level libs (4 in public, 2 in infra) that were missing this structure entirely before this pass.                                                                                                                                            |
