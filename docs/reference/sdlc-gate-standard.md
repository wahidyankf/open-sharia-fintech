...
| 6   | `apps/rhino-cli/scripts/rhino-bin.sh repo-governance vendor validate`                                                                            | other (path-gated)                      | Governance docs stay vendor-neutral, no vendor leakage. **Trigger:** `repo-governance/**.md`.                                                                                                                                                                                                                                                                                                                                                           |
...
| 8   | `apps/rhino-cli/scripts/rhino-bin.sh governance word-budget validate`                                                                            | dark-launched (not yet wired as a gate) | Auto-loaded instruction files stay within their per-surface word budget. Surfaces declared explicitly in `repo-config.yml` `governance-word-budget:` (no registry-merge). Covers: `repo-governance/**/*.md`, `AGENTS.md`, `CLAUDE.md`, `.claude/**/*.md`, `.cursor/**/*.md`, `.codex/**/*.md`, `.opencode/**/*.md`, `.pi/**/*.md`, `.amazonq/**/*.md`, `**/README.md`. Not yet registered in the `gates:` registry — arming is a later plan phase.      |

Each governance validator (rows 4–7) is path-gated — invoked only when its trigger path is in the
changed set. Row 8 is dark-launched: it runs on demand and inside `repo-governance audit`, but is
not registered in the `gates:` registry and does not block pre-push.
...
cross-repo run; `ose-primer`/`ose-private` propagation of the shim is tracked separately (see
...
that date — `instruction-size:` is still the live section name in `ose-primer` and `ose-private` as
of this note. The rename opens a cross-repo parity obligation (a `repo-config.yml` schema-section
rename is exactly the class of change the [Parity Status](#parity-status) table exists to track);
propagation to the sibling repos is not yet scheduled against a specific plan at the time of this
note.
