...
| `specs:behavior:coverage`    | `apps/rhino-cli/scripts/rhino-bin.sh specs behavior-coverage validate` | Every Gherkin step has a step definition, and every scenario is covered at exactly its required levels via explicit `@covers` markers.                                                                 | Every project |
...
| **specs-gate validator set (PR gate)**                                                     | structure (merged adoption + tree + counts) + behavior:coverage + gherkin-cardinality (full); spec links via repo-wide `md links validate`                                                                       | Public's fuller set wins; a bound repo deferring any structural validator must promote it.                                                                                                                                                                                                                             |
