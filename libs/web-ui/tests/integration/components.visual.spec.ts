...
async function loadStory(page: StoryPage, storyId: string, theme?: "light" | "dark"): Promise<void> {
  ...
  // `@font-face` swaps in asynchronously; a screenshot racing that swap captures the fallback font's
  // metrics (different glyph widths/line heights), producing a few dozen to a few hundred stray
  // diff pixels against a baseline captured post-swap — the exact 1%-ratio flakiness this waits out.
...
test.describe("CopyButton", () => {
  test("resting state renders correctly in light theme", async ({ page }) => {
    ...
    const screenshot = await page.locator(STORYBOOK_ROOT).screenshot({ animations: "disabled" });
