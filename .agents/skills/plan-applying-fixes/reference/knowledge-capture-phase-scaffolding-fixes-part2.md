...
- [ ] [AI] Route each surviving learning to exactly one durable home per the open-ended routing
      matrix; code homes (`apps/`, `libs/`, tests) are NEVER landed inline (the only carve-out is a
      genuine blocker required to finish this plan's own scope). Only after the overlap scan,
      create or update a `plans/ideas/` two-pager with literal plan-artifact authorization; never
      create its backlog folder directly. Otherwise record `Reported without plan authorization`
...
- [ ] [AI] Every `learnings.md` entry is terminal (routed inline / filed with authorization /
      reported without plan authorization / discarded with reason), or the explicit "none" escape
      is recorded
