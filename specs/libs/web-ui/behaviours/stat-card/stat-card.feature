Feature: StatCard component
  ...
  Scenario: Renders InfoTip when info is provided
    When I render a StatCard with label "Steps" value "12500" unit "steps" hue "teal" icon "trend" and info "Daily step count"
    Then an InfoTip trigger should be visible

  Scenario: Does not render InfoTip when info is absent
    When I render a StatCard with label "Steps" value "12500" unit "steps" hue "teal" icon "trend" without info
