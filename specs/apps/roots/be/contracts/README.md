...
OpenAPI 3.1 specification for the Roots Sharia-compliance REST API.

## Purpose

This contract defines the exact shape of every request and response for `roots-be`. It is the
...
nx run roots-contracts:lint

# Bundle into single resolved YAML + JSON
nx run roots-contracts:bundle

# Generate browsable API documentation
nx run roots-contracts:docs
# Open specs/apps/roots/be/contracts/generated/docs/index.html
```
...
2. Run `nx run roots-contracts:lint` to validate
3. Run `nx run roots-contracts:bundle` to regenerate the bundled spec
4. Run codegen for the service: `nx run roots-be:codegen`
...
looks like. The specifications themselves share no components — `roots-be` is a separate product
...
- Changes to this contract trigger codegen for `roots-be` via the Nx dependency graph

- [generated](./generated/README.md) — Roots Contracts
- [paths](./paths/README.md) — Roots Contracts
- [schemas](./schemas/README.md) — Roots Contracts
