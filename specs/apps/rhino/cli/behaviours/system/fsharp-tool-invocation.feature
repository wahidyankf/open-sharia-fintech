...
Feature: Manifest-backed F# formatting checks
  ...
  # Exemption(e2e): repository lint-manifest inspection is an internal build-configuration boundary without a user command; alternative-proof: rhino-cli:test:integration / Every locally discovered F# lint target uses the pinned local Fantomas tool
  @e2e-exempt
