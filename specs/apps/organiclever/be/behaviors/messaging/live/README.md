...
Scenarios that need a live NATS/JetStream broker, e2e-tested by `organiclever-be-e2e` and
excluded from `organiclever-be:specs:behavior:coverage`'s unit-scoped drift scan via
`--exclude-dir live`.
