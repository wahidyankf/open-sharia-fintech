Feature: APP_ENV tier env-file loading
  ...
  As a developer running ose-www across local, staging, and production tiers
  ...
  @unit
  Scenario: ose-www builds against the staging tier
  ...
  @unit
  Scenario: ose-www process env wins over the local tier file
  ...
  @unit
  Scenario: ose-www tolerates a missing tier file
  ...
  @unit
  Scenario Outline: ose-www fails loudly on a stray auto-loaded env file
