Feature: APP_ENV tier env-file loading
  ...
  As a developer running ose-app-web across local, staging, and production tiers
  ...
  @unit
  Scenario: ose-app-web builds against the staging tier
  ...
  @unit
  Scenario: ose-app-web process env wins over the local tier file
  ...
  @unit
  Scenario: ose-app-web tolerates a missing tier file
  ...
  @unit
  Scenario Outline: ose-app-web fails loudly on a stray auto-loaded env file
