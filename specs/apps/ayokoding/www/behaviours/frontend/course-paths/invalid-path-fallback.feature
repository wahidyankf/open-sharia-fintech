Feature: Invalid path context fallback
  ...
  # Exemption(integration): the scenario is observable at the public browser or HTTP boundary and has no separate local resource boundary; alternative-proof: ayokoding-www-fe-e2e:test:e2e / An invalid path context falls back to the canonical view
  @integration-exempt
