...
const feature = await loadFeature(
  path.resolve(
    process.cwd(),
    "../../specs/apps/wahidyankf/behavior/wahidyankf-www/gherkin/env-loader/env-loader.feature",
  ),
);
...
function makeTmpAppDir(): string {
  const dir = mkdtempSync(path.join(tmpdir(), "wahidyankf-www-env-loader-test-"));
  tmpDirs.push(dir);
  return dir;
}
...
describeFeature(feature, ({ Scenario, ScenarioOutline }) => {
  Scenario("wahidyankf-www builds against the staging tier", ({ Given, When, Then }) => {
    let appDir: string;
    let env: EnvRecord;
    ...
    // @covers specs/apps/wahidyankf/behavior/wahidyankf-www/gherkin/env-loader/env-loader.feature:wahidyankf-www builds against the staging tier
  ...
  Scenario("wahidyankf-www process env wins over the local tier file", ({ Given, When, Then, And }) => {
    let appDir: string;
    let env: EnvRecord;
    ...
    // @covers specs/apps/wahidyankf/behavior/wahidyankf-www/gherkin/env-loader/env-loader.feature:wahidyankf-www process env wins over the local tier file
    And('the ".env.local" value is not applied over it', () => {
      expect(env["SOME_VAR"]).not.toBe("file-value");
    });
  });

  Scenario("wahidyankf-www tolerates a missing tier file", ({ Given, When, Then, And }) => {
    let appDir: string;
    let env: EnvRecord;
    let thrown: unknown;
    ...
    // @covers specs/apps/wahidyankf/behavior/wahidyankf-www/gherkin/env-loader/env-loader.feature:wahidyankf-www tolerates a missing tier file
    And("startup proceeds using whatever the process environment already supplies", () => {
      expect(env["EXISTING_VAR"]).toBe("already-set");
    });
  });

  ScenarioOutline("wahidyankf-www fails loudly on a stray auto-loaded env file", ({ Given, When, Then }, variables) => {
    let appDir: string;
    let thrown: unknown;
    ...
    // @covers specs/apps/wahidyankf/behavior/wahidyankf-www/gherkin/env-loader/env-loader.feature:wahidyankf-www fails loudly on a stray auto-loaded env file
