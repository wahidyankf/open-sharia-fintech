...
// =============================================================================
// CLASS: BeavernestApiClientException
// =============================================================================

/// HTTP exception handler for BeavernestApiClient
class BeavernestApiClientException implements Exception {
  BeavernestApiClientException({
  ...
  String toString() {
    ...
    return 'BeavernestApiClientException($s)';
  }
...
// =============================================================================
// CLASS: BeavernestApiClient
// =============================================================================

/// Client for BeaverNest API
///
/// BeaverNest foundation contract. Liveness and readiness are the declared operations; unmatched API routes return a JSON error envelope. No write operations exist in this phase.
class BeavernestApiClient {
  /// Creates a new BeavernestApiClient instance.
  ///
  /// - [BeavernestApiClient.baseUrl] Override base URL (default: server url defined in spec)
  /// - [BeavernestApiClient.headers] Global headers to be sent with every request
  /// - [BeavernestApiClient.queryParams] Global query parameters to be sent with every request
  /// - [BeavernestApiClient.client] Override HTTP client to use for requests
  BeavernestApiClient({
    this.baseUrl,
    this.headers = const {},
  ...
  Future<http.StreamedResponse> _request({
    ...
    if (requestType.contains('multipart')) {
      ...
      try {
        if (body != null) {
          switch (requestType) {
            case 'application/x-www-form-urlencoded':
              var parts = [];
              Map<String, String> bodyMap = Map<String, String>.from(
                body as Map<dynamic, dynamic>,
              );
        ...
        throw BeavernestApiClientException(
          uri: uri,
          method: method,
  ...
  /// Liveness check
  ///
  /// Reports whether beavernest-be has finished starting and is serving traffic.
  ///
  /// `GET` `/api/v1/health`
  Future<Health> getHealth() async {
    ...
    return Health.fromJson(_jsonDecode(r));
  }
  ...
  /// Workspace readiness
  ///
  /// Reports the safe database and schema readiness state after startup migrations.
  ///
  /// `GET` `/api/v1/readiness`
  Future<ReadinessReady> getReadiness() async {
    final r = await makeRequest(
      baseUrl: 'http://localhost:19320',
