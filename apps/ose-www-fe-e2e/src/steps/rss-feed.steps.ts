...
When("the RSS feed is generated", async ({ request }) => {
  ...
  backendState.rssFeedBody = await response.text();
});
...
Then("the feed entry has the title {string}", async ({}, expectedTitle: string) => {
  const body = backendState.rssFeedBody as string;
  // Titles may be CDATA-wrapped and the Gherkin title may be a partial match, e.g. "Phase 0 End"
  // matches "End of Phase 0: Foundation Complete, Phase 1 Begins".
