...
import { useJournal } from "../../../../../src/contexts/journal/presentation/use-journal";
...
describe("useJournal", () => {
  ...
  it("exposes updateEntry, deleteEntry, bumpEntry, and clearEntries that drive the underlying machine", async () => {
    const runtime = makeTestRuntime();
    ...
    act(() => {
      result.current.updateEntry(id, { name: makeName("learning") });
    });
    await waitFor(
      () => {
        expect(result.current.entries[0]?.name).toBe("learning");
      },
      { timeout: 30000 },
    );

    act(() => {
      result.current.bumpEntry(id);
    });
    await waitFor(
      () => {
        expect(result.current.status).toBe("ready");
  ...
  it("retry re-triggers loading after an error", async () => {
    const failingLayer = Layer.scoped(
      PgliteService,
      Effect.acquireRelease(
        Effect.tryPromise({
    ...
    // Retry re-enters `initializing`, which always fails again against the
    // same failing layer, but it must transition out of `error` first —
    // this exercises the `retry` wiring itself, independent of outcome.
    await waitFor(
      () => {
        expect(result.current.status).toBe("error");
      },
