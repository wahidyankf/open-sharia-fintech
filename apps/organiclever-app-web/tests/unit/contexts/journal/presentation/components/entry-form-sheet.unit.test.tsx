...
import { JournalEntry } from "../../../../../../src/contexts/journal/domain/schema";
import { EntryFormSheet } from "../../../../../../src/contexts/journal/presentation/components/entry-form-sheet";
...
describe("EntryFormSheet - edit mode", () => {
  ...
  it("submits an edited payload after changing the payload textarea", async () => {
    const user = userEvent.setup();
    const onSubmit = vi.fn();
    const entry = makeEntry({ name: "workout", payload: { reps: 5 } });
    render(<EntryFormSheet open={true} mode="edit" initial={entry} onSubmit={onSubmit} onCancel={() => {}} />);

    const payloadInput = screen.getByLabelText("Payload (JSON object)");
    fireEvent.change(payloadInput, { target: { value: '{"reps":10}' } });

    await user.click(screen.getByRole("button", { name: /save/i }));

    expect(onSubmit).toHaveBeenCalledTimes(1);
    const patch = onSubmit.mock.calls[0]![0] as { payload: Record<string, unknown> };
    expect(patch.payload).toEqual({ reps: 10 });
  });
});
