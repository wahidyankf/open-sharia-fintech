...
import { JournalEntry, EntryId, EntryName, IsoTimestamp } from "../../../../../../src/contexts/journal/domain/schema";
import type { UseJournalReturn } from "../../../../../../src/contexts/journal/presentation/use-journal";
...
vi.mock("../../../../../../src/contexts/journal/presentation/use-journal", () => ({
  useJournal: () => mockReturn,
}));

// makeJournalRuntime is called inside JournalPage via useMemo — mock it to a no-op
vi.mock("../../../../../../src/contexts/journal/infrastructure/runtime", () => ({
  makeJournalRuntime: () => ({}),
  PgliteLive: {},
...
import { JournalPage } from "../../../../../../src/contexts/journal/presentation/components/journal-page";
...
describe("JournalPage", () => {
  ...
  it("shows the storage-unavailable banner when status is error with a StorageUnavailable tag", () => {
    mockReturn = { ...mockReturn, status: "error", error: { _tag: "StorageUnavailable" } as UseJournalReturn["error"] };
    render(<JournalPage />);
    expect(screen.getByTestId("storage-error-banner")).toHaveTextContent("Storage unavailable — data was not saved.");
  });

  it("shows a generic error banner when status is error with a non-StorageUnavailable tag", () => {
    mockReturn = { ...mockReturn, status: "error", error: { _tag: "NotFound" } as UseJournalReturn["error"] };
    render(<JournalPage />);
    expect(screen.getByTestId("storage-error-banner")).toHaveTextContent("An error occurred. Please reload.");
  });

  it("clicking Cancel on the create sheet closes it without calling addBatch", () => {
    mockReturn = { ...mockReturn, status: "ready", entries: [] };
    render(<JournalPage />);
  ...
  it("clicking Cancel on the edit sheet closes it without calling updateEntry", () => {
    const entry = makeEntry({ name: "workout", id: "entry-cancel-edit" });
    mockReturn = { ...mockReturn, status: "ready", entries: [entry] };
    render(<JournalPage />);
  ...
  it("closes the edit sheet automatically if the entry disappears while editing", () => {
    const entry = makeEntry({ name: "workout", id: "entry-gone" });
    mockReturn = { ...mockReturn, status: "ready", entries: [entry] };
    ...
    // Entry disappears (e.g. deleted from another tab) — JournalPage must
    // fall back to a closed sheet rather than rendering edit UI for a
    // now-nonexistent entry.
    mockReturn = { ...mockReturn, entries: [] };
