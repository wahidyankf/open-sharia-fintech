...
import {
  ...
  makeJournalRuntime,
  PgliteLive,
} from "../../../../../src/contexts/journal/infrastructure/runtime";
...
describe("runtime - makeJournalRuntime", () => {
  ...
  it("PgliteLive rejects with StorageUnavailable during SSR (no window)", async () => {
    ...
    try {
      ...
      if (exit._tag === "Failure") {
        ...
        if (failure._tag === "Some") {
          expect(failure.value).toBeInstanceOf(StorageUnavailable);
          expect((failure.value as StorageUnavailable).cause).toBeInstanceOf(Error);
          expect(((failure.value as StorageUnavailable).cause as Error).message).toContain("SSR");
        }
      }
    } finally {
  ...
  it("PgliteLive rejects with StorageUnavailable when the browser has no IndexedDB backend", async () => {
    // jsdom provides `window` but not a working IndexedDB, so the real
    // `idb://` PGlite backend PgliteLive opens in production cannot
    // initialize under this unit-test environment — it fails the same way
    // it would in any browser without IndexedDB support, exercising the
    // exact acquire-then-fail path production hits in that case.
    const exit = await Effect.runPromiseExit(
      Effect.scoped(
        Effect.provide(
          Effect.gen(function* () {
