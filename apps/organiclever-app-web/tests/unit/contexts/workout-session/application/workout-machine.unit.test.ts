...
import {
  ...
  resolvedRest,
  buildWorkoutEntry,
} from "../../../../../src/contexts/workout-session/application/workout-machine";
import type { WorkoutMachineContext } from "../../../../../src/contexts/workout-session/application/workout-machine";
...
describe("buildWorkoutEntry", () => {
  it("maps context exercises into the entry payload, preserving their data", () => {
    const exerciseWithSet: ActiveExercise = {
      ...mockExercise,
      sets: [{ reps: 10, weight: "20 kg", duration: null, restTaken: 60 }],
    };
    const context: WorkoutMachineContext = {
      routine: mockRoutine,
    ...
    const entry = buildWorkoutEntry(context);

    expect(entry.name).toBe("workout");
    expect(entry.payload.routineName).toBe("Test Routine");
    expect(entry.payload.durationSecs).toBe(300);
    expect(entry.payload.exercises).toHaveLength(1);
    expect(entry.payload.exercises[0]?.name).toBe("Push-up");
    expect(entry.payload.exercises[0]?.sets).toEqual([{ reps: 10, weight: "20 kg", duration: null, restTaken: 60 }]);
  });

  it("defaults routineName to null when there is no routine", () => {
    const context: WorkoutMachineContext = {
      routine: null,
    ...
    const entry = buildWorkoutEntry(context);
    expect(entry.payload.routineName).toBeNull();
    expect(entry.payload.exercises).toHaveLength(1);
  });
});
