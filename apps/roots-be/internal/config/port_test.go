...
func TestDefaultPortAppliesWhenNothingIsSet(t *testing.T) {
	got, err := config.ResolvePort("", envOf(map[string]string{}), "ROOTS_BE_PORT")
	if err != nil {
...
func TestPrefixedVariableOverridesTheDefault(t *testing.T) {
	got, err := config.ResolvePort("", envOf(map[string]string{"ROOTS_BE_PORT": "9402"}), "ROOTS_BE_PORT")
	if err != nil {
		t.Fatalf("expected no error, got %v", err)
	}
	if got != 9402 {
...
func TestFlagOverridesThePrefixedVariable(t *testing.T) {
	got, err := config.ResolvePort("9500", envOf(map[string]string{"ROOTS_BE_PORT": "9402"}), "ROOTS_BE_PORT")
	if err != nil {
		t.Fatalf("expected no error, got %v", err)
	}
	if got != 9500 {
...
func TestMalformedPortFailsAtStartup(t *testing.T) {
	got, err := config.ResolvePort("", envOf(map[string]string{"ROOTS_BE_PORT": "not-a-port"}), "ROOTS_BE_PORT")
	if err == nil {
		t.Fatalf("expected an error, got port %d", got)
	}
	if !strings.Contains(err.Error(), "ROOTS_BE_PORT") {
		t.Errorf("expected the message to name ROOTS_BE_PORT, got %q", err.Error())
	}
...
func TestBarePortVariableIsIgnored(t *testing.T) {
	got, err := config.ResolvePort("", envOf(map[string]string{"PORT": "9999"}), "ROOTS_BE_PORT")
	if err != nil {
		t.Fatalf("expected no error, got %v", err)
	}
	if got != 8402 {
...
func TestMalformedFlagFailsAtStartup(t *testing.T) {
	_, err := config.ResolvePort("not-a-port", envOf(map[string]string{}), "ROOTS_BE_PORT")
	if err == nil {
...
func TestOutOfRangePortsAreRejected(t *testing.T) {
	for _, raw := range []string{"0", "65536", "-1"} {
		if _, err := config.ResolvePort("", envOf(map[string]string{"ROOTS_BE_PORT": raw}), "ROOTS_BE_PORT"); err == nil {
