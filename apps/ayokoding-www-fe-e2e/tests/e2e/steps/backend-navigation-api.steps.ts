import { expect } from "@playwright/test";
import { createBdd } from "playwright-bdd";
import { backendState } from "./backend-helpers";

const { Given, Then } = createBdd();

// Feature-specific Given steps (fixtures — no-op, real data comes from the running app)
Given(
  "content exists in locale {string} with sections {string}, {string}, and {string}",
  async ({}, _locale: string, _s1: string, _s2: string, _s3: string) => {},
);
Given(
  "a section {string} in locale {string} has child nodes with weights {int}, {int}, and {int}",
  async ({}, _section: string, _locale: string, _w1: number, _w2: number, _w3: number) => {},
);
Given(
  "a section {string} in locale {string} contains at least one child page",
  async ({}, _section: string, _locale: string) => {},
);

// Note: When 'the client calls content.getTree with locale {string}' is in backend-common.steps.ts

Then('the response tree should contain top-level nodes for "programming", "ai", and "security"', async () => {
  const tree = backendState.treeResult as unknown[];
  expect(tree.length).toBeGreaterThan(0);
});

// @covers specs/apps/ayokoding/www/behaviors/backend/navigation/navigation-api.feature:Navigation tree structure matches the filesystem hierarchy
Then("each node should reflect its position in the directory hierarchy", async () => {
  const tree = backendState.treeResult as Record<string, unknown>[];
  expect(tree[0]).toHaveProperty("slug");
});

// @covers specs/apps/ayokoding/www/behaviors/backend/navigation/navigation-api.feature:Navigation nodes are ordered by weight ascending
Then('the children of "programming" should appear in order: weight 10, weight 20, weight 30', async () => {
  const tree = backendState.treeResult as { weight: number }[];
  for (let i = 1; i < tree.length; i++) {
    expect(tree[i]!.weight).toBeGreaterThanOrEqual(tree[i - 1]!.weight);
  }
});

Then('the "programming" node should have a non-empty "children" array', async () => {
  const tree = backendState.treeResult as { isSection: boolean; children: unknown[] }[];
  const sectionNode = tree.find((n) => n.isSection);
  expect(sectionNode).toBeDefined();
  expect(Array.isArray(sectionNode?.children)).toBe(true);
});

// @covers specs/apps/ayokoding/www/behaviors/backend/navigation/navigation-api.feature:Section nodes include a children array
Then('each child should include a "slug" and "title"', async () => {
  const tree = backendState.treeResult as { isSection: boolean; children: { slug: string }[] }[];
  const sectionNode = tree.find((n) => n.isSection && n.children.length > 0);
  expect(sectionNode).toBeDefined();
  expect(sectionNode!.children[0]).toHaveProperty("slug");
});
