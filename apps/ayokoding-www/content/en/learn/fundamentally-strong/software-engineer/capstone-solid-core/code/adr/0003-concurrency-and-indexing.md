...
`EXPLAIN QUERY PLAN` output for a "recent activity across all my habits" query showed `SCAN
checkins` (a full table scan) plus `USE TEMP B-TREE FOR ORDER BY` -- the normalized schema
forces a join from `checkins` to `habits` just to filter by `user_id`, then a sort. **Decision**:
denormalize `user_id` onto `checkins` (it never changes after a habit is created, so this
specific denormalization introduces no update-anomaly risk) and add a composite index
`(user_id, checkin_date DESC)` (`migration_v3.sql`). The SAME query re-read via
`EXPLAIN QUERY PLAN` afterward shows a single `SEARCH checkins USING INDEX
idx_checkins_user_id_date (user_id=?)` -- no scan, no join, no sort.
