...
from typing import (
    NamedTuple,
)  # => co-15: a typed instruction beats a bare tuple for readability


class Instruction(
    NamedTuple
):  # => co-15: one fetch-decode-execute cycle's worth of work
...
class RegisterMachine:  # => co-15: the fetch-decode-execute cycle, a memory array, and one accumulator register
    ...
    def __init__(
        self, memory_size: int = 8
    ) -> None:  # => co-15: fixed-size memory, all zero-initialized
        self.accumulator = 0  # => co-15: the ONE fast register this machine has -- all arithmetic goes through it
        self.memory: list[int] = [
            0
        ] * memory_size  # => co-15: RAM -- addressable by a plain integer index

    def execute(
        self, program: list[Instruction]
    ) -> int:  # => co-15: runs every instruction, in order, FETCH-DECODE-EXECUTE
        ...
        for instr in (
            program
        ):  # => co-15: FETCH -- pull the next instruction off the program list
            if instr.op == "LOAD":  # => co-15: DECODE -- dispatch on the opcode
                self.accumulator = (
                    instr.arg
                )  # => co-15: EXECUTE -- literal load into the accumulator register
            elif instr.op == "ADD":  # => co-15: DECODE -- the ALU's addition operation
                self.accumulator += (
                    instr.arg
                )  # => co-15: EXECUTE -- accumulator += literal, via the ALU
            elif (
                instr.op == "STORE"
            ):  # => co-15: DECODE -- write the register back out to memory
                self.memory[instr.arg] = (
                    self.accumulator
                )  # => co-15: EXECUTE -- memory[address] = accumulator
            else:  # => co-15: an unrecognized opcode is a machine fault, not silently ignored
                raise ValueError(
                    f"unknown opcode: {instr.op}"
                )  # => co-15: fails loudly instead of guessing
        return (
            self.accumulator
        )  # => co-15: the accumulator's final value after every instruction has run
...
if (
    ...
    machine = (
        RegisterMachine()
    )  # => co-15: a fresh machine, accumulator=0, memory all zeros
    final_accumulator = machine.execute(
        program
    )  # => co-15: runs the whole program, cycle by cycle
    print(f"final accumulator = {final_accumulator}")  # => co-15: expect 22
    print(
        f"memory[0] = {machine.memory[0]}"
    )  # => co-15: expect 22 -- the STORE instruction's effect
    assert final_accumulator == 22, (
        "accumulator must hold 10 + 5 + 7 = 22"
    )  # => co-15: the arithmetic result
    assert machine.memory[0] == 22, (
        "STORE must have written the accumulator's value to memory[0]"
    )  # => co-15
    print(
        f"Accumulator holds expected result: True"
    )  # => co-15: both asserts above passed
