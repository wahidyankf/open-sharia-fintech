...
Clause = tuple[
    int, int, int
]  # => co-25: a 3-SAT clause -- three literals (positive = var, negative = its negation)
Node = tuple[
    int, int
]  # => co-25: a clique-graph node -- (clause index, literal position within that clause)


def literals_compatible(
    lit_a: int, lit_b: int
) -> bool:  # => co-25: the reduction's edge rule
    ...
    return not (
        lit_a == -lit_b
    )  # => co-25: x and NOT x can never BOTH be True -- exactly the excluded pair


def reduce_3sat_to_clique(
    clauses: list[Clause],
) -> tuple[set[tuple[Node, Node]], dict[Node, int]]:  # => co-25
    ...
    for ci, clause in enumerate(
        clauses
    ):  # => co-25: one node per (clause index, position) pair
        for pos, literal in enumerate(
            clause
        ):  # => co-25: three positions per clause (3-SAT)
            nodes[(ci, pos)] = (
                literal  # => co-25: records which literal this graph node stands for
            )
    edges: set[tuple[Node, Node]] = (
        set()
    )  # => co-25: undirected edges, stored as (smaller, larger) tuples
    for (n1, lit1), (n2, lit2) in itertools.combinations(
        nodes.items(), 2
    ):  # => co-25: every distinct node pair
        if n1[0] != n2[0] and literals_compatible(
            lit1, lit2
        ):  # => co-25: DIFFERENT clauses AND not a negation pair
            edges.add(
                (n1, n2)
            )  # => co-25: an edge -- these two literals CAN be chosen together
    return edges, nodes  # => co-25: returns this computed value to the caller


def has_clique_of_size(
    edges: set[tuple[Node, Node]], nodes: list[Node], k: int
) -> bool:  # => co-25: brute-force clique check
    ...
    edge_lookup = edges | {
        (b, a) for a, b in edges
    }  # => co-25: a symmetric lookup set, both orderings present
    for subset in itertools.combinations(
        nodes, k
    ):  # => co-25: every possible k-node subset, exhaustive
        if all(
            (a, b) in edge_lookup for a, b in itertools.combinations(subset, 2)
        ):  # => co-25: EVERY pair connected
...
if (
    __name__ == "__main__"
):  # => co-25: entry point -- this block runs only when the file executes directly, not on import
    satisfiable_clauses: list[Clause] = [
        (1, 2, 3),
        (-1, 2, -3),
        (1, -2, 3),
    ]  # => co-25: assignment 1=T,2=T,3=T satisfies all
    unsatisfiable_clauses: list[Clause] = [
        (1, 1, 1),
        (-1, -1, -1),
    ]  # => co-25: x=T fails clause 2; x=F fails clause 1
    for label, clauses in [
        ("satisfiable", satisfiable_clauses),
        ("unsatisfiable", unsatisfiable_clauses),
    ]:  # => co-25
        edges, nodes = reduce_3sat_to_clique(
            clauses
        )  # => co-25: the reduction's graph, built mechanically
        clique_exists = has_clique_of_size(
            edges, list(nodes), k=len(clauses)
        )  # => co-25: does a size-k clique exist?
        ...
        direct_sat = any(  # => co-25: True iff SOME assignment satisfies every clause, checked directly
            all(  # => co-25: every clause satisfied under this candidate assignment
                ...
                )  # => co-25: at least one literal True
            ...
            for bits in itertools.product(
                [False, True], repeat=3
            )  # => co-25: all 8 assignments over 3 variables
        )  # => co-25: closes the multi-line construct opened above
        print(
            f"{label}: clique of size {len(clauses)} exists = {clique_exists}, direct SAT check = {direct_sat}"
        )  # => co-25: continues the statement started above
        assert clique_exists == direct_sat, (
            f"the reduction must preserve satisfiability for the {label} instance"
        )  # => co-25
    print(
        f"Clique existence matches direct satisfiability on both instances: True"
    )  # => co-25: both cases agreed
