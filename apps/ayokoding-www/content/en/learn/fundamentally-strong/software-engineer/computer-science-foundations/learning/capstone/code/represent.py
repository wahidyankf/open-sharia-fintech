...
from __future__ import (
    annotations,
)  # => DD-39 hygiene: postpones type-annotation evaluation, keeping this file interpreter-version-agnostic

import struct  # => co-03: struct.pack/unpack -- the bridge between a Python float and its raw IEEE-754 bytes
from typing import (
    NamedTuple,
)  # => co-03: typing import supporting the typed structures below


def int_to_binary(
    n: int,
) -> str:  # => co-01: positional-system conversion, unsigned magnitude only
    ...
    if (
        n < 0
    ):  # => co-01: this function's contract is unsigned -- negative ints need to_twos_complement below
        raise ValueError(
            "int_to_binary expects a non-negative int -- use to_twos_complement for negatives"
        )  # => co-01: continues the statement started above
    return (
        bin(n)[2:] if n else "0"
    )  # => co-01: bin() does the division-by-2 conversion; strip its "0b" prefix


def int_to_hex(
    n: int,
) -> str:  # => co-01: the SAME value, rendered in base 16 instead of base 2
    ...
    return (
        hex(n)[2:] if n else "0"
    )  # => co-01: hex() does the division-by-16 conversion; strip its "0x" prefix


def to_twos_complement(
    n: int, bits: int = 8
) -> str:  # => co-02: signed int -> fixed-width two's-complement bits
    ...
    mask = (
        (1 << bits) - 1
    )  # => co-02: keeps only the low `bits` bits -- Python's arbitrary-precision AND does the encoding
    return format(
        n & mask, f"0{bits}b"
    )  # => co-02: zero-padded binary string, exactly `bits` characters wide


class Ieee754Fields(
    NamedTuple
):  # => co-03: the three fields IEEE 754-2019 binary64 packs into 64 bits
...
def inspect_float(
    x: float,
) -> Ieee754Fields:  # => co-03: decomposes a float into its raw IEEE-754 bit fields
    ...
    bits = int.from_bytes(
        struct.pack(">d", x), byteorder="big"
    )  # => co-03: 8 big-endian bytes -> one 64-bit int
...
def float_bits_string(
    x: float,
) -> str:  # => co-03: the full 64-bit pattern, grouped for readability
...
def demonstrate_rounding_error() -> tuple[
    float, bool
]:  # => co-03: the structural (not buggy) 0.1+0.2 fact
    ...
    total = (
        0.1 + 0.2
    )  # => co-03: neither 0.1 nor 0.2 has an exact binary64 representation -- both already round
    return (
        total,
        total == 0.3,
    )  # => co-03: the sum and whether it happens to equal the (also-rounded) literal 0.3


if (
    __name__ == "__main__"
):  # => co-03: entry point -- this block runs only when the file executes directly, not on import
    ...
    print(
        f"{value} -> binary {int_to_binary(value)}, hex {int_to_hex(value)}"
    )  # => co-01: both conversions shown
    assert int_to_binary(value) == "10011100", (
        "must match the known bit pattern for 156"
    )  # => co-01
    assert int(int_to_binary(value), 2) == value == int(int_to_hex(value), 16), (
        "round-trip must recover 156"
    )  # => co-01

    print("\n=== two's complement ===")  # => co-02: section 2 of the toolkit demo
    negative = -42  # => co-02: the same fixed test value Example 3 used
    print(
        f"{negative} in 8-bit two's complement = {to_twos_complement(negative)}"
    )  # => co-02
    assert to_twos_complement(negative) == "11010110", (
        "must match the known 8-bit pattern for -42"
    )  # => co-02

    print(
        "\n=== IEEE-754 float inspection ==="
    )  # => co-03: section 3 of the toolkit demo
    known_value = 1.0  # => co-03: a float with a clean, easy-to-verify encoding
    print(
        f"float_bits(1.0)  = {float_bits_string(known_value)}"
    )  # => co-03: sign|exponent|mantissa, grouped
    fields = inspect_float(
        known_value
    )  # => co-03: decode 1.0's fields directly, for the exact-match assertions
    assert fields == Ieee754Fields(sign=0, exponent=1023, mantissa=0), (
        "1.0 must decode to sign=0 exp=1023 mant=0"
    )  # => co-03

    print(
        "\n=== 0.1 + 0.2 != 0.3 ==="
    )  # => co-03: section 4 -- the structural rounding-error demonstration
    total, equals_point_three = (
        demonstrate_rounding_error()
    )  # => co-03: the actual computed sum and its comparison
    print(
        f"0.1 + 0.2 = {total!r}"
    )  # => co-03: prints the EXACT stored value, not a rounded display
    print(
        f"0.1 + 0.2 == 0.3: {equals_point_three}"
    )  # => co-03: the headline claim -- expect False
    assert repr(total) == "0.30000000000000004", (
        "must print the syllabus's documented exact value"
    )  # => co-03
    assert equals_point_three is False, (
        "0.1 + 0.2 must NOT equal the literal 0.3"
    )  # => co-03: the structural fact

    print(
        "\nAll representation and IEEE-754 claims verified: True"
    )  # => co-01, co-02, co-03: every assert above passed
