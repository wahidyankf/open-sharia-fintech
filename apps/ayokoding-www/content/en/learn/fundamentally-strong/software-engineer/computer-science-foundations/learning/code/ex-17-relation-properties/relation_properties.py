...
def is_reflexive(
    domain: set[int], relation: set[tuple[int, int]]
) -> bool:  # => co-09: every x relates to itself
    ...
    return all(
        (x, x) in relation for x in domain
    )  # => co-09: checked for EVERY domain element, not a sample


def is_symmetric(
    relation: set[tuple[int, int]],
) -> bool:  # => co-09: (x,y) in R implies (y,x) in R
    ...
    return all(
        (b, a) in relation for a, b in relation
    )  # => co-09: checked for every existing pair


def is_transitive(
    relation: set[tuple[int, int]],
) -> bool:  # => co-09: (x,y) and (y,z) in R implies (x,z) in R
    ...
    for a, b in relation:  # => co-09: for every pair sharing a "middle" element...
        for c, d in relation:  # => co-09: ...paired against every other relation entry
            if (
                b == c and (a, d) not in relation
            ):  # => co-09: chain a->b->d found, but a->d missing
...
if (
    __name__ == "__main__"
):  # => co-09: entry point -- this block runs only when the file executes directly, not on import
    domain: set[int] = {
        1,
        2,
        3,
    }  # => co-09: the small finite domain this relation is defined over
    # "divides" restricted to {1,2,3}: 1|1, 1|2, 1|3, 2|2, 3|3 -- a KNOWN reflexive, transitive, non-symmetric case
    divides: set[tuple[int, int]] = {
        (1, 1),
        (1, 2),
        (1, 3),
        (2, 2),
        (3, 3),
    }  # => co-09: "a divides b" pairs
    reflexive = is_reflexive(
        domain, divides
    )  # => co-09: expect True -- (1,1),(2,2),(3,3) all present
    symmetric = is_symmetric(
        divides
    )  # => co-09: expect False -- (1,2) present but (2,1) is not
    transitive = is_transitive(
        divides
    )  # => co-09: expect True -- "divides" is always transitive
    print(f"relation = {sorted(divides)}")  # => co-09: prints the relation under test
    print(
        f"reflexive={reflexive} symmetric={symmetric} transitive={transitive}"
    )  # => co-09: the three flags
    assert reflexive is True, "'divides' on {1,2,3} must be reflexive"  # => co-09
    assert symmetric is False, (
        "'divides' on {1,2,3} must NOT be symmetric (1|2 but not 2|1)"
    )  # => co-09
    ...
    )  # => co-09: all three asserts passed
