...
def parse_balanced(
    s: str, pos: int = 0
) -> int | None:  # => co-20: recursive-descent parser for the CFG above
    ...
    while (
        pos < len(s) and s[pos] == "("
    ):  # => co-20: S -> "(" S ")" S -- consume opens, recursing per production
        inner_end = parse_balanced(
            s, pos + 1
        )  # => co-20: recursively parse the matching S inside this "("
        if (
            inner_end is None or inner_end >= len(s) or s[inner_end] != ")"
        ):  # => co-20: the closing ")" MUST be there
            return None  # => co-20: malformed -- no matching close, or ran out of input
        pos = (
            inner_end + 1
        )  # => co-20: past the ")" -- continue parsing the OUTER S's own trailing S
    return pos  # => co-20: S -> ε -- nothing left to consume at this nesting level, return where parsing stopped


def is_balanced(
    s: str,
) -> bool:  # => co-20: accepted iff the WHOLE string is consumed by one S derivation
    ...
    return end is not None and end == len(
        s
    )  # => co-20: must consume EVERY character, not just a prefix
...
if (
    ...
    for (
        ...
        test_cases.items()
    ):  # => co-20: run every case through the recursive-descent parser
        actual = is_balanced(s)  # => co-20: the CFG-derived parser's own verdict
        print(
            f"{s!r:<8} balanced={actual} expected={expected}"
        )  # => co-20: per-case report
        assert actual == expected, (
            f"balanced-parens verdict for {s!r} must be {expected}"
        )  # => co-20
    print(
        f"All balanced and unbalanced strings classified correctly: True"
