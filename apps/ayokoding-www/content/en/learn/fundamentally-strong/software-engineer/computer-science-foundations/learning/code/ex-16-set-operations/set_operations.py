...
if (
    ...
    a: set[int] = {
        ...
        4,
        5,
    }  # => co-09: set A -- a finite, unordered, duplicate-free collection
    ...
    print(
        f"A = {sorted(a)}"
    )  # => co-09: sorted() only for stable, readable printing -- sets are unordered
    print(f"B = {sorted(b)}")  # => co-09: same printing convention for B
    print(f"A | B (union) = {sorted(union)}")  # => co-09: prints the computed union
    print(
        f"A & B (intersection) = {sorted(intersection)}"
    )  # => co-09: prints the computed intersection
    print(
        f"A - B (difference) = {sorted(difference)}"
    )  # => co-09: prints the computed difference
    assert union == {1, 2, 3, 4, 5, 6, 7}, (
        "union must contain every element from either set"
    )  # => co-09
    assert intersection == {4, 5}, (
        "intersection must contain only the shared elements"
    )  # => co-09
    assert difference == {1, 2, 3}, (
        "difference must contain only A's elements not also in B"
    )  # => co-09
    print(
