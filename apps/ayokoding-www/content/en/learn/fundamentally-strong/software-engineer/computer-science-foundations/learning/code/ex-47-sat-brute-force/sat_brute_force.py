...
Clause = tuple[
    int, int, int
]  # => co-25: a 3-SAT clause -- three literals; a positive int is a var, negative is its negation


def clause_satisfied(
    clause: Clause, assignment: dict[int, bool]
) -> bool:  # => co-25: True iff >=1 literal is True
    ...
    for literal in (
        clause
    ):  # => co-25: a clause is an OR of its literals -- one True literal is enough
        var = abs(literal)  # => co-25: the underlying variable this literal refers to
        value = assignment[
            var
        ]  # => co-25: this assignment's truth value for that variable
        if (literal > 0 and value) or (
            literal < 0 and not value
        ):  # => co-25: positive lit true, or negated lit true
...
def brute_force_sat(
    clauses: list[Clause], num_vars: int
) -> dict[int, bool] | None:  # => co-25: tries EVERY assignment
    ...
    for bits in itertools.product(
        [False, True], repeat=num_vars
    ):  # => co-25: exactly 2**num_vars candidates, exhaustive
        assignment = {
            i + 1: bits[i] for i in range(num_vars)
        }  # => co-25: variables are numbered 1..num_vars
        if all(
            clause_satisfied(c, assignment) for c in clauses
        ):  # => co-25: EVERY clause must be satisfied (AND of ORs)
...
def build_satisfiable_instance(
    num_vars: int,
) -> list[Clause]:  # => co-25: a small family that stays satisfiable as n grows
    ...
    return [
        (i, i + 1 if i + 1 <= num_vars else 1, i + 2 if i + 2 <= num_vars else 1)
        for i in range(1, num_vars + 1)
    ]  # => co-25: returns this computed value to the caller
...
if (
    ...
    sizes = [
        ...
        20,
        24,
    ]  # => co-25: each size adds 4 more variables -- DOUBLES the search space every +1
    ...
    for n in sizes:  # => co-25: time the FULL brute-force search at each variable count
        clauses = build_satisfiable_instance(
            n
        )  # => co-25: a satisfiable instance -- forces the FULL exponential search
        start = time.perf_counter()  # => co-25: begin timing window
        brute_force_sat(
            clauses, n
        )  # => co-25: the timed operation -- 2**n candidate assignments checked
        ...
        print(
            f"n={n:>2} vars  2**n={2**n:>10,}  time={elapsed:.4f}s"
        )  # => co-25: size, search-space size, and duration
    ratios = [
        timings[i + 1] / timings[i] for i in range(len(timings) - 1)
    ]  # => co-25: growth factor per +4 variables
    print(
        f"time ratios per +4 variables: {[round(r, 1) for r in ratios]}"
    )  # => co-25: expect roughly 2**4 = 16x each step
    ...
    assert all(r > 4 for r in ratios), (
        "runtime must grow exponentially (roughly ~2**n), not polynomially"
    )  # => co-25
    print(
        f"Runtime grows exponentially with variable count: True"
    )  # => co-25: reached only if the assert passed
