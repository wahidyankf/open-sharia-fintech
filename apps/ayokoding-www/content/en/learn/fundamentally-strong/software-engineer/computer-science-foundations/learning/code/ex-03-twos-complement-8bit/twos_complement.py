...
def to_twos_complement(
    n: int, bits: int = BITS
) -> str:  # => co-02: negative -> 8-bit two's-complement bits
    ...
    mask = (
        1 << bits
    ) - 1  # => co-02: 0xFF for bits=8 -- keeps only the low `bits` bits
    encoded = (
        n & mask
    )  # => co-02: Python ints are arbitrary-precision, so AND-with-mask IS the
    # => two's-complement encoding -- invert-and-add-1 is what `&` with a negative int does internally
    return format(
        encoded, f"0{bits}b"
    )  # => co-02: zero-padded binary string, e.g. "11010110"
...
if (
    ...
    assert bits == "11010110", (
        "must match the syllabus's documented pattern"
    )  # => co-02: exact-match check
    identity = (
        value & 0xFF
    ) + 42  # => co-02: the defining property -- complement + magnitude == 2**bits
    print(f"(-42 & 0xFF) + 42 = {identity}")  # => co-02: prints the identity check
    assert identity == 256, (
        "two's-complement identity must hold: complement + |n| == 2**bits"
    )  # => co-02
    print(
        f"Pattern matches, identity holds: True"
    )  # => co-02: reached only if both asserts passed
