...
def recurse_forever(
    depth: int = 0,
) -> int:  # => co-17: no base case -- deliberately unbounded, to HIT the limit
    ...
    return 1 + recurse_forever(
        depth + 1
    )  # => co-17: each call pushes ANOTHER frame -- the stack can't grow forever
...
if (
    ...
    )  # => co-17: this interpreter's configured maximum call-stack depth
    print(
        f"sys.getrecursionlimit() = {limit}"
    )  # => co-17: the ceiling this run is expected to hit
    ...
    try:  # => co-17: without a base case, this call chain WILL exceed the limit -- expected, not a bug
        ...
        print(
            f"caught RecursionError near the configured limit of {limit}: {exc}"
        )  # => co-17: fired, not ignored
    assert raised, (
        "RecursionError must have been raised and caught, near sys.getrecursionlimit()"
    )  # => co-17
    print(
        f"RecursionError raised near sys.getrecursionlimit(): True"
    )  # => co-17: reached only if the assert passed
