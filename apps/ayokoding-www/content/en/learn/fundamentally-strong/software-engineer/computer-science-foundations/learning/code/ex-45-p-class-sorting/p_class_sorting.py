...
def measure_sort_time(
    n: int, seed: int = 0
) -> float:  # => co-24: times Python's own Timsort on a random list
    ...
    rng = random.Random(
        seed
    )  # => co-24: a seeded RNG -- deterministic input across repeated runs
    data = [
        rng.random() for _ in range(n)
    ]  # => co-24: n random floats, freshly generated for this size
    ...
    )  # => co-24: elapsed wall-clock seconds for JUST the sort call
...
if (
    ...
    sizes = [
        ...
        40_000,
        80_000,
    ]  # => co-24: each size DOUBLES the previous one
    times = [
        measure_sort_time(n) for n in sizes
    ]  # => co-24: one measured duration per size
    for n, t in zip(
        sizes, times
    ):  # => co-24: prints size alongside its measured sort time
        theoretical = n * math.log2(
            n
        )  # => co-24: the O(n log n) shape this measurement is expected to track
        print(
            f"n={n:>7}  time={t:.4f}s  n*log2(n)={theoretical:,.0f}"
        )  # => co-24: measured vs. theoretical shape
    ratios = [
        times[i + 1] / times[i] for i in range(len(times) - 1)
    ]  # => co-24: measured time growth per doubling
    print(
        f"time ratios between successive doublings: {[round(r, 2) for r in ratios]}"
    )  # => co-24: e.g. ~2.0-2.3x
    ...
    assert all(r < 8 for r in ratios), (
        "O(n log n) scaling must stay well under the exponential/factorial blowups Examples 47 and 49 measure"
    )  # => co-24
    print(
        f"Sorting scales like O(n log n), not exponentially: True"
    )  # => co-24: reached only if the assert passed
