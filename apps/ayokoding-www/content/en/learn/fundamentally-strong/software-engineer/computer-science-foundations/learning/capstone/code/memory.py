...
def build_matrix(
    ...
    flat = array.array("d", [0.0]) * (
        n * n
    )  # => co-16: n*n contiguous 8-byte slots, zero-initialized
    for k in range(
        n * n
...
def row_major_sum(
    ...
    for i in range(n):  # => co-16: outer loop over rows
        ...
        for j in range(
            ...
            total += flat[
                base + j
            ]  # => co-16: stride-1 access -- exactly how a cache-line prefetcher likes it
...
if (
    ...
    for trial in range(
        ...
        print(
            f"trial {trial}: row={row_times[-1]:.4f}s col={col_times[-1]:.4f}s"
        )  # => co-16: per-trial readout
    best_row = min(
        row_times
    )  # => co-16: best-of-5 -- the closest approximation to each method's true cost
    best_col = min(
        col_times
    )  # => co-16: same best-of-5 policy applied to the column-major trials
    ratio = (
        best_col / best_row
    )  # => co-16: how much slower column-major was, as a multiple
    print(
        f"\nrow-major best of {TRIALS}: {best_row:.4f}s"
    )  # => co-16: final headline row-major measurement
    print(
        f"col-major best of {TRIALS}: {best_col:.4f}s"
    )  # => co-16: final headline column-major measurement
    print(
    ...
    )  # => co-16: reached only if the timing assert held
