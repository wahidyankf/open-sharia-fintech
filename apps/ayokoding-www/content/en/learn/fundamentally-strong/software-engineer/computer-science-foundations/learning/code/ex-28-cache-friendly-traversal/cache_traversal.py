...
def build_matrix(
    ...
    float
]:  # => co-16: one FLAT contiguous buffer -- row i lives at [i*n : i*n+n]
    ...
    flat = array.array("d", [0.0]) * (
        n * n
    )  # => co-16: n*n contiguous 8-byte slots, all zero-initialized
    for k in range(
        n * n
    ):  # => co-16: fill with distinguishable, cheap-to-compute values
        flat[k] = float(
            k % 97
        )  # => co-16: content is irrelevant to the timing -- only the ACCESS PATTERN matters
    return flat  # => co-16: returns this computed value to the caller


def row_major_sum(
    flat: array.array[float], n: int
) -> float:  # => co-16: walks memory in STORAGE order -- stride 1
    ...
    for i in range(n):  # => co-16: outer loop over rows
        base = (
            i * n
        )  # => co-16: row i's starting flat index -- computed once per row, not once per element
        for j in range(
            n
        ):  # => co-16: inner loop walks CONSECUTIVE flat indices -- sequential memory access
            total += flat[
                base + j
            ]  # => co-16: stride-1 access -- exactly how the CPU's cache line prefetcher likes it
    return total  # => co-16: returns this computed value to the caller


def col_major_sum(
    flat: array.array[float], n: int
) -> float:  # => co-16: walks memory AGAINST storage order -- stride n
    ...
    for j in range(n):  # => co-16: outer loop over columns
        for i in range(
            n
        ):  # => co-16: inner loop jumps n flat-indices apart on every single step
            total += flat[
                i * n + j
            ]  # => co-16: stride-n access -- each step likely lands in a DIFFERENT cache line
    return total  # => co-16: returns this computed value to the caller


if (
    __name__ == "__main__"
):  # => co-16: entry point -- this block runs only when the file executes directly, not on import
    matrix = build_matrix(
        N
    )  # => co-16: one shared buffer -- both traversal orders read the SAME data
    row_times: list[float] = []  # => co-16: one measured duration per trial, row-major
    col_times: list[
        float
    ] = []  # => co-16: one measured duration per trial, column-major
    for _ in range(
        TRIALS
    ):  # => co-16: repeat both traversals, keeping the BEST (least-noisy) time of each
        ...
        t1 = (
            time.perf_counter()
        )  # => co-16: end of the row-major window, start of the column-major window
        ...
        assert row_result == col_result, (
            "both traversal orders must sum to the identical total"
        )  # => co-16
    best_row = min(
        row_times
    )  # => co-16: best-of-3 -- the closest approximation to each method's true cost
    best_col = min(
        col_times
    )  # => co-16: same best-of-3 policy applied to the column-major trials
    print(
        f"row-major best of {TRIALS}: {best_row:.4f}s"
    )  # => co-16: prints the row-major measurement
    print(
        f"col-major best of {TRIALS}: {best_col:.4f}s"
    )  # => co-16: prints the column-major measurement
    print(
        f"col-major / row-major ratio: {best_col / best_row:.2f}x"
    )  # => co-16: how much slower column-major was
    assert best_row < best_col, (
        "row-major (sequential access) must be measurably faster than column-major"
    )  # => co-16
    print(
        f"Row-major is measurably faster: True"
    )  # => co-16: reached only if the timing assert above held
