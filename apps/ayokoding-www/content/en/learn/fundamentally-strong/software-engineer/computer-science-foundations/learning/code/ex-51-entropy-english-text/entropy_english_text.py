...
from collections import (
    Counter,
)  # => co-26: tallies each character's observed frequency across the sample
...
def zero_order_entropy(
    text: str,
) -> float:  # => co-26: "zero-order" -- treats each character as independent
    ...
    n = len(
        text
    )  # => co-26: total character count -- the denominator for every probability estimate
    probabilities = (
        count / n for count in counts.values()
    )  # => co-26: empirical P(char) for every distinct symbol
    return -sum(
        p * math.log2(p) for p in probabilities
    )  # => co-26: Shannon's formula, summed over the alphabet


if (
    __name__ == "__main__"
):  # => co-26: entry point -- this block runs only when the file executes directly, not on import
    entropy = zero_order_entropy(
        SAMPLE_TEXT
    )  # => co-26: this sample's estimated per-character entropy
    distinct_chars = len(
        set(SAMPLE_TEXT)
    )  # => co-26: alphabet size actually observed in this sample
    uniform_entropy = math.log2(
        distinct_chars
    )  # => co-26: the MAXIMUM possible entropy for this alphabet size
    print(
        f"sample length: {len(SAMPLE_TEXT)} characters, {distinct_chars} distinct symbols"
    )  # => co-26
    print(
        f"estimated entropy: {entropy:.4f} bits/char"
    )  # => co-26: the headline measurement
    print(
        f"uniform-distribution ceiling: log2({distinct_chars}) = {uniform_entropy:.4f} bits/char"
    )  # => co-26
    assert entropy < uniform_entropy, (
        "real text must have LESS entropy than a uniform distribution over the same alphabet"
    )  # => co-26
    assert 3.5 < entropy < 4.5, (
        "ordinary English text's zero-order entropy lands near 4 bits/char"
    )  # => co-26: syllabus's claim
    print(
        f"Entropy lands near 4 bits/char: True"
    )  # => co-26: reached only if both asserts above passed
