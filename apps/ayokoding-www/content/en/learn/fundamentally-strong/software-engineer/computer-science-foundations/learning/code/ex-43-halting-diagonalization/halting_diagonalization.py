...
def d_behavior_given_oracle_answer(
    oracle_says_p_halts_on_itself: bool,
) -> str:  # => co-23: D's OWN definition
    ...
    if (
        oracle_says_p_halts_on_itself
    ):  # => co-23: D's definition: "if H(P, P) says halts..."
        return "LOOPS_FOREVER"  # => co-23: "...then loop forever" -- D deliberately defies the oracle's answer
    return (
        "HALTS"  # => co-23: "...otherwise, halt" -- D again defies the oracle's answer
    )
...
if (
    ...
    actual_behavior_if_true = d_behavior_given_oracle_answer(
        oracle_claims_true
    )  # => co-23: D's OWN definition
    contradiction_1 = (
        actual_behavior_if_true == "LOOPS_FOREVER"
    )  # => co-23: oracle said "halts", D actually loops
    print(
        f"oracle claims D(D) halts -> D's own definition makes it: {actual_behavior_if_true}"
    )  # => co-23

    oracle_claims_false = False  # => co-23: CASE 2 -- suppose the oracle instead claims "D(D) loops forever"
    actual_behavior_if_false = d_behavior_given_oracle_answer(
        oracle_claims_false
    )  # => co-23: D's OWN definition
    contradiction_2 = (
        actual_behavior_if_false == "HALTS"
    )  # => co-23: oracle said "loops", D actually halts
    print(
        f"oracle claims D(D) loops -> D's own definition makes it: {actual_behavior_if_false}"
    )  # => co-23

    assert contradiction_1, (
        "if the oracle claims 'halts', D's own definition must make it loop"
    )  # => co-23
    assert contradiction_2, (
        "if the oracle claims 'loops', D's own definition must make it halt"
    )  # => co-23
    print(
    ...
    )  # => co-23: continues the statement started above
