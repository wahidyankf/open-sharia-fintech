...
TRANSITIONS: dict[
    tuple[str, str], str
] = {  # => co-18: δ -- the full transition table for this DFA
    ("S0", "0"): "S1",
    (
        "S0",
        "1",
    ): "S0",  # => co-18: from S0: a "0" moves toward acceptance, a "1" stays put
    ("S1", "0"): "S1",
    (
        "S1",
        "1",
    ): "S2",  # => co-18: from S1: another "0" stays (still "just saw a 0"), "1" completes "01"
    ("S2", "0"): "S1",
    (
        "S2",
        "1",
    ): "S0",  # => co-18: from S2: a NEW "0" restarts the watch, a "1" breaks the ending
}  # => co-18: closes the multi-line construct opened above
ACCEPT = {
    "S2"
}  # => co-18: F -- accepting iff the walk currently ends in S2 ("...01" just seen)
REGEX = re.compile(
    r".*01$"
)  # => co-19: the SAME language, expressed as a regex -- Kleene's theorem says these must agree


@dataclass(
    frozen=True
)  # => co-18: a DFA is DATA -- reusable for any (states, alphabet, δ, start, accept) tuple
class Dfa:  # => co-18: continues the statement started above
    ...
    def run(
        self, s: str
    ) -> bool:  # => co-18: feed s through δ symbol by symbol, from q0
        state = self.start  # => co-18: every run begins at q0, unconditionally
        for symbol in (
            s
        ):  # => co-18: one transition per symbol -- exactly one next state each step
...
ENDS_IN_01 = Dfa(
    transitions=TRANSITIONS, start="S0", accept=ACCEPT
)  # => co-18: the machine this step exercises
...
if (
    ...
    test_cases = [
        ...
        "0110",
        "111101",
    ]  # => co-18: a spread of strings
    for s in (
        test_cases
    ):  # => co-18, co-19: run every string through BOTH the DFA and the regex
        dfa_verdict = ENDS_IN_01.run(
            s
        )  # => co-18: the hand-built machine's own accept/reject verdict
        regex_verdict = (
            REGEX.fullmatch(s) is not None
        )  # => co-19: Kleene's theorem's independent cross-check
        ends_with_01 = s.endswith(
            "01"
        )  # => co-18: a THIRD, brute-force sanity check against plain string logic
        print(
            f"{s!r:<8} dfa={dfa_verdict} regex={regex_verdict} ends_with_01={ends_with_01}"
        )  # => co-18, co-19
        assert dfa_verdict == regex_verdict == ends_with_01, (
            f"all three checks must agree for {s!r}"
        )  # => co-18, co-19
    print(
        "\nHand trace for '001': S0 -0-> S1 -0-> S1 -1-> S2 (accepting)"
    )  # => co-18: documents the trace by hand
    assert ENDS_IN_01.run("001") is True, (
        "the hand-traced walk for '001' must land in the accepting state S2"
    )  # => co-18
    print(
