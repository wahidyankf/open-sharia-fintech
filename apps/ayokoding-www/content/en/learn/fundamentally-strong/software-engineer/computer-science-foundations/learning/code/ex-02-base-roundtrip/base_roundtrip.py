...
from typing import (
    NamedTuple,
)  # => co-01: a typed record beats a bare tuple for the per-base report
...
def round_trip(
    value: int,
) -> list[BaseView]:  # => co-01: builds one BaseView per base, in a fixed order
    ...
    views: list[
        BaseView
    ] = []  # => co-01: accumulates the three rows this function returns
    for (
        name,
        literal,
    ) in (  # => co-01: (label, Python's own builtin literal, prefix included)
        ...
        base = {"binary": 2, "octal": 8, "hex": 16}[
            name
        ]  # => co-01: numeric base matching this literal
        round_tripped = int(
            literal, base
        )  # => co-01: int() parses the FULL literal, prefix included
        views.append(
            BaseView(name, literal, round_tripped)
        )  # => co-01: one completed row
    return views  # => co-01: returns this computed value to the caller


if (
    __name__ == "__main__"
):  # => co-01: entry point -- this block runs only when the file executes directly, not on import
    value = 156  # => co-01: same test value as Example 1, for continuity
    for view in round_trip(value):  # => co-01: one printed line per base
        print(
            f"{view.base_name:<7} {view.literal:<12} -> int(..) = {view.round_tripped}"
        )  # => co-01
        assert view.round_tripped == value, (
            f"{view.base_name} round-trip must equal {value}"
        )  # => co-01
    print(
        f"All three bases round-trip to {value}: True"
    )  # => co-01: reached only if every assert passed
