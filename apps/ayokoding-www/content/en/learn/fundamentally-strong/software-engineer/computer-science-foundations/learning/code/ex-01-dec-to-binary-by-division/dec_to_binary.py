...
def to_binary(
    n: int,
) -> str:  # => co-01: positional-system conversion via repeated division
    ...
    if n == 0:  # => co-01: the one base case repeated division never reaches on its own
        return (
            "0"  # => co-01: 0 in any base is just "0" -- short-circuit before the loop
        )
    remainders: list[
        int
    ] = []  # => co-01: collects bits LEAST-significant-first, one per division
    working = (
        n  # => co-01: a local copy -- the loop mutates this, never the caller's `n`
    )
    while working > 0:  # => co-01: stop the instant the quotient reaches 0
        remainders.append(
            working % 2
        )  # => co-01: the next bit is this step's remainder (0 or 1)
        working //= 2  # => co-01: integer-divide by the base (2) -- the "repeated division" step
    return "".join(
        str(bit) for bit in reversed(remainders)
    )  # => co-01: reverse -- bits came out LSB-first
...
if (
    ...
    expected = bin(n)[
        2:
    ]  # => co-01: Python's own built-in conversion, stripped of its "0b" prefix
    print(f"to_binary({n}) = {result}")  # => co-01: prints the hand-rolled result
    print(
        f"bin({n})       = 0b{expected}"
    )  # => co-01: prints Python's own conversion for comparison
    assert result == expected, (
        "hand-rolled conversion must match bin()"
    )  # => co-01: the two must agree
    print(
        f"MATCH: {result == expected}"
    )  # => co-01: confirms agreement -- expect "10011100"
