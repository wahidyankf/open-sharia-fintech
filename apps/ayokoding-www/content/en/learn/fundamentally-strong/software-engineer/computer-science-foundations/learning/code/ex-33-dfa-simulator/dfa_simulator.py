...
from dataclasses import (
    dataclass,
)  # => co-18: a typed, reusable DFA definition -- states, alphabet, delta


@dataclass(
    frozen=True
)  # => co-18: a DFA is DATA -- five components, per the formal (Q, Σ, δ, q0, F) definition
class Dfa:  # => co-18: this SAME class drives any DFA supplied to it -- the "generic simulator" this example is
    ...
    def run(
        ...
        for symbol in s:  # => co-18: one transition per symbol -- a DFA has exactly one next state per step
            ...
            ):  # => co-18: outside Σ is undefined for THIS machine
                raise ValueError(
                    f"{symbol!r} not in alphabet {sorted(self.alphabet)}"
                )  # => co-18: fail loudly
            state = self.transitions[
                (state, symbol)
            ]  # => co-18: δ(state, symbol) -- the ONE next state
...
if (
    ...
    ends_in_one = Dfa(  # => co-18: proves the simulator is generic by running a machine Example 32 never defined
        states=frozenset(
            {"S0", "S1"}
        ),  # => co-18: S0 = "last symbol was 0 or start", S1 = "last symbol was 1"
        alphabet=frozenset({"0", "1"}),  # => co-18: Σ
        transitions={  # => co-18: δ -- the full transition table for this machine
            ("S0", "0"): "S0",
            ("S0", "1"): "S1",  # => co-18: from S0: "0" stays, "1" moves to S1
            ("S1", "0"): "S0",
            ("S1", "1"): "S1",  # => co-18: from S1: "0" moves to S0, "1" stays
        },  # => co-18: closes the multi-line construct opened above
        start="S0",  # => co-18: q0
        accept=frozenset(
            {"S1"}
        ),  # => co-18: F -- accept iff the string's last symbol was "1"
    )  # => co-18: closes the multi-line construct opened above
    test_cases = {
        "1": True,
        "0": False,
        ...
        "110": False,
        "": False,
    }  # => co-18: hand-traced expectations
    for (
        s,
        expected,
    ) in test_cases.items():  # => co-18: run every case through the generic simulator
        actual = ends_in_one.run(
            s
        )  # => co-18: the SAME Dfa.run() method Example 32's machine would also use
        print(
            f"{s!r:<5} accepted={actual} expected={expected}"
        )  # => co-18: per-case report
        assert actual == expected, (
            f"verdict for {s!r} must match hand-traced expectation"
        )  # => co-18
    print(
        f"Generic simulator correctly runs a supplied machine: True"
