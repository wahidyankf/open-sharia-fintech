...
def in_anbn(
    ...
    bool
):  # => co-20, co-21: independent membership check -- exactly equal runs of a then b
    ...
    while i < len(s) and s[i] == "b":  # => co-21: consume the b-run
        ...
        i += 1  # => co-21: updates the running total/counter in place
    return (
        i == len(s) and a_count == b_count
    )  # => co-21: whole string consumed AND counts equal


def pump(
    s: str, x_len: int, y_len: int, k: int
) -> str:  # => co-21: x.(y^k).z, per the pumping lemma's construction
    ...
    x, y, z = (
        ...
        s[x_len : x_len + y_len],
        s[x_len + y_len :],
    )  # => co-21: the mandated 3-way split
...
if (
    ...
    s = (
        ("a" * p) + ("b" * p)
    )  # => co-21: a string of length 2p, definitely long enough to force |xy| <= p into the a-run
    print(
        f"candidate pump length p={p}, witness string s={s!r} (in a^n b^n: {in_anbn(s)})"
    )  # => co-21
    assert in_anbn(s), (
        "the witness string itself must be a valid a^n b^n member"
    )  # => co-21: sanity check
    ...
    x_len, y_len = (
        0,
        1,
    )  # => co-21: a valid split satisfying |xy| = 1 <= p=4 and |y| = 1 >= 1, per the lemma's constraints
    pumped_up = pump(
        s, x_len, y_len, k=2
    )  # => co-21: "pump up" -- repeat y twice instead of once
    pumped_down = pump(
        s, x_len, y_len, k=0
    )  # => co-21: "pump down" -- remove y entirely (k=0)
    print(
        f"pumped up (k=2)   = {pumped_up!r} (in a^n b^n: {in_anbn(pumped_up)})"
    )  # => co-21
    print(
        f"pumped down (k=0) = {pumped_down!r} (in a^n b^n: {in_anbn(pumped_down)})"
    )  # => co-21
    assert not in_anbn(pumped_up), (
        "pumping y up must break membership -- extra a's, same b's"
    )  # => co-21
    assert not in_anbn(pumped_down), (
        "pumping y down must break membership -- fewer a's, same b's"
    )  # => co-21
    print(
        f"Every valid split's pumped string breaks a^n b^n membership: True"
    )  # => co-21: the counterexample holds
