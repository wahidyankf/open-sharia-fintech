...
def server(ready: threading.Event) -> None:  # => runs on a background thread (co-10)
    with socket.socket(
    ...
    ) as sock:  # => AF_INET+SOCK_STREAM = TCP
        sock.setsockopt(
            socket.SOL_SOCKET, socket.SO_REUSEADDR, 1
        )  # => allow instant re-bind
        sock.bind(
            (HOST, PORT)
        )  # => bind: CLAIMS this (address, port) pair for this process
        # => after bind, the socket has an address but is not yet accepting connections
        sock.listen(
            1
        )  # => listen: flips the socket into PASSIVE mode, backlog of 1 pending conn
        ...
        conn, addr = (
            sock.accept()
        )  # => accept: BLOCKS until a client completes the handshake
...
ready_event = (
    threading.Event()
)  # => co-11: coordinates "server is ready" without a sleep guess
...
ready_event.wait(
    timeout=5
)  # => blocks the main thread until bind()+listen() have both run

with socket.create_connection(
    (HOST, PORT), timeout=5
) as client_sock:  # => triggers accept() above
    client_sock.sendall(
        b"hi"
    )  # => a few bytes so the server's recv() has something to drain
    print("state: client connected successfully")  # => Output line 3

server_thread.join(
    timeout=5
)  # => waits for the server thread to finish handling that one client
