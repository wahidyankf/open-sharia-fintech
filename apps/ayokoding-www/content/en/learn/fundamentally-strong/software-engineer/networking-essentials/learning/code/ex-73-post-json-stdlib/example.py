...
response = (
    conn.getresponse()
)  # => co-13: parses the echoed response's status line + headers
status = response.status  # => an int, e.g. 200 -- the POST's own success/failure signal
response_body = json.loads(
    response.read()
)  # => postman-echo replies with JSON describing what it saw
...
print(
    f"echoed content-type: {response_body['headers']['content-type']}"
)  # => the header round-trip
print(
    f"echoed json: {response_body['json']}"
)  # => the exact payload dict, echoed back unchanged
...
assert (
    response_body["json"] == payload
)  # => confirms the exact payload round-tripped correctly
print(
    "ex-73 OK"
)  # => confirms the headers, encoding, and body all worked together correctly
