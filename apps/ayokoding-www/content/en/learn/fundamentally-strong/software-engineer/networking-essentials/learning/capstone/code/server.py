...
def read_line(sock: socket.socket, buffer: bytearray) -> bytes | None:
    ...
    while b"\n" not in buffer:  # => keep reading until a full line has actually arrived
        ...
        if (
            not chunk
        ):  # => an empty recv() means the peer closed -- co-07's graceful signal
            return None
...
def handle_command(command: bytes) -> bytes:
    ...
    )  # => a graceful reply, never a crash (co-11)
...
def handle_client(conn: socket.socket, addr: tuple[str, int]) -> None:
    ...
    with conn:
        ...
        while (
            True
        ):  # => co-07: loops until the CONNECTION itself signals it is finished
            command = read_line(conn, buffer)
            if (
                command is None
            ):  # => co-07: the client closed -- exit this loop gracefully
                break
...
def run_server(host: str, port: int, client_count: int | None) -> None:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        ...
        )  # => the signal client.py waits for
        ...
        while client_count is None or served < client_count:
            conn, addr = (
                sock.accept()
            )  # => co-07: blocks for the next client's handshake
