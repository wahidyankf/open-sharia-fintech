...
UNREACHABLE_HOST = (
    "192.0.2.1"  # => co-01: guaranteed non-routed, unlike Example 59's real hosts
)
PORT = 80  # => co-05: the port doesn't matter here -- the HOST itself is unreachable
TIMEOUT_SECONDS = (
    2.0  # => the exact upper bound this example's assertion below checks against
)

start = (
    time.perf_counter()
)  # => a clock reading taken right before the doomed connect() attempt
try:  # => wrapped because a TimeoutError (or, on some networks, an OSError) is EXPECTED here
    ...
    TimeoutError
):  # => co-10: no SYN-ACK and no RST ever arrived -- silence, not rejection
    outcome = "TimeoutError raised, as expected"
except (
    OSError
) as err:  # => some networks respond with an ICMP unreachable instead of silence
    outcome = f"OSError raised instead: {err}"
elapsed = time.perf_counter() - start  # => convert to a real, measured elapsed duration

print(
    f"outcome: {outcome}"
)  # => expect "TimeoutError raised, as expected" on a real network
print(
    f"elapsed: {elapsed:.2f}s (timeout was set to {TIMEOUT_SECONDS}s)"
)  # => bounded, not endless

assert (
    elapsed < TIMEOUT_SECONDS + 1.0
)  # => confirms the call didn't hang well past the timeout
print(
    "ex-74 OK"
)  # => confirms the timeout was honored, bounding an otherwise-endless wait
