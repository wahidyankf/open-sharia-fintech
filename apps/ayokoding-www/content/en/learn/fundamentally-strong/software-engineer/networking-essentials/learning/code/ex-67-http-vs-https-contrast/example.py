...
def fetch_plaintext_over(
    port: int,
) -> bytes:  # => sends the SAME raw bytes to either port
    request = f"GET / HTTP/1.1\r\nHost: {HOST}\r\nConnection: close\r\n\r\n".encode()
    with socket.create_connection(
        (HOST, port), timeout=5
    ) as sock:  # => co-05: port varies, host doesn't
        sock.sendall(
            request
        )  # => co-17: no TLS handshake attempted here -- raw bytes only
        data = b""  # => accumulates whatever bytes come back, however the port chooses to respond
        sock.settimeout(
            3
        )  # => bounds the read loop below in case a port simply stays silent
        try:  # => port 443 may time out instead of replying -- that path is handled, not a crash
            while (
                True
            ):  # => co-11: loop until the peer closes or the timeout above fires
                chunk = sock.recv(
                    4096
                )  # => reads whatever arrives next, up to 4096 bytes
            ...
            TimeoutError
        ):  # => co-17: 443 may simply refuse to reply to un-negotiated bytes
            pass
...
port_80_response = fetch_plaintext_over(
    80
)  # => the exact same request bytes as port 443 below
port_80_status = port_80_response.split(b"\r\n", 1)[
    0
]  # => co-13: everything up to the first \r\n
...
port_443_response = fetch_plaintext_over(
    443
)  # => identical request() call, different PORT only
port_443_status = port_443_response.split(b"\r\n", 1)[
    0
]  # => co-13: the same parsing as port 80
print(f"port 443 (expects TLS), sent raw HTTP directly: {port_443_status.decode()}")

assert (
    port_80_status == b"HTTP/1.1 200 OK"
)  # => co-17: port 80 speaks plain HTTP, no complaints
assert (
    port_443_status == b"HTTP/1.1 400 Bad Request"
)  # => port 443 rejects un-encrypted bytes
assert (
    b"plain HTTP request was sent to HTTPS port" in port_443_response
)  # => the WHY, in plain text
...
print(
    "ex-67 OK"
)  # => confirms both ports were probed with the identical plaintext request
