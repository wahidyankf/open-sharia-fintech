...
def handle_command(
    command: bytes,
) -> bytes:  # => the same PING/TIME protocol as Example 36
...
def handle_client(
...
) -> None:  # => co-10: runs on ITS OWN thread
    with conn:  # => this handler's own connection -- closes automatically on block exit
        ...
        command = read_line(
            conn, buf
        )  # => blocks only THIS thread while waiting for a command
...
) -> None:  # => co-01: one thread spawned per accepted client
    with socket.socket(
        ...
        for delay in (
            0.3,
            0.0,
        ):  # => accept exactly two clients, one delayed, one immediate
            conn, _ = (
                sock.accept()
        ...
        for (
            handler
        ) in handlers:  # => waits for BOTH handler threads, not just the last started
            handler.join(
                timeout=5
...
def client(
    name: bytes, command: bytes, results: dict[bytes, tuple[bytes, float]]
) -> None:
    ...
    buf = (
        bytearray()
    )  # => this client's own leftover-bytes buffer -- separate from the server's
    reply = read_line(
        sock, buf
    )  # => blocks until this client's OWN handler thread replies
    elapsed = (
        time.monotonic() - start
    )  # => total time from connect to being served, for THIS client
    results[name] = (
        reply,
        elapsed,
    )  # => stores both the reply AND the timing, keyed by name
...
results: dict[
    bytes, tuple[bytes, float]
] = {}  # => co-01: measured, not assumed, per-client timing
overall_start = time.monotonic()  # => a SEPARATE clock measuring both clients TOGETHER
thread_a = threading.Thread(
    target=client, args=(b"A", b"PING", results)
)  # => A's own thread
thread_b = threading.Thread(
    target=client, args=(b"B", b"PING", results)
)  # => B's own thread
...
assert (
    a_reply == b"PONG"
)  # => confirms A's command genuinely round-tripped through the server
assert (
    b_reply == b"PONG"
)  # => confirms B's command genuinely round-tripped through the server
...
print(
    "ex-76 OK"
)  # => confirms the PING/TIME protocol survived being served concurrently
