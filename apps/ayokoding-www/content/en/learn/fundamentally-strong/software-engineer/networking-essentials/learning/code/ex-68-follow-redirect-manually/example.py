...
conn = http.client.HTTPConnection(
    HOST, 80, timeout=5
)  # => co-23: plaintext port -- same as Ex 61
conn.request("GET", "/")  # => co-14: the FIRST of two requests this example makes
first_response = (
    conn.getresponse()
)  # => co-13: parses the redirect status line + headers
first_status = (
    first_response.status
)  # => co-15: a 3xx status signals "go elsewhere instead"
...
print(
    f"first request: {first_status}, Location: {location}"
)  # => co-15: a 3xx, plus WHERE to go
...
assert first_status in (
    ...
    307,
    308,
)  # => confirms this really was a redirect status

# Step 2: parse the Location header and issue a SECOND, manual request to it (co-18).
parsed = urlsplit(
    location
)  # => co-02: breaks the redirect target into scheme/host/path parts
assert parsed.hostname is not None  # => narrows str | None to str for the type checker
target_host: str = (
    parsed.hostname
)  # => the SECOND request's host -- may differ from HOST above
follow_conn: (
    http.client.HTTPConnection
)  # => declared here since its concrete type varies below
if (
    parsed.scheme == "https"
):  # => co-17: the redirect target may switch schemes entirely
    follow_conn = http.client.HTTPSConnection(
        target_host, timeout=5
    )  # => co-17: TLS this time
else:
    follow_conn = http.client.HTTPConnection(
        target_host, timeout=5
    )  # => stays plaintext
follow_conn.request(
    "GET", parsed.path or "/"
)  # => co-14: the SECOND request, to the NEW target
final_response = (
    follow_conn.getresponse()
)  # => co-13: parses the FINAL page's status + headers
final_status = (
    final_response.status
)  # => expected to be a real 200, not another redirect
...
assert (
    final_status == 200
)  # => confirms manually following the redirect reached the real page
print(
    "ex-68 OK"
)  # => confirms both the redirect detection and the manual follow-up succeeded
