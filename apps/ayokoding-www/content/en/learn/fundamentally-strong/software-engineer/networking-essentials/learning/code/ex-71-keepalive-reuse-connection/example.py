...
conn.request(
    "GET", "/"
)  # => request 2, over the EXACT SAME socket -- no reconnect happened
second = conn.getresponse()  # => co-13: parses request 2's status line + headers
second_status = (
    second.status
)  # => captured from the SAME conn, no new HTTPConnection() call
...
print(
    "ex-71 OK"
)  # => confirms two full request/response cycles shared one real connection
