...
OPEN_HOST = (
    "example.com"  # => co-05: a real host with a real listener on port 443 (HTTPS)
)
...
def try_connect(
    host: str, port: int
) -> str:  # => co-10: connect() either succeeds or raises
...
closed_result = try_connect(
    CLOSED_HOST, CLOSED_PORT
)  # => a genuinely closed local port

print(f"{OPEN_HOST}:{OPEN_PORT} -> {open_result}")  # => expect "connected successfully"
print(
    f"{CLOSED_HOST}:{CLOSED_PORT} -> {closed_result}"
)  # => expect "ConnectionRefusedError: ..."

assert (
    open_result == "connected successfully"
)  # => confirms the genuinely open port succeeds
assert closed_result.startswith(
    "ConnectionRefusedError"
)  # => confirms the closed port raises, not hangs
print(
    "ex-59 OK"
)  # => confirms both the success case and the failure case were reproduced
