...
def run_server() -> None:  # => a minimal, hand-rolled HTTP/1.1 responder (co-12, co-13)
    with socket.socket(
        ...
        )  # => the signal nc's caller waits for
        conn, _ = sock.accept()  # => accepts nc's raw TCP connection, no TLS involved
        with (
            conn
        ):  # => nc's connected socket -- closes automatically when this block exits
            request = conn.recv(
                4096
            )  # => reads whatever raw bytes nc piped in verbatim
            print(
                f"server saw raw request bytes:\n{request.decode(errors='replace')}"
            )  # => co-12
            ...
            conn.sendall(
                response
            )  # => nc prints these exact raw bytes to its own stdout
