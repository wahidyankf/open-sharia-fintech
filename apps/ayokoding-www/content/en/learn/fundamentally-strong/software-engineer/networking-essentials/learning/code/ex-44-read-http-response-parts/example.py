...
HOST = (
    "info.cern.ch"  # => chosen SPECIFICALLY for its plain, non-chunked response shape
)
...
def fetch_raw(
...
) -> bytes:  # => same handcrafted request as Ex 43
    ...
    with socket.create_connection(
        ...
        )  # => co-12: the raw request bytes, sent as-is
...
raw = fetch_raw(
    HOST, PORT, "/"
)  # => one function call replaces Example 43's inline socket code
...
assert status_line.startswith(
    b"HTTP/1.1 200"
)  # => confirms the status-line split is correct
assert len(header_lines) > 0  # => confirms at least one header was isolated
assert body.startswith(
    b"<html>"
)  # => confirms the body split lands on real HTML, not headers
print(
    "ex-44 OK"
)  # => confirms the three-way split (status/headers/body) was correct end to end
