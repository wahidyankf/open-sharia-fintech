...
PORT = (
    50032  # => co-05: a fresh ephemeral port, distinct from every other example's port
)


def server(
    ready: threading.Event,
) -> None:  # => a minimal echo server, backgrounded (co-10)
...
with socket.create_connection(
    (HOST, PORT), timeout=5
) as sock:  # => connect: the TCP handshake
    ...
    reply = sock.recv(
        1024
    )  # => recv: reads up to 1024 bytes, blocking until at least 1 arrives
...
print(
    "ex-32 OK"
)  # => a final marker confirming the assertion above passed, not just ran
