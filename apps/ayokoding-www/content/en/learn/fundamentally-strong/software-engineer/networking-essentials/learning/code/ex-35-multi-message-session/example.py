...
HOST = (
    "127.0.0.1"  # => loopback -- keeps this multi-message demo local and deterministic
)
...
) -> None:  # => co-11: many messages, ONE persistent connection
    with socket.socket(
        ...
        with (
            ...
            buf = (
                bytearray()
            )  # => ONE buffer shared across all three messages on this connection
            for _ in range(
                3
            ):  # => the client sends exactly 3 lines -- reply to each in turn
                line = read_line(
                    conn, buf
                )  # => co-07: TCP guarantees these arrive IN ORDER
                reply = line.upper() + b"\n"  # => a trivial per-message transformation
                conn.sendall(
                    reply
                )  # => each request gets its own response before the next arrives
...
replies: list[
    bytes
] = []  # => co-01: measured responses, one per message, in receipt order
with socket.create_connection(
    ...
    for word in (
        ...
        b"second",
        b"third",
    ):  # => three separate messages, one connection
        sock.sendall(word + b"\n")  # => sent one at a time, waiting for each reply
        replies.append(
            read_line(sock, buf)
        )  # => waits for THIS message's reply before looping
        print(
            f"client got: {replies[-1]!r}"
        )  # => confirms each reply arrives before the next send
...
assert replies == [
    ...
    b"SECOND",
    b"THIRD",
]  # => confirms strict request/response order
...
print(
    "ex-35 OK"
)  # => confirms all three messages round-tripped in order, unmodified beyond upper()
