...
conn = http.client.HTTPSConnection(
    HOST, 443, timeout=5
)  # => co-05: HTTPS's well-known port
conn.request(
    "GET", "/"
)  # => co-14: sends the GET -- identical call shape to Example 61
response = (
    conn.getresponse()
)  # => co-13: parses the (now decrypted) status line + headers
status = response.status  # => an int, e.g. 200 -- unchanged by TLS being present
body = (
    response.read()
)  # => the decrypted body, already assembled -- no manual TLS handling
...
)  # => confirms real HTML bytes, not TLS handshake bytes

assert status == 200  # => confirms a real, successfully DECRYPTED 200 response
assert body.startswith(
    b"<!doctype"
)  # => confirms the actual HTML, not TLS handshake bytes
print(
    "ex-64 OK"
)  # => confirms the same request/response cycle worked, now fully encrypted
