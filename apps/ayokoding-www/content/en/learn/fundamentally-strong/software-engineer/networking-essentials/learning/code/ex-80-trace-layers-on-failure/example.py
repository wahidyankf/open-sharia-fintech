...
def classify_failure(
    host: str, port: int
) -> str:  # => tries DNS FIRST, then TCP, in that order
    try:  # => wrapped so a DNS failure doesn't crash -- it's reported, not raised uncaught
        socket.gethostbyname(
            host
        )  # => co-03: DNS resolution -- the FIRST layer that can fail
    except (
        socket.gaierror
    ) as err:  # => "gai" = getaddrinfo -- the resolver itself failed
        return f"failed at DNS layer: {err}"

    try:  # => reached only if DNS succeeded -- this tries the NEXT layer down
        with socket.create_connection(
            (host, port), timeout=5
        ):  # => co-07: the SECOND layer
            return "connected successfully"
    except (
        ConnectionRefusedError
    ) as err:  # => DNS succeeded, but nobody answered on this port
        return f"failed at TCP layer: {err}"


dns_failure = classify_failure(
    BAD_HOST, 80
)  # => guaranteed to fail at DNS, never reaches TCP
tcp_failure = classify_failure(
    CLOSED_HOST, CLOSED_PORT
)  # => DNS succeeds, TCP is what fails

print(f"{BAD_HOST}:80 -> {dns_failure}")
print(f"{CLOSED_HOST}:{CLOSED_PORT} -> {tcp_failure}")

assert dns_failure.startswith(
    "failed at DNS layer"
)  # => the bad host never even got an IP
assert tcp_failure.startswith(
    "failed at TCP layer"
)  # => this host resolved fine; TCP refused
