...
) -> None:  # => co-22: a server that reads the Accept header
    with socket.socket(
        ...
        with (
            ...
            )  # => co-16: read ONE header's value
            if (
                wants_json
            ):  # => co-22: the client's Accept header decides the representation
                body = b'{"greeting": "hello"}'  # => the JSON representation of the SAME greeting
                content_type = (
                    b"application/json"  # => the Content-Type that MATCHES the body
                )
            else:
            ...
            conn.sendall(
                response
            )  # => the chosen representation, sent back over this connection
...
with socket.create_connection(
    ...
    sock.sendall(
        request.encode()
    )  # => co-12: the raw hand-built request, sent as bytes
    response = (
        b""  # => accumulates the full response -- its final size isn't known upfront
    )
    while (
        True
...
print(
    f"headers:\n{head.decode()}"
)  # => expect Content-Type: application/json in this output
print(
    f"body: {body!r}"
)  # => expect the JSON body, since the request's Accept asked for JSON

assert b"application/json" in head  # => confirms the server chose JSON based on Accept
assert (
    body == b'{"greeting": "hello"}'
)  # => confirms the BODY is actually JSON, not plain text
...
print(
    "ex-51 OK"
)  # => confirms the Accept -> Content-Type negotiation worked end to end
