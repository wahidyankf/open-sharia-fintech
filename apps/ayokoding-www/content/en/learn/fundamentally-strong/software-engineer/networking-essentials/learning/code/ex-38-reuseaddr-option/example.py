...
def bind_serve_and_close(reuse: bool) -> socket.socket:
    ...
    )  # => a fresh IPv4 TCP socket each call
    if (
        reuse
    ):  # => co-10: SO_REUSEADDR lets a new socket bind to a port stuck in TIME_WAIT
        sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    sock.bind(
        (HOST, PORT)
    )  # => this is the call that fails if the port is still TIME_WAIT'd
    sock.listen(
        1
    )  # => flips the socket passive -- required before this fn can be reused
    return (
        sock  # => intentionally NOT closed here -- the caller controls WHEN it closes
    )
...
first_server = bind_serve_and_close(
    reuse=True
)  # => reuse=True here so THIS bind can't fail
peer = socket.create_connection(
    (HOST, PORT), timeout=5
)  # => a real client triggers accept()
conn, _ = (
    first_server.accept()
)  # => completes the handshake peer's connect() started above
...
try:
    second_server = bind_serve_and_close(
        reuse=False
    )  # => no SO_REUSEADDR set this time
    second_server.close()  # => only reached if bind() above did NOT raise
    without_reuse_result = (
        "bind succeeded"  # => on this OS/timing, no collision occurred
    )
except OSError as err:
    without_reuse_result = (
        f"bind FAILED: {err}"  # => the expected "Address already in use"
    )
...
third_server = bind_serve_and_close(
    reuse=True
)  # => SO_REUSEADDR: reuse a TIME_WAIT port
print(
    "with SO_REUSEADDR: bind succeeded"
)  # => reaching this line at all proves bind() worked
