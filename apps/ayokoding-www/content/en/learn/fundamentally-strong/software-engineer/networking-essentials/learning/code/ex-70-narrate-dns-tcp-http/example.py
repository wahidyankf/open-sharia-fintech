...
def narrate(
    ...
    print(
        f"[TCP]  connected to {address}:{port}  ({tcp_ms:.1f} ms)"
    )  # => stage 2's own timing

    # Stage 3 -- HTTP: a request/response message exchanged OVER that open connection (co-12).
    http_start = (
        time.perf_counter()
    )  # => a THIRD clock, isolating just the request/response
    ...
    print(
        f"[HTTP] {status_line}  ({http_ms:.1f} ms)"
    )  # => stage 3's own isolated timing


narrate(
    HOST, PORT, PATH
)  # => runs all three stages, timing each one separately, in sequence
