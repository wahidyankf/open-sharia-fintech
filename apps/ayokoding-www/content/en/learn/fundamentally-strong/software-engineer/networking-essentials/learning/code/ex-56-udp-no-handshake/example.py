...
with socket.socket(
...
) as sock:  # => a bare UDP socket, no bind()
    sock.settimeout(
        1.0
    )  # => co-08: UDP gives no delivery confirmation -- bound the wait
    ...
    sock.sendto(
        b"is anyone there?", (HOST, PORT)
    )  # => this call returns immediately, no error
    elapsed_to_send = (
        time.monotonic() - start
    )  # => the time sendto() itself took to return
    print(
        f"sendto() returned in {elapsed_to_send:.4f}s with NO error"
    )  # => co-08: no handshake
    ...
    try:  # => wrapped because recvfrom() below is EXPECTED to time out, not to error
        ...
        TimeoutError
    ):  # => co-08: silence is the ONLY signal -- no "port closed" notification
        outcome = (
...
assert (
    elapsed_to_send < 0.1
)  # => confirms sendto() itself never blocked on the missing listener
print(
    "ex-56 OK"
)  # => confirms both the instant sendto() and the eventual timeout were observed
