...
def http_get_status_line(
    host: str, port: int, path: str
) -> str:  # => DNS+TCP+HTTP, all by hand
    ...
    with socket.create_connection(
        (host, port), timeout=5
    ) as sock:  # => co-07: TCP handshake
        sock.sendall(
            request.encode("ascii")
        )  # => co-12: the hand-built request, sent raw
        response = b""  # => accumulates the full response -- its final size isn't known in advance
        while (
            True
        ):  # => co-11: keep reading until the server closes (Connection: close)
...
status_line = http_get_status_line(
    "example.com", 80, "/"
)  # => resolves, connects, requests, parses
print(
    f"status line: {status_line}"
)  # => expect a real status line, e.g. "HTTP/1.1 200 OK"

assert (
    status_line == "HTTP/1.1 200 OK"
)  # => a real status line, from a real server, real bytes
print(
    "ex-69 OK"
)  # => confirms a hand-built HTTP client, with zero libraries, genuinely worked
