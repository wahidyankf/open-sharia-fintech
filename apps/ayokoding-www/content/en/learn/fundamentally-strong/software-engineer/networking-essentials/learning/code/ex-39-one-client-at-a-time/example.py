...
def server(ready: threading.Event) -> None:  # => co-01: a SINGLE-threaded accept loop
    with socket.socket(
        ...
        sock.listen(
            5
        )  # => backlog of 5 -- client B can QUEUE here while client A is served
        ready.set()
        ...
        for _ in range(2):  # => this demo serves exactly two clients, one at a time
            conn, _ = (
                sock.accept()
            )  # => blocks until the NEXT client is ready to be served
            with (
                conn
            ):  # => this one connection's socket -- closes automatically on block exit
                data = conn.recv(
                    1024
                )  # => blocks HERE until this client actually sends
                ...
                conn.sendall(
                    data.upper()
                )  # => the ONLY per-client work: uppercase and reply
...
def client(name: bytes, delay_before_send: float, results: dict[bytes, float]) -> None:
    ...
    time.sleep(
        delay_before_send
    )  # => client A stalls here, holding up the server's recv()
...
time.sleep(
    0.05
)  # => a tiny head start so A's connection is accepted before B's arrives
client_b.start()  # => B's thread starts, and B's connect() races A's ongoing stall
client_a.join(
    timeout=5
)  # => waits for A's full round trip (connect, stall, send, recv) to finish
client_b.join(
    timeout=5
)  # => waits for B's full round trip, including its queued wait, to finish
server_thread.join(timeout=5)  # => waits for the server to have served both clients

print(
    f"client A total time: {results[b'A']:.3f}s"
)  # => expect roughly 0.3s, A's own stall
print(
    f"client B total time: {results[b'B']:.3f}s"
)  # => expect roughly 0.3s too, from QUEUEING
...
assert (
    results[b"B"] >= 0.2
)  # => B's wait proves it queued behind A, not merely its own path
print(
    "ex-39 OK"
)  # => confirms the sequential-serving claim was measured, not just asserted
