...
def read_line(
    ...
    buffer[:] = (
        rest  # => whatever came AFTER the newline stays buffered for the NEXT reply
    )
    return bytes(
        line
...
def run_client(
    client_name: str,
) -> list[bytes]:  # => "client_name" only labels this run's output
    replies: list[
        bytes
    ] = []  # => co-01: measured, not assumed, one reply per command sent
    with socket.create_connection(
        (HOST, PORT), timeout=5
    ) as sock:  # => co-07: one connection
        buf = (
            bytearray()
        )  # => this client's own leftover-bytes buffer, private to this run
        for command in (
            b"PING",
            b"TIME",
        ):  # => two commands over the ONE connection (co-11)
            sock.sendall(
                command + b"\n"
            )  # => sends this command, then waits for its own reply
            replies.append(
                read_line(sock, buf)
            )  # => waits for THIS command's reply before looping
    ...
    print(
        f"{client_name}: PING -> {replies[0]!r}, TIME -> {replies[1]!r}"
    )  # => both replies at once
...
if __name__ == "__main__":  # => only runs when invoked directly, not when imported
    ...
    assert replies[
        1
    ].isdigit()  # => confirms TIME returned a plausible-looking epoch timestamp
    print(
        "ex-81 client OK"
    )  # => confirms both commands round-tripped over one connection
