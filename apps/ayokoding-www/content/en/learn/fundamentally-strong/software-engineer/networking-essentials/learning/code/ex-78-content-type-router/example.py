...
def build_response(
    accept_header: str,
) -> bytes:  # => co-22: content negotiation, both directions
    if (
        "application/json" in accept_header
    ):  # => co-22: the client explicitly asked for JSON
        ...
        content_type = (
            b"text/plain"  # => the Content-Type that MATCHES this body instead
        )
...
def handle_one(
    port: int, ready: threading.Event
) -> None:  # => a fresh single-shot server
    with socket.socket(
        socket.AF_INET, socket.SOCK_STREAM
    ) as sock:  # => same triple as Ex 29+
        sock.setsockopt(
            socket.SOL_SOCKET, socket.SO_REUSEADDR, 1
        )  # => lets round 2 reuse PORT
        ...
        conn, _ = (
            sock.accept()
        )  # => BLOCKS until this round's client completes the handshake
        with conn:  # => this round's connection -- closes automatically on block exit
            request = conn.recv(
                4096
            ).decode()  # => reads the raw request bytes, decoded to text
            ...
            conn.sendall(
                build_response(accept_line)
            )  # => the negotiated reply, sent back


def request_with_accept(
    accept_value: str,
) -> bytes:  # => sends one GET with a chosen Accept value
    with socket.create_connection(
        ...
        sock.sendall(
            request.encode()
        )  # => co-12: the hand-built request, sent as raw bytes
...
def run_round(
    accept_value: str,
) -> bytes:  # => co-01: one fresh server + one request, per round
    ready = (
        threading.Event()
    )  # => a NEW event per round -- this round's own ready-signal
    server_thread = threading.Thread(
        target=handle_one, args=(PORT, ready), daemon=True
    )  # => co-11
    server_thread.start()  # => runs THIS round's single-shot server concurrently
    ready.wait(
        timeout=5
    )  # => blocks until THIS round's bind()+listen() genuinely completed
    response = request_with_accept(
        accept_value
    )  # => sends the request, waits for the reply
    server_thread.join(
        timeout=5
    )  # => waits for THIS round's server to finish before returning
    return response  # => the ONE value this function hands back to its caller


# Round 1: client asks for JSON.
json_response = run_round(
    "application/json"
)  # => a fresh server + one JSON-requesting client
json_head, _, json_body = json_response.partition(
    b"\r\n\r\n"
)  # => co-13: split at the boundary

# Round 2: client asks for plain text -- a FRESH server, same port, reused via SO_REUSEADDR.
text_response = run_round(
    "text/plain"
)  # => a SECOND fresh server + one plain-text-requesting client
text_head, _, text_body = text_response.partition(
    b"\r\n\r\n"
)  # => co-13: split at the boundary

print(
    f"Accept: application/json -> {json_body!r}"
)  # => expect the JSON body from round 1
print(
    f"Accept: text/plain -> {text_body!r}"
)  # => expect the plain-text body from round 2

assert (
    b"application/json" in json_head and json_body == b'{"status": "ok"}'
)  # => round 1 correct
