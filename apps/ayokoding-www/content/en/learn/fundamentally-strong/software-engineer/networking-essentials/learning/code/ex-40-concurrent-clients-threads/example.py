...
def handle_client(
    conn: socket.socket, delay: float
) -> None:  # => runs on its OWN thread
    with conn:  # => this handler's own connection -- closes automatically on block exit
        data = conn.recv(
            1024
        )  # => blocks only THIS thread -- other handler threads are unaffected
        time.sleep(
            delay
        )  # => simulates slow per-client work -- does NOT block other clients
        conn.sendall(
            data.upper()
        )  # => the same trivial per-client transformation as Example 39


def server(
    ready: threading.Event, delays: list[float]
) -> None:  # => one thread spawned per delay
    with socket.socket(
        ...
        for delay in delays:  # => accept exactly two clients, then stop
            ...
            handler = threading.Thread(
                target=handle_client, args=(conn, delay)
            )  # => co-10: NOT run yet
        ...
        for handler in (
            handlers
        ):  # => waits for BOTH handler threads, not just the last one started
            handler.join(
                timeout=5
            )  # => waits for every spawned handler thread to finish


def client(
    name: bytes, results: dict[bytes, float]
) -> None:  # => same shape as Example 39's
    ...
    sock.sendall(
        name
    )  # => no artificial client-side delay this time -- the SERVER delays now
    sock.recv(1024)  # => only returns once its OWN handler thread has replied
    results[name] = (
        time.monotonic() - start
    )  # => total time from connect to being served
...
server_thread = threading.Thread(
    target=server, args=(ready_event, [0.3, 0.3]), daemon=True
)
...
results: dict[
    bytes, float
] = {}  # => co-01: measured, not assumed, per-client round-trip times
overall_start = time.monotonic()  # => a SEPARATE clock measuring both clients TOGETHER
client_c = threading.Thread(
    target=client, args=(b"C", results)
)  # => C's own thread, unstarted
client_d = threading.Thread(
    target=client, args=(b"D", results)
)  # => D's own thread, unstarted
...
client_c.join(
    timeout=5
)  # => waits for C's full round trip, including its 0.3s server delay
client_d.join(
    timeout=5
)  # => waits for D's full round trip, including its own 0.3s server delay
overall_elapsed = (
    time.monotonic() - overall_start
)  # => the aggregate proof of concurrency
server_thread.join(
    timeout=5
)  # => waits for the server to have handled both connections

print(
    f"client C time: {results[b'C']:.3f}s"
)  # => expect roughly 0.3s, C's own server delay
print(
    f"client D time: {results[b'D']:.3f}s"
)  # => expect roughly 0.3s too, served concurrently
print(
    f"overall wall-clock for BOTH clients: {overall_elapsed:.3f}s"
...
assert (
    overall_elapsed < 0.55
)  # => well under the 0.6s a sequential server would have taken
print(
    "ex-40 OK"
)  # => confirms the aggregate-time proof of concurrency held for this run
