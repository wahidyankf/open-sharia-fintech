...
def handle_command(
    command: bytes,
) -> bytes:  # => co-01, co-11: a tiny request/response protocol
...
def server(
    ...
    with socket.socket(
        ...
        with (
            ...
            for _ in range(
                2
            ):  # => this client sends exactly two commands, PING then TIME
...
with socket.create_connection(
    ...
    time_reply = read_line(
        sock, buf
    )  # => waits for the server's dynamic epoch-timestamp reply
    print(
        f"TIME -> {time_reply!r}"
    )  # => expect a DIFFERENT number each run, unlike PING's PONG
...
assert (
    time_reply.isdigit()
)  # => confirms TIME returned a plausible-looking epoch timestamp
print(
    "ex-36 OK"
)  # => confirms both request/reply pairs completed correctly over one connection
