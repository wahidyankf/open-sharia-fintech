...
HOST = (
    "127.0.0.1"  # => loopback -- keeps this graceful-close demo local and deterministic
)
PORT = 50037  # => co-05: a fresh ephemeral port, unique to this example


def server(
    ready: threading.Event, observed: list[str]
) -> None:  # => "observed" is the shared log
    with socket.socket(
        ...
        with (
            ...
            while (
                True
            ):  # => co-07: loops until the connection itself signals it is done
                data = conn.recv(
                    1024
                )  # => reads whatever is available -- returns b'' on close
                if (
                    not data
                ):  # => co-10: recv() returning b'' means the PEER closed its side
                    observed.append(
                ...
                observed.append(
                    f"received {data!r}"
                )  # => logs every message that arrives before close
...
thread.join(
    timeout=5
)  # => waits for the server's loop to actually observe the close and exit
...
assert (
    log[-1] == "empty recv -- peer closed, exiting loop cleanly"
)  # => confirms clean shutdown
print(
    "ex-37 OK"
)  # => confirms the server's loop exited via the graceful path, not a crash
