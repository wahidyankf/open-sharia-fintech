...
PORT = (
    50029  # => an ephemeral, unregistered port well above the well-known range (co-05)
)
...
def run_server() -> None:  # => binds, listens, accepts ONE client, echoes, then exits
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_sock:
        ...
        server_sock.bind(
            (HOST, PORT)
        )  # => claims (HOST, PORT) -- fails if already in use
        server_sock.listen(
            1
        )  # => marks the socket passive: ready to queue incoming connections
        print(f"listening on {HOST}:{PORT}")  # => a signal the client script waits for
        conn, addr = (
            server_sock.accept()
        )  # => BLOCKS until a client connects (co-07 handshake)
        ...
        with conn:
            ...
            data = conn.recv(
                1024
            )  # => reads up to 1024 bytes sent by the client, blocking
            print(f"received: {data!r}")
            # => shows the raw bytes BEFORE echoing, so the transcript reads as a clear pair
            conn.sendall(
                data
            )  # => echoes the EXACT bytes back -- sendall loops until all sent
