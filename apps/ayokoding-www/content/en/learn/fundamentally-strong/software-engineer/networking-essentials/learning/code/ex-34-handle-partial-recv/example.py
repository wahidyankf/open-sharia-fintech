...
HOST = (
    "127.0.0.1"  # => loopback -- keeps this large-payload demo local and deterministic
)
...
def recv_exact(sock: socket.socket, count: int) -> bytes:
    ...
    while remaining > 0:  # => keeps looping until the full payload has arrived
        chunk = sock.recv(
            min(65536, remaining)
        )  # => never over-read past the target size
        if (
            not chunk
        ):  # => peer closed before sending everything -- a real, checkable failure
    ...
    return b"".join(
        chunks
    )  # => reassembles every partial read into the original bytes, once
...
def server(
    ...
    with socket.socket(
        ...
        with (
            ...
            data = recv_exact(
                conn, PAYLOAD_SIZE
            )  # => reassembles the full 200,000-byte payload
            intact = (
                data == b"x" * PAYLOAD_SIZE
            )  # => confirms every one of the 200,000 bytes matches
            print(
                f"server received {len(data)} bytes, intact: {intact}"
            )  # => size AND content check


ready_event = (
    threading.Event()
)  # => the same ready-signal pattern used since Example 34's peers
...
with socket.create_connection(
    ...
    sock.sendall(
        b"x" * PAYLOAD_SIZE
    )  # => one logical send; the OS may still split it on the wire
