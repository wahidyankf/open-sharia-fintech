...
static uint32_t swap_bytes_manual(uint32_t value) {
  ...
  return (byte0 << 24) | (byte1 << 16) | (byte2 << 8) |
         byte3; // => co-15: fully reversed byte order
}
...
int main(void) {
  ...
  uint32_t manual_swap = swap_bytes_manual(
      value); // => co-15: this example's own hand-written implementation
  ...
  // reversals here)
  int correct =
      (manual_swap == library_swap); // => co-15: direct equality check between
