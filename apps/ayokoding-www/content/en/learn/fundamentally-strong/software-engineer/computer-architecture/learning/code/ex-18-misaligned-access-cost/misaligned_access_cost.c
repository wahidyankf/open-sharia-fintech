...
#define BUF_BYTES                                                              \
  (64u * 1024u * 1024u) // => co-16: 64 MiB -- far bigger than any cache level
                        // on this machine
#define PASSES                                                                 \
  5 // => co-25: best-of-N -- DD-20 requires re-running noisy timings

static double
now_seconds(void) { // => co-25: one monotonic-clock read, converted to seconds
  struct timespec
      ts; // => co-25: CLOCK_MONOTONIC -- immune to wall-clock adjustments
  clock_gettime(
      CLOCK_MONOTONIC,
      &ts); // => co-25: POSIX call, portable to any Unix (per the shared brief)
  return (double)ts.tv_sec +
         (double)ts.tv_nsec /
             1e9; // => co-25: whole seconds + fractional nanoseconds
}
...
static uint64_t sum_stride8(const unsigned char *start,
  ...
  volatile uint64_t total =
      0; // => co-16: volatile defeats dead-code elimination of the reads
  size_t count =
      (buf_bytes - 8) /
      8; // => co-16: number of full 8-byte reads that fit before the end
  for (size_t i = 0; i < count;
       i++) { // => co-16: one read per 8-byte stride step
    const uint64_t *p =
        (const uint64_t *)(start + i * 8); // => co-16: pointer into the buffer,
...
int main(void) { // program entry point
  unsigned char *buf =
      malloc(BUF_BYTES + 8); // => co-16: +8 headroom so the misaligned pass
  ...
  for (size_t i = 0; i < BUF_BYTES + 8; i++)
    buf[i] =
        (unsigned char)(i *
                        2654435761u); // => co-16: deterministic filler pattern

  double
      aligned_best = 1e300,
      misaligned_best =
          1e300; // => co-25: best-of-N trackers, start at "worse than anything"
  uint64_t sink =
      0; // => co-16: accumulates results so the compiler can't discard passes

  for (int pass = 0; pass < PASSES;
       pass++) { // => co-25: PASSES repetitions, keep the fastest of each
    ...
    if (t3 - t2 < misaligned_best)
      misaligned_best =
          t3 - t2; // => co-25: keep the fastest misaligned run so far
  }

  double ratio =
      misaligned_best / aligned_best; // => co-25: relative comparison, not a
  ...
  printf("sink (ignore)         = %llu\n",
         (unsigned long long)
             sink); // => co-16: proves the reads were not optimized away
  ...
  if (ratio > 1.05) { // => co-25: a real, if modest, measured slowdown
    ...
    printf(
        "VERDICT: no reliably measurable gap on this arm64 machine -- "); // prints
                                                                          // a
                                                                          // report
                                                                          // line
