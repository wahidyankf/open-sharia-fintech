...
static long sum_column0(const int *mat, int rows, int cols,
  ...
  for (int r = 0; r < repeats; r++) { // loop header controlling the sweep below
    for (int i = 0; i < rows; i++) {  // loop header controlling the sweep below
      total += mat[(size_t)i *
                   cols]; // => co-06: stride = cols ints between accesses
    }
  }
...
int main(void) { // program entry point
  int *mat_pow2 = malloc((size_t)ROWS * COLS_POW2 *
                         sizeof(int)); // heap-allocates memory for mat_pow2
  int *mat_padded = malloc((size_t)ROWS * COLS_PADDED *
                           sizeof(int)); // heap-allocates memory for mat_padded
  ...
  long sum_pow2 =
      sum_column0(mat_pow2, ROWS, COLS_POW2, REPEATS); // declares sum_pow2
  ...
  printf("rows=%d, pow2 stride=%d ints (%zu B), padded stride=%d ints (%zu B), "
         ...
         (size_t)COLS_PADDED *
             sizeof(int), // continues the printf(...) call above
  ...
  double ratio =
      secs_pow2 /
      secs_padded; // => co-06: how much the power-of-two conflict costs
