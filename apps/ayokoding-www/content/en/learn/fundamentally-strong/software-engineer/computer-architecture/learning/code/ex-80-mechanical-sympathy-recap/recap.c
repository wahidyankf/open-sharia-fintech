...
#define TIME_CALL(                                                             \
    RESULT_TYPE, FN, ARGS_TYPE,                                                \
    ...) /* macro TIME_CALL(...): expands inline at compile time */            \
  ({                                                                           \
    ...
    for (int _trial = 0; _trial < 3;                                           \
         _trial++) { /* loop header controlling the sweep below */             \
      ...
      double _secs = (_t1.tv_sec - _t0.tv_sec) +                               \
                     (_t1.tv_nsec - _t0.tv_nsec) / 1e9; /* declares _secs */   \
...
// differs.
static long k1_sum_random(
    const int *a, const int *idx,
...
static int run_kernel1(
  ...
  double t_seq =
      TIME_CALL(long, k1_sum_sequential, (const int *, const int *, long), a,
                idx, K1_N); // declares t_seq
  double t_rand =
      TIME_CALL(long, k1_sum_random, (const int *, const int *, long), a, idx,
...
static int run_kernel2(
    void) { // defines run_kernel2(): helper function used by this example
  int *m = malloc(sizeof(int) * K2_N * K2_N); // heap-allocates memory for m
  for (long i = 0; i < K2_N * K2_N; i++)
...
static long k3_matmul_blocked(const int *a, const int *b, int *c,
  ...
  for (long ii = 0; ii < n;
       ii += K3_BLOCK) // loop header controlling the sweep below
    for (long jj = 0; jj < n;
         jj += K3_BLOCK) // loop header controlling the sweep below
      for (long kk = 0; kk < n;
           kk += K3_BLOCK) // loop header controlling the sweep below
        for (long i = ii; i < ii + K3_BLOCK;
             i++) // loop header controlling the sweep below
          for (long j = jj; j < jj + K3_BLOCK;
               j++) {                // loop header controlling the sweep below
...
static int run_kernel3(
    void) { // defines run_kernel3(): helper function used by this example
  ...
  for (long i = 0; i < K3_N * K3_N; i++) {
    a[i] = (int)(i % 5);
    b[i] = (int)(i % 7);
  } // loop header controlling the sweep below
  double t_naive =
      TIME_CALL(long, k3_matmul_naive, (const int *, const int *, int *, long),
                a, b, c1, K3_N); // declares t_naive
  double t_blocked = TIME_CALL(long, k3_matmul_blocked,
                               (const int *, const int *, int *, long), a, b,
                               c2, K3_N); // declares t_blocked
...
static int run_kernel4(
    void) { // defines run_kernel4(): helper function used by this example
  ...
  for (long i = 0; i < K4_N; i++) {
    aos[i].hot = soa[i] = (int)(i % 31);
  } // loop header controlling the sweep below
