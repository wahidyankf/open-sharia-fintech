...
chase_pointer_chain(const int *perm, long iters) { // calls __attribute__(...)
...
__attribute__((noinline)) static long
sum_independent(const int *data, long iters) { // calls __attribute__(...)
...
static double time_ns_per_iter(long (*fn)(const int *, long), const int *arr,
                               long iters,
  ...
  long (*volatile fn_indirect)(const int *, long) =
      fn; // => co-20: volatile function-pointer VARIABLE
...
int main(void) { // program entry point
  ...
  int pass =
      stall_ns >
      0.2; // dependent chain must be measurably, not just noisily, slower
