...
static void *
inc_long(void *arg) { // => co-24: one shared thread body for BOTH experiments
  volatile long *p = (volatile long *)
      arg; // => co-24: works on any single volatile long address
...
static double run_two(volatile long *a,
  ...
  return now_seconds() -
         t0; // => co-25: total wall-clock for both threads combined
}

int main(void) { // program entry point
  printf(
      "sizeof(PaddedCounter) = %zu bytes (padded to one %d B cache line)\n", // prints a report line
      sizeof(PaddedCounter),
      LINE); // => co-16: prove the padding actually landed at 128

  Unpadded *u = calloc(
      1, sizeof(Unpadded)); // => co-02: false-sharing baseline, reproduced here
  if (!u) {
    fprintf(stderr, "alloc failed\n");
    return 1;
  } // prints a report line
  double t_unpadded = run_two(
      &u->unpadded_a, &u->unpadded_b); // => co-24: ex-33's slow case, rerun

  PaddedCounter *p = calloc(
      2,
      sizeof(
          PaddedCounter)); // => co-02: array of 2 -- p[1] starts a WHOLE line
  if (!p) {
    fprintf(stderr, "alloc failed\n");
    return 1;
  ...
  double speedup =
      t_unpadded / t_padded; // => co-24: how much the padding fix recovered
  printf("padding speedup: %.2fx -> %s\n", speedup, // prints a report line
         speedup > 1.3 ? "PASS (padding fix removes the slowdown)"
                       : "FAIL"); // => co-25: self-judged PASS/FAIL
  free(u); // => release the false-sharing baseline struct
  free(p); // => release the padded-counter array
  return speedup > 1.3
             ? 0
             : 1; // => real process exit code reflecting the PASS/FAIL
}
