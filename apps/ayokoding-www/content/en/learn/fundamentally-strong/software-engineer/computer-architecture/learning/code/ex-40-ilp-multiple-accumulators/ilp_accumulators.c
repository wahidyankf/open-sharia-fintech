...
#pragma clang loop vectorize(disable)                                          \
    unroll(disable) // supporting statement for this example
  for (int i = 0; i < n;
       i += 4) {        // => co-22: n/4 iterations, 4 elements consumed each
  ...
  return sum0 + sum1 + sum2 +
         sum3; // => co-22: combined ONCE, after all chains finish
}
...
int main(void) {                              // program entry point
  ...
  double speedup =
      best_1acc /
      best_4acc; // => co-22: how much breaking the dependency bought
  printf(
      "4-accumulator is %.2fx faster -> %s\n", speedup, // prints a report line
  ...
  return (sum_1 == sum_4 && speedup > 1.2) ? 0
