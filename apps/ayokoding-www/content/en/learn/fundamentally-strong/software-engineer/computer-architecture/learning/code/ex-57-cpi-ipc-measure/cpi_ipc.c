...
sum_dependent(long n, long salt) { // calls __attribute__(...)
  volatile long acc = salt; // => co-25: volatile -- blocks the loop-collapse
                            // (see bug #2 above);
  for (long i = 0; i < n;
       i++) { //    salt distinguishes each trial's call (see bug #1 above)
    acc =
        acc + 1; // => 1 add instruction, fully serialized by the acc dependency
  }
...
__attribute__((noinline)) static long
sum_independent4(long n, long salt) { // calls __attribute__(...)
...
static double
best_of_seconds(long (*fn)(long, long), long n,
  ...
  for (int t = 0; t < TRIALS; t++) { // loop header controlling the sweep below
    ...
    result =
        fn(n, (long)t) -
        (long)t; // => co-25: salted call, then subtract the salt back out --
    clock_gettime(
        CLOCK_MONOTONIC,
...
int main(void) {                         // program entry point
  ...
  double t_dep =
      best_of_seconds(sum_dependent, N, &r_dep_check); // declares t_dep
  double t_ind =
      best_of_seconds(sum_independent4, N, &r_ind_check); // declares t_ind
  ...
  double cpi_proxy_dep =
      t_dep / (double)N; // => "seconds per instruction" -- relative CPI proxy
