...
int main(void) {  // program entry point
  ...
  double exact =
      (double)N *
      (double)x; // => co-13: exact reference -- N*x computed as ONE double
  ...
  for (long i = 0; i < N; i++) { // loop header controlling the sweep below
    naive_sum +=
        x; // => co-14: each add can lose the low bits of x once sum is large
  }
  ...
  for (long i = 0; i < N; i++) { // loop header controlling the sweep below
    float y =
        x - c; // => co-14: correct this iteration's input by the PRIOR error
    float t = kahan_sum +
              y; // => co-14: the addition that may itself lose low bits of y
    c = (t - kahan_sum) -
        y; // => co-14: recover exactly what got lost -- (t-kahan_sum) is
  ...
  double improvement =
      naive_error /
      (kahan_error > 0 ? kahan_error : 1e-12); // declares improvement
  printf("kahan is %.1fx more accurate than naive (smaller absolute error)\n",
         improvement); // prints a report line
  int pass = (kahan_error < naive_error) &&
             (naive_error > 1.0); // naive must show REAL, visible drift
