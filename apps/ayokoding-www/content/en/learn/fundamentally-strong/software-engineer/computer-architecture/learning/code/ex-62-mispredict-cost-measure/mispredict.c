...
make_predictable(long n) { // defines make_predictable(): helper function used
                           // by this example
...
static unsigned char *make_random(
    long n) { // defines make_random(): helper function used by this example
  unsigned char *flags = malloc((size_t)n); // heap-allocates memory for flags
  ...
  for (long i = 0; i < n; i++) { // loop header controlling the sweep below
    ...
    flags[i] =
        (unsigned char)((seed >> 24) & 1u); // => co-21: unpredictable 0/1
  }
  return flags; // returns the computed result
}
...
// real `cmp`+`b.ne`.
__attribute__((noinline, optnone)) static long
sum_with_branch(const unsigned char *flags,
...
static double time_it(
    const unsigned char *flags, long n,
...
int main(void) {                              // program entry point
  ...
  double extra_ns_per_iter =
      ns_per_iter_rand - ns_per_iter_pred; // declares extra_ns_per_iter
  ...
  // misprediction.
  double ns_per_mispredict =
      extra_ns_per_iter / 0.5; // declares ns_per_mispredict
  ...
  printf(" the expected order of magnitude for a deep out-of-order core's "
         "pipeline-flush cost.)\n"); // prints a report line
  int pass = (extra_ns_per_iter > 0.3) && (sum_pred != 0) &&
             (sum_rand != 0); // real, positive, non-trivial cost
