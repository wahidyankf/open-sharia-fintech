...
#define LARGE_INTS                                                             \
  (32u * 1024u * 1024u /                                                       \
   sizeof(int)) // => co-04: 32 MiB working set -- far bigger than L2/LLC
...
// cache level?"
static void
make_permutation(size_t *perm, size_t count,
  ...
  for (size_t i = count - 1; i > 0; i--) { // => co-04: Fisher-Yates shuffle
    size_t j = (size_t)rand() %
               (i + 1); // => co-04: pick a random earlier-or-equal index
    size_t tmp = perm[i];
    perm[i] = perm[j];
...
static double time_repeated_random_scans(
    const int *buf, const size_t *perm,
  ...
  size_t touches_done =
      0; // => co-04: counts individual element reads across all rescans
  ...
  while (touches_done < TOTAL_TOUCHES) { // => co-04: keep rescanning until the
    ...
    for (size_t i = 0; i < count && touches_done < TOTAL_TOUCHES;
         i++) { // => co-04: one full (or partial, at the end) rescan
  ...
  (void)
      sink; // => co-04: silences "unused" -- volatile already forced the reads
  return now_seconds() -
         t0; // => co-25: total elapsed time for exactly TOTAL_TOUCHES reads
}

int main(void) { // program entry point
  int *small =
      malloc(SMALL_INTS * sizeof(int)); // => co-04: the L1-sized working set
  int *large = malloc(LARGE_INTS *
                      sizeof(int)); // => co-04: the L2/LLC-busting working set
  size_t *small_perm = malloc(
      SMALL_INTS *
      sizeof(size_t)); // => co-04: random visiting order for the small set
  size_t *large_perm = malloc(
      LARGE_INTS *
      sizeof(size_t)); // => co-04: random visiting order for the large set
  ...
  for (int r = 0; r < REPEATS;
       r++) { // => co-25: REPEATS runs, keep the fastest of each
    double t_small = time_repeated_random_scans(
        small, small_perm,
    ...
    double t_large = time_repeated_random_scans(
        large, large_perm,
  ...
  double small_ns_per_touch = (small_best / TOTAL_TOUCHES) *
                              1e9; // => co-04: normalized to a per-touch cost
  double large_ns_per_touch =
      (large_best / TOTAL_TOUCHES) *
      1e9; // => co-04: same normalization for the large set

  printf(
      "small set (%zu ints, %zu B) best-of-%d = %.4f s, %.3f ns/touch\n", // prints
                                                                          // a
                                                                          // report
                                                                          // line
      (size_t)SMALL_INTS, (size_t)(SMALL_INTS * sizeof(int)), REPEATS,
      small_best, small_ns_per_touch); // => co-04: small-set summary
  printf(
      "large set (%zu ints, %zu MiB) best-of-%d = %.4f s, %.3f ns/touch\n", // prints a report line
      (size_t)LARGE_INTS, (size_t)(LARGE_INTS * sizeof(int) / 1024 / 1024),
      REPEATS, large_best, large_ns_per_touch); // => co-04: large-set summary
  printf("large / small per-touch ratio = %.2fx\n",
         large_ns_per_touch /
             small_ns_per_touch); // => co-04: how much slower the large set is,
                                  // per touch
  ...
  int correct =
      (large_ns_per_touch >
       small_ns_per_touch * 1.2); // => co-04: a modest, honest threshold
