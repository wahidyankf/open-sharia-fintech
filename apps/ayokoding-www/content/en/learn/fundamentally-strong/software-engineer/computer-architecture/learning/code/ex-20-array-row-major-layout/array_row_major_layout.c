...
int main(void) {            // program entry point
  ...
  for (int i = 0; i < ROWS;
       i++) // => co-17: fill with a distinct value per cell, for sanity only
    for (int j = 0; j < COLS; j++) // loop header controlling the sweep below
      a[i][j] =
          i * 100 +
          j; // => co-17: value is irrelevant to this example -- ADDRESS is

  uintptr_t base =
      (uintptr_t)&a[0][0]; // => co-17: the array's own base address --
                           // everything below is relative to it

  int all_match =
      1; // => co-17: accumulates the verdict across every (i, j) checked
  for (int i = 0; i < ROWS; i++) { // => co-17: check several rows, not just one
    for (int j = 0; j < COLS;
         j++) { // => co-17: and several columns within each row
      // ex-20: row-major means element (i, j) sits at base + (i*COLS + j) *
      // sizeof(int) -- row i's COLS elements all come before row i+1 starts
      uintptr_t hand_computed =
          base + (uintptr_t)(i * COLS + j) *
                     sizeof(int); // => co-17: the manual formula
      uintptr_t compiler_computed =
          (uintptr_t)&a[i][j]; // => co-17: what &a[i][j] actually gives
      int match = (hand_computed ==
                   compiler_computed); // => co-17: per-cell equality check
      if (!match) { // => co-17: only print failures, to keep output short
        printf(
            "MISMATCH at (%d,%d): hand=0x%lx compiler=0x%lx\n", // prints a
                                                                // report line
            i, j, (unsigned long)hand_computed,
            (unsigned long)
                compiler_computed); // continues the printf(...) call above
  ...
  printf("base + (1*%d+2)*4    = %p\n",
         ...
         (void *)(base +
                  (uintptr_t)(1 * COLS + 2) *
                      sizeof(int))); // supporting statement for this example
