...
int main(void) {                              // program entry point
  ...
  double ratio = best_write /
                 best_read; // => co-07: how much extra the store traffic costs
  printf(
      "write is %.3fx the read cost -> %s\n", ratio, // prints a report line
      ratio > 1.02
          ? "PASS (real, if modest on this hardware, extra store-traffic cost)"
