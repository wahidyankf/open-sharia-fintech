...
static const struct LatencyRow TABLE[] = {
    ...
    {"register",
     1}, // => co-01: ~1 cycle -- effectively free, the CPU's own storage
    {"L1 cache",
     4}, // => co-01: ~4 cycles -- CS:APP's commonly cited L1 hit latency
    {"L2 cache",
     12}, // => co-01: ~10-20 cycles -- one order of magnitude above L1
    {"L3 cache",
     40}, // => co-01: ~30-70 cycles -- another step up, shared across cores
...
int main(void) { // program entry point
  ...
  for (size_t i = 0; i < TABLE_ROWS;
       i++) { // => co-01: one printed row per hierarchy level
    printf("%-12s %ld\n", TABLE[i].level,
           TABLE[i]
               .approx_cycles); // => co-01: level name and its approximate cost
  }
  ...
  int strictly_increasing =
      1; // => co-01: accumulates the ordering check across all rows
  for (size_t i = 1; i < TABLE_ROWS;
       i++) { // => co-01: compares each row to the one before it
    if (TABLE[i].approx_cycles <=
        TABLE[i - 1].approx_cycles) { // => co-01: a violation would mean the
                                      // table is out of order
      strictly_increasing = 0;        // => co-01: records the violation
      printf(
          "ORDER VIOLATION: %s (%ld) <= %s (%ld)\n", // prints a report line
          TABLE[i].level,
  ...
  return strictly_increasing ? 0
                             : 1; // => co-01: nonzero exit on assertion failure
}
