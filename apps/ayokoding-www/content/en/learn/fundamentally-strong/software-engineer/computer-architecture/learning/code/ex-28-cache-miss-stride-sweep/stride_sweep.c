...
#define BUF_MB                                                                 \
  256UL // => co-05: 256 MiB buffer, far larger than any cache on this machine
#define ACCESSES                                                               \
  8000000L // => co-05: fixed access count per stride -- same work, only stride
           // varies

static double
now_seconds(void) {   // => co-25: wall-clock via clock_gettime, portable timer
  ...
  return (double)ts.tv_sec +
         (double)ts.tv_nsec /
             1e9; // => co-25: combine into one double-seconds value
}
...
int main(void) {
  ...
  size_t n_ints =
      (BUF_MB * 1024UL * 1024UL) /
      sizeof(int); // => co-01: element count, a power of two by construction
  ...
  int *buf = malloc(
      n_ints *
      sizeof(int)); // => co-01: one big flat array -- the memory under test
  if (buf == NULL) {
  ...
  for (size_t i = 0; i < n_ints; i++)
    buf[i] =
        (int)i; // => co-01: touch every page once so faults don't skew timing

  size_t mask =
      n_ints -
      1; // => co-02: n_ints is a power of two -- mask replaces slow modulo
  int strides[] = {1,  2,  4,   8,   16,  24,  32,
                   48, 64, 128, 256, 512, 1024}; // => co-02: elements per step,
                                                 // sweeping past 32 (=128B/4B)
  int n_strides = (int)(sizeof(strides) /
                        sizeof(strides[0])); // => co-02: array length, computed
  ...
  double baseline_ns =
      0.0; // => co-05: ns/access at the smallest stride -- the "all hits" floor
  double cliff_ns =
      0.0; // => co-05: ns/access once stride exceeds the cache-line size
  for (int s = 0; s < n_strides;
       s++) { // => co-02: one timed pass per candidate stride
    volatile long sum =
        0; // => co-25: volatile sink -- forces the load to really happen
    size_t idx =
        0; // => co-02: running index, wrapped with the power-of-two mask
    double t0 =
        now_seconds(); // => co-25: start the clock for this stride's run
    for (long i = 0; i < ACCESSES;
         i++) { // => co-05: fixed number of accesses, identical across strides
      idx = (idx + (size_t)strides[s]) &
            mask;      // => co-02: advance by `stride` elements, wrap via mask
    ...
    double ns_per_access =
        (t1 - t0) * 1e9 /
        (double)ACCESSES; // => co-25: normalize to a per-access cost
    printf("%8d %12.2f %10.1f\n", strides[s],
           ns_per_access, // => co-25: print this stride's real measured cost
           128.0 /
               (strides[s] *
                (double)sizeof(
                    int))); // => co-02: how many strides fit in one 128 B line
    if (strides[s] == 1)
      baseline_ns = ns_per_access; // => co-05: smallest stride -- best-case,
                                   // all same-line hits
    if (strides[s] == 32)
      cliff_ns =
          ns_per_access; // => co-02: exactly one cache line (128B/4B) per step
    (void)sum; // => co-25: sum only exists to defeat dead-code elimination
  }

  double ratio =
      cliff_ns /
      baseline_ns; // => co-05: how much slower the one-miss-per-access case is
  ...
  printf("cliff ratio: %.2fx -> %s\n",
         ...
         ratio > 1.5 ? "PASS (clear cliff at the cache-line stride)"
                     : "FAIL (no clear cliff)"); // => co-05: the assertion
  free(buf); // => co-01: release the 256 MiB buffer before exiting
  return ratio > 1.5
             ? 0
             : 1; // => co-25: a real process exit code reflecting the PASS/FAIL
}
