...
#define REPEATS                                                                \
  150 // => co-04: revisit the data this many times -- amplifies the signal
...
sum_x_aos(const Point *pts, int n, int repeats,
...
__attribute__((noinline)) static double
sum_x_soa(const double *x, int n, int repeats,
          double salt) {              // calls __attribute__(...)
  double total = salt;                // declares total
  for (int r = 0; r < repeats; r++) { // loop header controlling the sweep below
    for (int i = 0; i < n; i++)
...
int main(void) { // program entry point
  Point *pts = malloc(
      (size_t)N *
      sizeof(Point)); // => co-17: 128 B * 300K ~= 36.6 MB -- far bigger than L2
  double *x = malloc(
      (size_t)N *
      sizeof(
          double)); // => co-17: 8 B * 300K ~= 2.3 MB -- 16x smaller footprint
  if (!pts || !x) {
  ...
  for (int t = 0; t < TRIALS;
    ...
    sum_before = sum_x_aos(pts, N, REPEATS, (double)t) -
                 (double)t;    // => co-25: the "profile" (before) measurement
    ...
    sum_after = sum_x_soa(x, N, REPEATS, (double)t) -
                (double)t;     // => co-25: the "re-profile" (after) measurement
  ...
  return (abs_diff < 1e-3 && speedup > 1.3) ? 0
