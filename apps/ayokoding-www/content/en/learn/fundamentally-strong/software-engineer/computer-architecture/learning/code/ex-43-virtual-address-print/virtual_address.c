...
#define FIXED_ADDR                                                             \
  ((void *)0x200000000UL) // constant FIXED_ADDR = ((void *)0x200000000UL)
...
int main(void) {      // program entry point
  ...
  void *m = mmap(FIXED_ADDR, MAP_SIZE, PROT_READ | PROT_WRITE,
                 MAP_FIXED | MAP_ANON | MAP_PRIVATE, -1, // declares m
  ...
  *(int *)m = 111; // => co-08: write a DIFFERENT value than the child's
  ...
  int matches_target =
      (m ==
       FIXED_ADDR); // => co-08: the parent really got the requested address
  ...
  printf(                                // prints a report line
      ...
      "  -> %s\n",     // continues the printf(...) call above
      *(int *)m, m,
      (matches_target && *(int *)m == 111)
          ? "PASS"
          : "FAIL"); // continues the printf(...) call above
  return (matches_target && *(int *)m == 111)
             ? 0
