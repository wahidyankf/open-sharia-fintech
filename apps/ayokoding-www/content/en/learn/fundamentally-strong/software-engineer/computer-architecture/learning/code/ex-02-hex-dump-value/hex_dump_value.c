...
int main(void) {
  uint32_t value =
      0xCAFEBABEu; // => co-11: a value using every hex digit range A-F, 0-9

  unsigned char *bytes =
      (unsigned char
           *)&value; // => co-11: same reinterpret-cast technique as ex-01
  ...
  snprintf(
      reference, sizeof reference, "%08x",
  ...
  int match = strcmp(dump, reference) ==
              0; // => co-11: the assertion this example exists to check
  printf("%s\n",
