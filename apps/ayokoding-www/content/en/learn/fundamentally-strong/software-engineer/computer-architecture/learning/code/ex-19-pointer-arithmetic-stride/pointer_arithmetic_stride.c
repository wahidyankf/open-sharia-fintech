...
int main(void) { // program entry point
  ...
  struct Point *p_next =
      p + 1; // => co-17: pointer arithmetic -- C scales by sizeof(*p), not by 1

  uintptr_t addr_p = (uintptr_t)p; // => co-17: raw address of p, as an integer
  uintptr_t addr_next =
      (uintptr_t)p_next; // => co-17: raw address of p+1, as an integer
  uintptr_t stride =
      addr_next -
      addr_p; // => co-17: the ACTUAL byte distance the increment moved
  ...
  // C
  int correct =
      (stride == sizeof(struct Point)); // => co-17: direct equality check
