...
#define BIG_N                                                                  \
  (32L * 1024 *                                                                \
   1024) // => co-01: 32M floats = 128 MB per array -- far bigger than the
         // 4 MiB L2, so this is a genuine DRAM-bandwidth-bound workload
static double
now_secs(void) {     // defines now_secs(): helper function used by this example
  struct timespec t; // supporting statement for this example
  clock_gettime(CLOCK_MONOTONIC, &t); // calls clock_gettime(...)
  return (double)t.tv_sec +
         (double)t.tv_nsec / 1e9; // returns the computed result
}
...
calibrate_bandwidth_gbs(const float *src, float *dst,
  ...
  for (int t = 0; t < 7; t++) {   // => best-of-7: this is a shared dev machine,
    ...
    for (long i = 0; i < n; i++)
      dst[i] =
          src[i] *
          1.0000001f; // => co-01: 1 read + 1 write, independent per element
    double secs = now_secs() - t0; // declares secs
    if (best < 0 || secs < best)
      best = secs; // conditional check
  }
  double bytes =
      (double)n * 2.0 * sizeof(float); // => 1 read + 1 write, 4 bytes each
...
compute_calibration_gflops(double *out_gflops) { // calls __attribute__(...)
  ...
  float32x4_t mul = vdupq_n_f32(
      1.0000001f); // => co-23: fractionally above 1.0 -- bounded growth
  float32x4_t add =
      vdupq_n_f32(0.0000001f); // over COMPUTE_ITERS (e^10 ~= 22026, nowhere
                               // near float overflow)
  double t0 = now_secs();      // declares t0
  for (long i = 0; i < COMPUTE_ITERS;
       i++) {                         // loop header controlling the sweep below
  ...
  double secs = now_secs() - t0; // declares secs
  float32x4_t sum =
      vaddq_f32(vaddq_f32(acc0, acc1),
                vaddq_f32(acc2, acc3)); // => combine so nothing is dead
  float total = vgetq_lane_f32(sum, 0) + vgetq_lane_f32(sum, 1) +
                vgetq_lane_f32(sum, 2) +
                vgetq_lane_f32(sum, 3); // declares total
  double flops = (double)COMPUTE_ITERS * 4.0 /* chains */ * 4.0 /* lanes */ *
                 2.0 /* mul+add */;
...
__attribute__((noinline)) static double
kernel_a_triad(const float *a, const float *b, float *c,
               long n, // calls __attribute__(...)
  ...
  for (int t = 0; t < 7; t++) {   // => best-of-7, matching the bandwidth
    ...
    for (long i = 0; i < n; i++)
      c[i] = a[i] +
             (float)scalar * b[i]; // => co-01: 2 mul+add flops, 12 bytes moved
    double secs = now_secs() - t0; // declares secs
    if (best < 0 || secs < best)
      best = secs; // conditional check
  }
...
// calibration.
#define SMALL_N                                                                \
  4096 // => 4096 floats = 16 KB -- comfortably inside the 64 KiB L1d
#define SMALL_REPEATS                                                          \
  40000 // => co-01: enough outer repeats to make L1-resident compute dominate
        // the timing
__attribute__((noinline)) static double
kernel_b_compute(float *arr, long n, long repeats,
  ...
  for (long r = 0; r < repeats;
       r++) {                      // loop header controlling the sweep below
    for (long i = 0; i < n; i++) { // loop header controlling the sweep below
      arr[i] = arr[i] * m +
               a; // => co-01: 1 mul + 1 add per element, ALL from L1d after r=0
    }
  }
  double secs = now_secs() - t0;                    // declares secs
...
int main(void) { // program entry point
  float *big_a =
      malloc(sizeof(float) * BIG_N); // heap-allocates memory for big_a
  float *big_b =
      malloc(sizeof(float) * BIG_N); // heap-allocates memory for big_b
  float *big_c =
      malloc(sizeof(float) * BIG_N); // heap-allocates memory for big_c
  float *big_d =
      malloc(sizeof(float) *
             BIG_N); // => scratch destination for the bandwidth calibration
  float *small =
      malloc(sizeof(float) * SMALL_N); // heap-allocates memory for small
  ...
  for (long i = 0; i < SMALL_N; i++)
    small[i] = 1.0f + (float)(i % 7) *
                          0.001f; // loop header controlling the sweep below

  double bw_calibration_gbs = calibrate_bandwidth_gbs(
      big_a, big_d, BIG_N); // declares bw_calibration_gbs
  ...
  float compute_result =
      compute_calibration_gflops(&compute_gflops); // declares compute_result
  ...
  volatile float small_checksum =
      0.0f; // => same reasoning -- OBSERVE `small`'s post-kernel-B contents
  for (long i = 0; i < SMALL_N; i++)
  ...
  double predicted_a_gflops =
      ai_a * bw_calibration_gbs; // => roofline's bandwidth-bound prediction
  ...
  double a_predicted_ratio =
      a_gflops / predicted_a_gflops; // declares a_predicted_ratio
  double b_vs_compute_ratio =
      b_gflops / compute_gflops;             // declares b_vs_compute_ratio
  ...
  int pass =
      (a_predicted_ratio > 0.25 &&
       a_predicted_ratio < 4.0) && // A tracks its bandwidth-bound prediction
      (b_vs_compute_ratio >
       0.05) &&             // B is genuinely compute-bound, not stalled
