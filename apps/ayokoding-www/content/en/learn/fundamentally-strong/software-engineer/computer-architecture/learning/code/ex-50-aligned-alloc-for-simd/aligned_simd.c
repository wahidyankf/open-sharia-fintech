...
simd_sum(const float *p, int n, float salt) { // calls __attribute__(...)
  float32x4_t vsum = vdupq_n_f32(0.0f);       // declares vsum
  int i = 0;                                  // declares i
  for (; i + 4 <= n; i += 4) { // loop header controlling the sweep below
    float32x4_t v =
        vld1q_f32(&p[i]); // => co-23: vld1q_f32 handles unaligned addresses
  ...
  return sum +
         salt; // => co-25: salt distinguishes each trial's call (see ex-47/48)
}
...
int main(void) { // program entry point
  ...
  bytes = (bytes + ALIGN - 1) / ALIGN *
          ALIGN; // => co-16: round UP to the next multiple of ALIGN
  float *buf =
      aligned_alloc(ALIGN, bytes); // => co-16: C11's alignment-aware allocator
  if (!buf) {
  ...
  int is_aligned = (addr % ALIGN ==
                    0); // => co-16: verify the CONTRACT aligned_alloc promises
  printf("aligned_alloc(%d, %zu) returned %p, addr %% %d == %lu -> %s\n", ALIGN,
         bytes, (void *)buf, // prints a report line
         ALIGN, (unsigned long)(addr % ALIGN),
         is_aligned ? "PASS (properly aligned)"
                    : "FAIL"); // continues the printf(...) call above

  srand(41); // calls srand(...)
  for (size_t i = 0; i < raw_floats; i++)
    buf[i] = (float)(rand() % 100) *
             0.01f; // loop header controlling the sweep below

  float *aligned_view = buf; // => co-16: starts exactly at the aligned address
  float *misaligned_view =
      buf + 1; // => co-16: shifted by 1 float (4 B) -- deliberately
               //    NOT a multiple of 16 B (NEON's natural vector width)
  printf(
      "aligned_view addr %% 16 == %lu, misaligned_view addr %% 16 == %lu\n", // prints a report line
      (unsigned long)((uintptr_t)aligned_view % 16),
      (unsigned long)((uintptr_t)misaligned_view %
                      16)); // continues the printf(...) call above
  ...
  for (int t = 0; t < TRIALS;
    ...
    last_aligned =
        simd_sum(aligned_view, N, (float)t) - (float)t; // assigns last_aligned
    ...
    last_misaligned = simd_sum(misaligned_view, N, (float)t) -
                      (float)t; // assigns last_misaligned
  ...
  printf(
      "misaligned NEON sum: %.6f, %.4f s (SAME data, shifted by 1 element)\n",
  ...
  printf(                                             // prints a report line
      ...
      ratio > 1.05
          ...
          : "cost essentially the same as aligned loads", // continues the
                                                          // printf(...) call
                                                          // above
