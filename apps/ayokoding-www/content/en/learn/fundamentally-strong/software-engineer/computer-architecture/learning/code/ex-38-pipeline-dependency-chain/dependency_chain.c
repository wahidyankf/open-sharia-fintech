...
static long
independent_chains(long ops) { // defines independent_chains(): helper function
                               // used by this example
  volatile long x0 = 0, x1 = 0, x2 = 0,
                x3 = 0; // => co-22: four separate accumulators, no cross-deps
  for (long i = 0; i < ops / 4;
       i++) {    // => co-22: ops/4 iterations * 4 chains = same total OPS
...
int main(void) {             // program entry point
  ...
  long dep_result =
      dependent_chain(OPS);  // => co-20: the latency-bound baseline
  double t1 = now_seconds(); // declares t1
  long ilp_result = independent_chains(
      OPS); // => co-22: the throughput-bound, overlapped version
  ...
  double speedup =
      secs_dep /
      secs_ilp; // => co-22: how much overlap the 4 independent chains bought
  printf(
      "independent chains are %.2fx faster -> %s\n",
      speedup, // prints a report line
      (dep_result == ilp_result &&
       speedup > 1.1) // continues the printf(...) call above
  ...
  return (dep_result == ilp_result && speedup > 1.1)
             ? 0
