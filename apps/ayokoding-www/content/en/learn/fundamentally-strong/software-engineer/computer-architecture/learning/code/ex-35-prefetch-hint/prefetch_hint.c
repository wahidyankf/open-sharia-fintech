...
// measures.
static long sum_no_prefetch(
    const int *data, const int *perm,
...
static long sum_with_prefetch(const int *data, const int *perm,
  ...
  int i = 0;                           // declares i
  for (; i + BLOCK <= n;
       i += BLOCK) { // => co-03: process one BLOCK of elements at a time
    int fbase =
        i + BLOCK_AHEAD; // => co-03: this block's prefetch TARGETS start here
    if (fbase + BLOCK <= n) { // conditional check
      for (int k = 0; k < BLOCK;
           k++) { // loop header controlling the sweep below
  ...
  for (; i < n; i++)
    total +=
        data[perm[i]]; // => co-03: scalar tail for n not a multiple of BLOCK
  return total;        // returns the computed result
}
...
int main(void) { // program entry point
  ...
  int *data =
      malloc((size_t)N * sizeof(int)); // => the 400 MB array being summed
  int *perm = malloc(
      (size_t)N * sizeof(int)); // => the random visiting order over that array
  if (!data || !perm) {
  ...
  double ratio =
      best_no_pf / best_pf; // => co-03: >1.0 would mean prefetch helped
  printf("prefetch ratio: %.2fx, correctness: %s\n", ratio,
         (sum_a == sum_b) ? "identical sums"
                          : "MISMATCH -- BUG"); // prints a report line
  printf(                                       // prints a report line
      ...
      "already overlaps independent gather loads without help here).\n", // continues
                                                                         ...
                                                                         // above
      ratio > 1.03
          ? "the prefetch hint measurably helped"
          : "the prefetch hint did NOT measurably help"); // continues the
                                                          // printf(...) call
                                                          // above
  free(data); // releases data's heap memory
  free(perm); // releases perm's heap memory
  return (sum_a == sum_b) ? 0
                          : 1; // => correctness is the only hard gate here --
