...
int main(void) { // program entry point
  int *data = malloc(
      N * sizeof(int)); // => co-03: the array both access patterns sum over
  size_t *perm = malloc(
      N * sizeof(size_t)); // => co-03: a RANDOM permutation of indices 0..N-1
  if (!data || !perm) {
  ...
  for (size_t i = N - 1; i > 0;
       i--) { // => co-03: Fisher-Yates shuffle -- genuinely randomizes perm
    size_t j = (size_t)rand() %
               (i + 1); // => co-03: pick a random earlier-or-equal index
    size_t tmp = perm[i];
    perm[i] = perm[j];
  ...
  volatile long sink =
      0; // => co-03: volatile -- prevents the compiler from discarding the sums

  for (int r = 0; r < REPEATS;
       r++) { // => co-25: REPEATS runs of each pattern, keep the fastest
    ...
    for (size_t i = 0; i < N; i++)
      seq_sum += data[i]; // => co-03: SEQUENTIAL access -- next line often
                          // shares a cache line
    double seq_elapsed =
        now_seconds() - t0; // => co-25: elapsed time for this sequential pass
    if (seq_elapsed < seq_best)
      seq_best = seq_elapsed; // => co-25: keep the fastest sequential run
    sink +=
        seq_sum; // => co-03: accumulate into sink so the sum is never dead code
    ...
    for (size_t i = 0; i < N; i++)
      rand_sum += data[perm[i]]; // => co-03: RANDOM access -- almost always a
                                 // cache miss
    double rand_elapsed =
        now_seconds() - t1; // => co-25: elapsed time for this random pass
  ...
  double speedup =
      rand_best /
      seq_best; // => co-05: relative comparison -- portable across machines
  ...
  int correct =
      (speedup >
       1.2); // => co-05: a modest, honest threshold -- not an inflated claim
