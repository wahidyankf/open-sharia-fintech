...
#define NTHREADS                                                               \
  8 // => co-24: 8 threads -- this machine has 12 logical CPUs, real contention
...
static void *
worker(void *arg) { // defines worker(): helper function used by this example
  (void)arg;        // discards arg to silence an unused-variable warning
  for (long i = 0; i < ITERS_PER_THREAD;
...
int main(void) {               // program entry point
  ...
  long expected =
      (long)NTHREADS *
      ITERS_PER_THREAD; // => co-24: the mathematically correct total
  long actual =
      atomic_load(&counter); // => co-24: read the final value, atomically
  ...
  printf("no updates lost: %s -> %s\n",
         ...
         (actual == expected) ? "PASS (atomic counter exactly correct)"
