...
int main(void) {       // program entry point
  ...
  double diff =
      fabs(sum -
           target); // => co-14: the ABSOLUTE difference between the two doubles
  double epsilon =
      1e-9; // => co-14: a tolerance many orders of magnitude above 1 ULP here

  int strict_equal =
      (sum == target); // => co-14: repeats ex-08's failing strict comparison
  ...
  printf("strict (==)      : %s\n",
         strict_equal ? "true"
                      : "false"); // => co-14: expected false -- repeats ex-08
  printf("epsilon (<1e-9)  : %s\n",
         epsilon_equal ? "true"
                       : "false"); // => co-14: expected true -- the fix works
  ...
  int correct =
      (!strict_equal) &&
      epsilon_equal; // => co-14: both halves of the contrast must hold
  printf(
