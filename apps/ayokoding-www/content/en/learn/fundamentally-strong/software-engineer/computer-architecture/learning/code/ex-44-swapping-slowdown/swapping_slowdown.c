...
// irresponsible here.
#define SMALL_KIB                                                              \
  256L // => co-10: 256 KiB -- fits inside this machine's 4 MiB L2 easily
#define LARGE_MB                                                               \
  512L // => co-10: 512 MB -- far bigger than any cache level, still safely
       // RAM-resident
#define STEPS                                                                  \
  4000000L // => co-10: pointer-chase steps timed per working-set size
...
int main(void) { // program entry point
  ...
  int *small_buf = malloc((size_t)small_n *
                          sizeof(int)); // heap-allocates memory for small_buf
  int *large_buf = malloc((size_t)large_n *
                          sizeof(int)); // heap-allocates memory for large_buf
  ...
  double ns_small =
      chase_ns_per_access(small_buf, STEPS); // => co-10: cache-resident latency
  double ns_large =
      chase_ns_per_access(large_buf, STEPS); // => co-10: DRAM-resident latency
  ...
  printf(
      "DRAM/cache latency ratio: %.2fx -> %s\n", ratio, // prints a report line
      ratio > 2.0
          ? "PASS (measured, real gap between cache- and DRAM-resident latency)"
