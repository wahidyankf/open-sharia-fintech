...
__attribute__((noinline, optnone)) static unsigned long
sum_branchy(const int *scores, long n) { // calls __attribute__(...)
...
static void build_grade_table(
    void) { // defines build_grade_table(): helper function used by this example
  for (int s = 0; s <= 100; s++)
    grade_table[s] = grade_of(s); // loop header controlling the sweep below
}
__attribute__((noinline)) static unsigned long
sum_lookup(const int *scores, long n) { // calls __attribute__(...)
  unsigned long sum = 0;                // declares sum
  for (long i = 0; i < n; i++) { // loop header controlling the sweep below
    sum += (unsigned char)
        grade_table[scores[i]]; // => co-21: one array load, zero branches
  }
  return sum; // returns the computed result
}
...
static double
best_of(unsigned long (*fn)(const int *, long), const int *scores,
...
int main(void) {                         // program entry point
  ...
  double t_branchy =
      best_of(sum_branchy, scores, N, &sum_b); // declares t_branchy
  ...
  printf("speedup: %.2fx (PASS: identical classification, lookup faster on "
         ...
         speedup,
         (mismatches == 0 && sum_b == sum_l && speedup > 1.05)
             ? "PASS"
