...
sum_combined(const Combined *records, int n,
...
__attribute__((noinline)) static long
sum_split(const int *hot, int n, int repeats,
...
int main(void) { // program entry point
  Combined *combined = malloc(
      (size_t)N * sizeof(Combined)); // => co-17: 128 MB -- far bigger than L2
  int *hot = malloc((size_t)N *
                    sizeof(int)); // => co-17: 2 MB -- comfortably L2-resident
  if (!combined || !hot) {
