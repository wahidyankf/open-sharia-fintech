...
typedef struct {
  ...
  float *
      reading; // => HOT -- the only array this kernel's hot loop actually reads
...
static double average_reading_soa(const float *reading,
  ...
  double sum =
      0.0; // => same accumulator type, same arithmetic -- ONLY layout changed
...
static double best_of_seconds(double *out_result, const float *reading, int n) {
  double best =
      -1.0; // => co-25: track the FASTEST of TRIALS runs, not the average
  double result = 0.0;
  for (int t = 0; t < TRIALS;
       t++) {               // => co-25: re-run to smooth shared-machine noise
    struct timespec t0, t1; // => co-25: POSIX timespec -- seconds + nanoseconds
    clock_gettime(
        CLOCK_MONOTONIC,
        &t0); // => co-25: monotonic clock -- immune to wall-clock adjustment
    result = average_reading_soa(
        reading, n); // => the ONLY line whose timing this function measures
    clock_gettime(CLOCK_MONOTONIC, &t1);
    double secs = (t1.tv_sec - t0.tv_sec) +
                  (t1.tv_nsec - t0.tv_nsec) /
                      1e9; // => combine into one double-seconds value
    if (best < 0 || secs < best)
      best = secs; // => co-25: keep the best (least noisy) timing so far
  }
  *out_result =
      result; // => captured to prove cache.c and this file compute the SAME sum
  return best;
}

int main(void) {
  SensorSoA
      soa; // => co-17: four independent heap allocations, not one struct array
  soa.sensor_id =
      malloc(sizeof(int32_t) *
             (size_t)N); // => cold array 1 -- id, never read by the hot loop
  soa.reading = malloc(
      sizeof(float) *
      (size_t)N); // => HOT array -- the only one average_reading_soa touches
  soa.location = malloc(
      sizeof(*soa.location) *
      (size_t)N); // => cold array 2 -- 112 B label per record, never read
  soa.timestamp =
      malloc(sizeof(int64_t) *
             (size_t)N); // => cold array 3 -- never read by the hot loop
  if (!soa.sensor_id || !soa.reading || !soa.location ||
      !soa.timestamp) { // => co-25: fail loudly, never time a null pointer
  ...
  // speedup
  unsigned seed =
      7u; // => same PRNG seed as cache.c -- reproducible, matching dataset
  for (int i = 0; i < N; i++) {
    seed =
        seed * 1103515245u +
        12345u; // => same LCG step as cache.c, applied in the same loop order
    soa.sensor_id[i] =
        (int32_t)(i %
                  4096); // => same formula as cache.c's records[i].sensor_id
    soa.reading[i] = (float)(seed % 10000u) *
                     0.01f; // => same formula as cache.c's records[i].reading
                            // -- must match bit-for-bit
    for (int k = 0; k < 112; k++)
      soa.location[i][k] =
          0; // => same zero-fill as cache.c's memset on the AoS field
    soa.timestamp[i] =
        1700000000LL + i; // => same formula as cache.c's records[i].timestamp
  }

  size_t total_bytes =
      (sizeof(int32_t) + sizeof(float) + 112 + sizeof(int64_t)) *
      (size_t)N; // => co-17: same total byte count as cache.c's AoS -- only the
                 // arrangement differs
