...
static double average_grade_aos(const StudentRecord *records,
  ...
  for (int i = 0; i < n;
       i++) { // => co-17: one full 132 B record loaded per iteration...
...
static double average_grade_soa(const int *grade,
  ...
  long sum =
      0; // => same accumulator type, same arithmetic -- ONLY layout changed
  for (int i = 0; i < n;
       i++) { // => co-03: sequential dense reads, 32 useful ints per line
    sum += grade[i]; // updates sum
  }
  return (double)sum /
         n; // => must equal average_grade_aos's result bit-for-bit (both are
...
static double best_of_seconds(double (*fn)(const void *, int), const void *data,
  ...
  double best =
      -1.0; // => co-25: best-of-N methodology -- report the fastest clean run,
  for (int t = 0; t < TRIALS;
       t++) { // not a single noisy sample, per this topic's measurement rule
    struct timespec t0, t1; // supporting statement for this example
    clock_gettime(CLOCK_MONOTONIC, &t0); // calls clock_gettime(...)
    volatile double result = fn(
        data,
...
int main(void) { // program entry point
  ...
  StudentRecord *records =
      malloc(sizeof(StudentRecord) * N); // => co-17: 132 B * 4M ~= 528 MB AoS
  if (!records) {
    fprintf(stderr, "alloc failed\n");
    return 1;
  } // prints a report line
  unsigned seed =
      12345u; // => co-25: fixed seed -- reproducible dataset every run
  for (int i = 0; i < N; i++) { // loop header controlling the sweep below
    seed = seed * 1103515245u +
           12345u; // => a tiny deterministic LCG, no <stdlib.h> rand() state
    records[i].grade = (int)(seed % 101u); // grades 0..100
    memset(
        records[i].name, 'x',
        sizeof(records[i].name)); // => cold bytes: realistic non-zero payload
    records[i].enrolled_year =
        2020 + (int)(seed % 6u); // cold field, never read by the kernel
  }

  double aos_secs = best_of_seconds(
      wrap_aos, records, N); // => co-25: profile step -- measure the slow path
  double aos_avg =
      average_grade_aos(records, N); // => capture the actual numeric result too

  // Step 2 ("fix its layout"): extract just the hot field into a dense SoA
  // buffer.
  int *grade =
      malloc(sizeof(int) *
             N); // => co-17: 4 B * 4M = 16 MB -- 33x smaller footprint than AoS
  if (!grade) {
    fprintf(stderr, "alloc failed\n");
    return 1;
  } // prints a report line
  ...
  // stopwatch.
  double soa_secs = best_of_seconds(
      wrap_soa, grade, N); // => co-25: re-profile step -- measure the fix
  ...
  printf("dataset: N=%d records, sizeof(StudentRecord)=%zu bytes (%.1f MB "
         ...
         (double)(sizeof(StudentRecord) * N) /
             (1024.0 * 1024.0)); // continues the printf(...) call above
  printf(
      "step 1 (profile)  AoS average_grade: best of %d = %.4f s, result=%.6f\n",
