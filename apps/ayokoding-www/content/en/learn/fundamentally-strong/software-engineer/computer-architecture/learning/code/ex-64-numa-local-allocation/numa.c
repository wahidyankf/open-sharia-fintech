...
static void touch_local(
    int *buf,
    long n) { // defines touch_local(): helper function used by this example
  for (long i = 0; i < n; i++)
    buf[i] =
        (int)(i & 0xff); // => co-01: first-touch -- allocates the physical page
}
...
static void *touch_thread(
    void *arg) { // defines touch_thread(): helper function used by this example
...
time_read(const int *buf,
  ...
  for (int t = 0; t < TRIALS; t++) { // loop header controlling the sweep below
    ...
    volatile long sum =
        0; // => volatile: stop the optimizer from deleting the "unused" read
...
int main(void) { // program entry point
  ...
  int *buf_local =
      malloc(sizeof(int) * N); // heap-allocates memory for buf_local
  ...
  // matters here at all.
  int *buf_remote =
      malloc(sizeof(int) * N); // heap-allocates memory for buf_remote
  ...
  printf(
      ...
      pass
          ? "PASS"
          : "FAIL (unexpected gap on UMA hardware -- investigate)"); // prints a
                                                                     // report
                                                                     // line
