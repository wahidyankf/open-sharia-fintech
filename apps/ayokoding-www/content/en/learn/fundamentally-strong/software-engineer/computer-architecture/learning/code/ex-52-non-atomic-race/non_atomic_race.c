...
#define NTHREADS                                                               \
  8 // => co-24: SAME thread count as ex-51, for a direct, fair comparison
#define ITERS_PER_THREAD 2000000L // => co-24: SAME iteration count as ex-51

volatile long counter =
    0; // => co-24: PLAIN read-modify-write, no atomicity -- volatile only
       // forces
...
int main(void) {               // program entry point
  ...
  printf(
      "undercounts (a real, reproducible race): %s -> %s\n",
