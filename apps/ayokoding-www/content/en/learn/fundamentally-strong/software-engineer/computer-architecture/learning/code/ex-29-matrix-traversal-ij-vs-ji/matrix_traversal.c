...
static double now_seconds(
  ...
  return (double)ts.tv_sec +
         (double)ts.tv_nsec / 1e9; // => co-25: combined into one double-seconds
                                   // value for subtraction
}

static long sum_row_major(
    int (*m)[N]) { // => co-17: m[i][j] is the pointer-to-array-of-N-ints layout
                   // C actually uses
  ...
  long total =
      0; // => co-03: accumulator -- summed in the SAME order memory is laid out
  for (int i = 0; i < N;
       i++) { // => co-03: outer loop over rows -- each row is N contiguous ints
    for (int j = 0; j < N;
         j++) { // => co-03: inner loop walks j sequentially WITHIN one row
...
static long
sum_col_major(int (*m)[N]) { // => co-17: SAME matrix, SAME memory layout --
  ...
  // hugely
  long total =
      0; // => co-17: accumulator -- summed in COLUMN order, against the grain
  for (int j = 0; j < N; j++) { // => co-17: outer loop over columns now
    for (int i = 0; i < N;
         i++) { // => co-17: inner loop walks i -- each step jumps N*4 = 16 KiB
...
int main(void) { // => co-25: single self-contained process -- both traversals
                 // run in it
  int (*m)[N] =
      malloc(sizeof(int[N][N])); // => co-17: one 64 MiB row-major allocation,
  ...
  for (int i = 0; i < N;
       i++) // => co-01: fill once so page faults don't skew either timed pass
    for (int j = 0; j < N;
         j++) // => co-01: touches every one of the 16M ints exactly once
  ...
  for (int r = 0; r < REPEATS; r++) { // => co-25: re-run to confirm the result
    ...
    double t0 =
        now_seconds(); // => co-25: time ONLY the traversal, not the setup above
    row_sum = sum_row_major(
        m); // => co-03: the cache-friendly, sequential-access traversal
    double t1 =
        now_seconds(); // => co-25: stop the clock for the row-major pass
    if (t1 - t0 < best_row)
      best_row =
          t1 -
          t0; // => co-25: keep the best (least noisy) row-major timing so far

    double t2 =
        now_seconds(); // => co-25: start the clock for the column-major pass
    col_sum =
        sum_col_major(m); // => co-17: the SAME matrix, strided-access traversal
    double t3 =
        now_seconds(); // => co-25: stop the clock for the column-major pass
    if (t3 - t2 < best_col)
  ...
  printf(
      "row-major is %.2fx faster -> %s\n",
      speedup, // => co-25: prints the ratio the reader should verify against
               // below
      (row_sum == col_sum && speedup > 1.2)
          ? // => co-03: correctness (equal sums) AND speed both gate the PASS
  ...
  return (row_sum == col_sum && speedup > 1.2)
             ? 0
