...
static float fast_inv_sqrt(
    float x) { // defines fast_inv_sqrt(): helper function used by this example
  ...
  bits = MAGIC -
         (bits >>
          1); // => co-13: the bit-hack -- an approximate log-domain halving
  float y;    // declares y
  memcpy(&y, &bits, sizeof(y)); // => co-13: reinterpret the hacked bits back as
                                // a float -- crude guess
  y = y * (1.5f -
           (x_half * y *
            y)); // => one Newton-Raphson iteration: y_new = y*(1.5 - x/2*y^2),
...
int main(void) { // program entry point
  ...
  float tests[] = {1.0f,   2.0f,  4.0f,  10.0f,
                   100.0f, 0.25f, 0.01f, 1000.0f}; // declares tests
  ...
  for (int i = 0; i < n; i++) {      // loop header controlling the sweep below
    ...
    double rel_error = fabs((double)approx - (double)exact) /
                       (double)exact; // declares rel_error
    if (rel_error > max_rel_error)
      max_rel_error = rel_error; // conditional check
    printf("%-10.4f %-15.6f %-16.6f %.6f%%\n", (double)x, (double)approx,
           (double)exact, rel_error * 100.0); // prints a report line
  }
