...
#define N_SIZES                                                                \
  12 // => co-01: number of working-set sizes swept, 16 KiB .. 32 MiB
#define STEPS                                                                  \
  4000000L // => co-05: pointer-chase steps timed per working-set size
...
static void sattolo_shuffle(int *arr,
  ...
  for (int i = n - 1; i > 0; i--) { // => co-05: Sattolo's variant of
    ...
    int j = rand() %
            i; // => co-05: j strictly < i (never swaps an element with itself)
    int tmp = arr[i];
    arr[i] = arr[j];
    arr[j] =
        tmp; // => co-05: swap -- builds the single-cycle permutation in place
...
static double chase_ns_per_access(int *arr, int n, long steps) {
  volatile int idx =
      0; // => co-25: volatile -- forces every dependent load to really happen
  double t0 = now_seconds(); // => co-25: start the clock right before the timed
                             // dependent-load loop
  for (long i = 0; i < steps;
       i++) {       // => co-05: STEPS sequential, data-dependent loads
  ...
  (void)
      n; // => co-25: n only used by the caller for the printed table, not here
...
int main(void) {
  ...
  size_t sizes_kib[N_SIZES] = {
      16,   32,   64,   128,  256,   512,
      1024, 2048, 4096, 8192, 16384, 32768}; // => co-01: KiB working sets
  int max_elems =
      (int)((sizes_kib[N_SIZES - 1] * 1024UL) /
            sizeof(int)); // => co-01: largest size drives the one shared buffer
  int *buf = malloc((size_t)max_elems *
                    sizeof(int)); // => co-01: one 32 MiB buffer, reused
  ...
  double prev_ns =
      0.0; // => co-05: previous size's latency, for computing the jump ratio
  int printed_first =
      0; // => co-05: skip the ratio print on the very first (smallest) size
  for (int s = 0; s < N_SIZES;
       s++) { // => co-01: one timed pointer-chase per working-set size
    int n = (int)((sizes_kib[s] * 1024UL) /
                  sizeof(int)); // => co-01: element count for THIS size, within
                                // the shared buffer
    sattolo_shuffle(buf, n);    // => co-05: a fresh single-cycle permutation
                                // confined to the first n ints
    double ns = chase_ns_per_access(
        buf, n,
