...
static double best_of_seconds(double *out_result, const SensorRecord *data,
                              int n) {
  double best = -1.0;
  double result = 0.0;
...
int main(void) {
  static_assert(sizeof(SensorRecord) == 128,
                "SensorRecord must be exactly one 128 B cache line");
  ...
  unsigned seed =
      7u; // => deterministic PRNG seed -- reproducible dataset every run
  for (int i = 0; i < N; i++) {
    seed = seed * 1103515245u + 12345u;
    records[i].sensor_id = (int32_t)(i % 4096);
    records[i].reading =
        (float)(seed % 10000u) * 0.01f; // => reading in [0.00, 99.99)
  ...
  printf("dataset: N=%d records, sizeof(SensorRecord)=%zu bytes (%.1f MB total "
         "AoS)\n",
         N, sizeof(SensorRecord),
         (double)(sizeof(SensorRecord) * (size_t)N) / 1e6);

  double result;
  double secs = best_of_seconds(&result, records, N);
  printf(
      "AoS (cache-hostile) average_reading: best of %d = %.4f s, result=%.6f\n",
      TRIALS, secs, result);
