...
static void update_aos(
    ParticleAoS *p, int n,
    float dt) { // defines update_aos(): helper function used by this example
#pragma clang loop vectorize(disable)                                          \
    interleave(disable) // supporting statement for this example
  for (int i = 0; i < n;
       i++) { // => co-17: each iteration loads a full 28 B particle
    p[i].x +=
        p[i].vx * dt; // to update 3 of its 7 floats -- the rest rides along
...
static void update_soa_simd(ParticleSoA *p, int n,
                            float dt) { // defines update_soa_simd(): helper
                                        // function used by this example
  float32x4_t vdt =
      vdupq_n_f32(dt); // => co-23: broadcast dt into all 4 lanes once
  int i = 0;           // declares i
  for (; i + 4 <= n;
       i += 4) { // => co-23: process 4 particles per axis per iteration
...
static void
run_aos(void *ctx, int n, float dt,
...
static void run_soa_simd(
    void *ctx, int n, float dt,
...
int main(void) { // program entry point
  ParticleAoS *aos =
      malloc(sizeof(ParticleAoS) * N); // heap-allocates memory for aos
  ParticleSoA soa_scalar = {
      malloc(sizeof(float) * N), malloc(sizeof(float) * N),
      malloc(sizeof(float) * N), // declares soa_scalar
      malloc(sizeof(float) * N), malloc(sizeof(float) * N),
      malloc(sizeof(float) * N)}; // calls malloc(...)
  ParticleSoA soa_simd = {malloc(sizeof(float) * N), malloc(sizeof(float) * N),
                          malloc(sizeof(float) * N), // declares soa_simd
                          malloc(sizeof(float) * N), malloc(sizeof(float) * N),
                          malloc(sizeof(float) * N)}; // calls malloc(...)
  if (!aos || !soa_scalar.x || !soa_simd.x) {
  ...
  for (int i = 0; i < N; i++) { // loop header controlling the sweep below
    ...
    aos[i].x = aos[i].y = aos[i].z =
        x; // => identical starting state across all 3 variants
    aos[i].vx = aos[i].vy = aos[i].vz = vx; // assigns aos[i].vx
    aos[i].mass = 1.0f;                     // assigns aos[i].mass
    soa_scalar.x[i] = soa_scalar.y[i] = soa_scalar.z[i] =
        x; // assigns soa_scalar.x[i]
    soa_scalar.vx[i] = soa_scalar.vy[i] = soa_scalar.vz[i] =
        vx; // assigns soa_scalar.vx[i]
    soa_simd.x[i] = soa_simd.y[i] = soa_simd.z[i] = x; // assigns soa_simd.x[i]
    soa_simd.vx[i] = soa_simd.vy[i] = soa_simd.vz[i] =
        vx; // assigns soa_simd.vx[i]
  }
  ...
  int pass = (max_diff < 1e-3) && (t_simd < t_soa) &&
             (t_simd < t_aos); // declares pass
