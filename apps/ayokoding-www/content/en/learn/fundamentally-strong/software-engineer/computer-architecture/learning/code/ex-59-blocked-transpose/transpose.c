...
// column-by-column
static void
transpose_blocked(const int *src, int *dst, int n,
                  int block) { // defines transpose_blocked(): helper function
                               // used by this example
  for (int ii = 0; ii < n; ii += block) { // => co-03: outer loop over tile rows
    for (int jj = 0; jj < n;
         jj += block) { // => co-03: outer loop over tile columns
      int i_max = ii + block < n
                      ? ii + block
                      : n; // => clamp the last tile if n isn't a multiple
      int j_max = jj + block < n ? jj + block : n; // declares j_max
      for (int i = ii; i < i_max;
           i++) { // => co-03: inner loops stay INSIDE one tile --
        for (int j = jj; j < j_max;
             j++) { // both src and dst touches land in the same
          dst[j * n + i] =
              src[i * n + j]; // small hot region for the whole tile's work
        }
      }
    }
  }
...
static double best_of(void (*fn)(const int *, int *, int, int), const int *src,
                      int *dst, int n,         // declares function pointer fn
  ...
  for (int t = 0; t < trials; t++) { // loop header controlling the sweep below
    ...
    double secs = (t1.tv_sec - t0.tv_sec) +
                  (t1.tv_nsec - t0.tv_nsec) / 1e9; // declares secs
    if (best < 0 || secs < best)
...
int main(void) { // program entry point
  int *src =
      malloc(sizeof(int) * (size_t)N * N); // heap-allocates memory for src
  int *dst_naive = malloc(sizeof(int) * (size_t)N *
                          N); // heap-allocates memory for dst_naive
  int *dst_blocked = malloc(sizeof(int) * (size_t)N *
                            N); // heap-allocates memory for dst_blocked
  ...
  double naive_secs =
      best_of(naive_wrap, src, dst_naive, N, BLOCK, 3); // declares naive_secs
  double blocked_secs = best_of(transpose_blocked, src, dst_blocked, N, BLOCK,
                                3); // declares blocked_secs

  int mismatches =
      0; // => co-03: correctness check -- both outputs must be IDENTICAL
  ...
  for (int i = 0; i < N;
       i +=
       511) { // sample a handful of rows, not all 2048 (keeps output short)
    for (int j = 0; j < N;
         j += 511) { // loop header controlling the sweep below
  ...
  printf("matrix: %dx%d int (%.1f MB), block=%d (%.1f KB per tile)\n", N,
         ...
         (double)(sizeof(int) * BLOCK * BLOCK) /
             1024.0); // continues the printf(...) call above
  ...
  printf(
      "transpose property dst[j][i]==src[i][j] sampled-verified: %s\n", // prints
                                                                        // a
                                                                        // report
                                                                        // line
      property_ok ? "yes"
                  : "NO -- BUG"); // continues the printf(...) call above
  ...
  printf("speedup: %.2fx (PASS: blocked faster + correct) -> %s\n",
         ...
         (mismatches == 0 && property_ok && speedup > 1.05)
             ? "PASS"
