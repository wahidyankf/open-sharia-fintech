...
static long minflt_now(
    void) { // => co-10: shared helper -- reads the current minor-fault count
...
int main(void) { // program entry point
  long page_size = sysconf(
      _SC_PAGESIZE); // => co-08: THIS machine's real page size (16384 B,
                     //    confirmed against `vm_stat`'s reported page size)
  size_t map_bytes =
      (size_t)MAP_MB * 1024 * 1024; // => co-10: total mapping size, both cases
  long npages =
      (long)(map_bytes / (size_t)page_size); // => co-10: pages about to be
  ...
  char *fbase =
      (char *)file_map; //    deleting this "unused" first-touch read loop
  for (long p = 0; p < npages; p++) { // loop header controlling the sweep below
    fbase[p * page_size] =
        (char)(p & 0xFF);        // => co-10: FIRST touch of this page
  ...
  // ----
  void *anon_map = mmap(NULL, map_bytes, PROT_READ | PROT_WRITE,
                        MAP_ANON | MAP_PRIVATE, -1, 0); // => co-08
  ...
  for (long p = 0; p < npages; p++) { // loop header controlling the sweep below
    abase[p * page_size] =
        (char)(p & 0xFF); // => co-10: FIRST touch of this page (anonymous)
  ...
  printf(
      "anonymous delta >= npages (first touch faults, as expected): %s\n", // prints
                                                                           // a
                                                                           // report
                                                                           // line
      delta_anon >= npages
          ? "yes -> PASS"
          : "no -> FAIL"); // continues the printf(...) call above
  printf(                  // prints a report line
      ...
      "it here)\n", // continues the printf(...) call above
      delta_file >= npages ? "yes"
                           : "no"); // continues the printf(...) call above

  munmap(file_map, map_bytes); // calls munmap(...)
  munmap(anon_map, map_bytes); // calls munmap(...)
  close(fd);                   // calls close(...)
  return delta_anon >= npages
             ? 0
             : 1; // => co-10: the anonymous case is this example's hard gate
}
