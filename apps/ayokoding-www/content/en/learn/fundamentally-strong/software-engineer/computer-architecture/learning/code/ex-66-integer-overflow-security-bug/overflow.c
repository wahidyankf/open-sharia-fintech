...
static void *alloc_records_vulnerable(
    uint32_t n, uint32_t elem_size,
...
static void *
alloc_records_checked(uint32_t n, uint32_t elem_size, uint32_t *out_total,
...
int main(void) { // program entry point
  ...
  uint32_t elem_size =
      (uint32_t)sizeof(Record); // => 64, a fixed, trusted constant
  ...
  size_t correct_bytes =
      (size_t)n_evil * (size_t)elem_size; // declares correct_bytes

  printf("scenario: n=%u records of %u bytes each (attacker-controlled n)\n",
         n_evil, elem_size); // prints a report line
  printf(
      "correct byte count (64-bit size_t, no overflow): %zu bytes (%.2f MB)\n",
      correct_bytes, // prints a report line
      (double)correct_bytes /
  ...
  void *checked_buf = alloc_records_checked(
      n_evil, elem_size, &checked_total, &overflowed); // declares checked_buf
  printf("\nchecked path (__builtin_mul_overflow): overflow detected=%s, "
         "allocation=%s\n", // prints a report line
         overflowed ? "yes" : "no",
         checked_buf ? "proceeded"
                     : "REFUSED"); // continues the printf(...) call above
  ...
  int pass =
      (vuln_total ==
       64) && // the exact predicted wrapped value (see comment above)
      (overflowed == 1) &&
      (checked_buf == NULL) && // checked path correctly REJECTED the bad case
      (overflowed_ok == 0) && (ok_buf != NULL) &&
      (ok_total == n_ok * elem_size); // and allowed the good one
