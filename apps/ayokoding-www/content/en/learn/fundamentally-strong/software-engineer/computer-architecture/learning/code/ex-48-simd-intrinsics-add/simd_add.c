...
scalar_add(const float *a, const float *b, float *c,
...
__attribute__((noinline)) static float
simd_add(const float *a, const float *b, float *c,
         int n,                // calls __attribute__(...)
         float salt) {         // supporting statement for this example
  int i = 0;                   // declares i
...
int main(void) {                                 // program entry point
  ...
  printf("mismatches: %d, speedup: %.2fx -> %s\n", mismatches,
         ...
         (mismatches == 0 && speedup > 1.1)
             ? "PASS (identical output, NEON measurably faster)"
  ...
  return (mismatches == 0 && speedup > 1.1) ? 0
