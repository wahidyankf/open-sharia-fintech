...
#define INTS_PER_PAGE                                                          \
  (PAGE_BYTES / sizeof(int)) // => co-08: 4096 ints span exactly one page
#define NPAGES_BIG                                                             \
...
static long touch_pages(const int *buf, const int *page_order, int npages,
  ...
  for (long i = 0; i < accesses;
    ...
    int p =
        page_order[i % npages]; // => co-09: cycles through npages repeatedly --
    total +=
        buf[(size_t)p *
            INTS_PER_PAGE]; //    with NPAGES_BIG, the TLB never stays warm;
...
int main(void) { // program entry point
  ...
  int *buf_big = malloc((size_t)NPAGES_BIG * INTS_PER_PAGE *
                        sizeof(int)); // => ~1.6 GB, safely within RAM
  int *buf_small =
      malloc((size_t)NPAGES_SMALL * INTS_PER_PAGE * sizeof(int)); // => ~128 KB
  int *order_big = malloc((size_t)NPAGES_BIG *
                          sizeof(int)); // heap-allocates memory for order_big
  int *order_small =
      malloc((size_t)NPAGES_SMALL *
             sizeof(int)); // heap-allocates memory for order_small
  if (!buf_big || !buf_small || !order_big ||
      !order_small) {                  // conditional check
    fprintf(stderr, "alloc failed\n"); // prints a report line
    return 1;                          // returns the computed result
  }
  for (long i = 0; i < (long)NPAGES_BIG * INTS_PER_PAGE; i++)
    buf_big[i] = (int)(i % 97); // loop header controlling the sweep below
  for (long i = 0; i < (long)NPAGES_SMALL * INTS_PER_PAGE; i++)
  ...
  long sum_big =
      touch_pages(buf_big, order_big, NPAGES_BIG, ACCESSES); // declares sum_big
  ...
  double ratio =
      secs_big /
      secs_small; // => co-09: how much slower the many-page pattern is
  printf(
      "many-page/compact ratio: %.2fx -> %s\n", ratio, // prints a report line
      ratio > 1.3
          ? "PASS (TLB/page pressure measurably dominates the many-page case)"
