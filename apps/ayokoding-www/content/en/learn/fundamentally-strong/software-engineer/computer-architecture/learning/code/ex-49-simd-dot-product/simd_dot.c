...
scalar_dot(const float *a, const float *b, int n,
...
__attribute__((noinline)) static float
simd_dot(const float *a, const float *b, int n,
         float salt) { // calls __attribute__(...)
  float32x4_t vsum =
      vdupq_n_f32(0.0f); // => co-23: 4 independent partial-sum lanes, all zero
  int i = 0;             // declares i
  for (; i + 4 <= n; i += 4) { // => co-23: process 4 elements per iteration
    ...
    vsum = vmlaq_f32(
        vsum, va, vb); // => co-22: vsum += a*b for all 4 lanes, ONE instruction
  }
  float sum =
      vaddvq_f32(vsum); // => co-23: horizontal reduction -- sums all 4 lanes
  for (; i < n; i++)
...
int main(void) {                                // program entry point
  ...
  for (int i = 0; i < N;
       i++) { // => small values -- keeps the running sum well-scaled
  ...
  for (int t = 0; t < TRIALS;
    ...
    last_scalar = scalar_dot(a, b, N, (float)t) -
                  (float)t;    // => co-25: salt added then removed -- see ex-48
  ...
  printf("speedup: %.2fx -> %s\n", speedup, // prints a report line
         (rel_diff < 1e-2 && speedup > 1.1)
             ? "PASS (correct within epsilon, NEON measurably faster)" // continues
                                                                       // the
                                                                       // printf(...)
                                                                       // call
                                                                       // above
  ...
  return (rel_diff < 1e-2 && speedup > 1.1) ? 0
