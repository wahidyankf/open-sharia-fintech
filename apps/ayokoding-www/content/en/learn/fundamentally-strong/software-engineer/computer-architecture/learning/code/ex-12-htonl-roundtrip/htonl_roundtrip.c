...
int main(void) {                     // program entry point
  ...
  unsigned char *host_bytes =
      (unsigned char
           *)&host_value; // => co-15: raw storage bytes of the HOST-order value
  unsigned char *wire_bytes =
      (unsigned char
           *)&wire_value; // => co-15: raw storage bytes of the WIRE-order value
  ...
  int bytes_differ = // => co-15: the wire representation is NOT the host one
      host_bytes[0] != wire_bytes[0] ||
      host_bytes[1] != wire_bytes[1] || // supporting statement for this example
      host_bytes[2] != wire_bytes[2] ||
      host_bytes[3] != wire_bytes[3]; // supporting statement for this example
  int correct = roundtrip_ok &&
                bytes_differ; // => co-15: both halves of the claim must hold
  printf("%s\n",
