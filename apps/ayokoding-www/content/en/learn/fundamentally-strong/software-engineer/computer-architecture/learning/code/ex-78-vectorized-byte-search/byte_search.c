...
#define BUF_SIZE                                                               \
  (64L * 1024 *                                                                \
   1024) // => co-23: 64 MB -- large enough that a byte-at-a-time scalar
...
scalar_find(const unsigned char *buf, long n,
...
__attribute__((noinline)) static long
neon_find(const unsigned char *buf, long n,
          unsigned char target) { // calls __attribute__(...)
  uint8x16_t vtarget = vdupq_n_u8(
      target); // => co-23: broadcast the target byte into all 16 lanes
  long i = 0;  // declares i
  for (; i + 16 <= n; i += 16) { // loop header controlling the sweep below
    uint8x16_t chunk =
        vld1q_u8(buf + i); // => co-23: load 16 bytes in ONE instruction
    uint8x16_t cmp = vceqq_u8(
        chunk, vtarget); // => co-23: 16 parallel byte-equality comparisons
    if (vmaxvq_u8(cmp) !=
        0) { // => co-23: "any lane nonzero?" reduction, ONE instruction
      for (int j = 0; j < 16;
           j++) { // => co-23: rare slow path -- pin down the exact byte
        if (buf[i + j] == target)
...
int main(void) {                         // program entry point
  ...
  unsigned char target =
      0xFF; // => co-23: a byte value that appears NOWHERE except
  long plant_at =
      BUF_SIZE - 100;     // ...one planted position near the very END --
  ...
  // real number).
  long (*volatile scalar_fn)(const unsigned char *, long, unsigned char) =
      scalar_find; // declares function pointer scalar_fn
  long (*volatile neon_fn)(const unsigned char *, long, unsigned char) =
      neon_find; // declares function pointer neon_fn
  ...
  for (int trial = 0; trial < 5;
       trial++) { // loop header controlling the sweep below
  ...
  int correct = (found_scalar == plant_at) &&
                (found_neon == plant_at);   // declares correct
