...
int main(void) {
  ...
  int is_little_endian =
      (probe.as_bytes[0] ==
       0x01); // => co-15: true only if byte[0] (lowest address) holds the LSB
  int is_big_endian =
      (probe.as_bytes[0] ==
       0x00); // => co-15: true only if byte[0] holds the MOST-significant byte
  ...
  printf("detected order   = %s\n", // => co-15: the human-readable verdict
         is_little_endian ? "little-endian"
                          : (is_big_endian ? "big-endian" : "unknown"));
  ...
  // (0x01)
  int correct =
      is_little_endian; // => co-15: the expected verdict on THIS machine
  printf(
      "%s\n",
