...
#define WIRE_BYTES                                                             \
  7 // => 4 + 2 + 1 -- the record's TRUE information content, with zero padding
...
static void serialize_be(Record r,
  ...
  buf[0] =
      (uint8_t)(r.id >>
                24); // => co-15: most-significant byte of id, written FIRST
  buf[1] = (uint8_t)(r.id >> 16); // assigns buf[1]
  buf[2] = (uint8_t)(r.id >> 8);  // assigns buf[2]
  buf[3] =
      (uint8_t)(r.id); // => co-15: least-significant byte of id, written LAST
  uint16_t s =
      (uint16_t)
          r.score; // => co-15/co-11: reinterpret the signed value's bit pattern
...
static Record
deserialize_be(const uint8_t *buf) { // defines deserialize_be(): helper
                                     // function used by this example
  Record r;                          // declares r
  ...
  r.id = ((uint32_t)buf[0] << 24) | ((uint32_t)buf[1] << 16) |
         ((uint32_t)buf[2] << 8) | (uint32_t)buf[3]; // assigns r.id
  uint16_t s =
      (uint16_t)(((uint16_t)buf[4] << 8) | (uint16_t)buf[5]); // declares s
...
static void serialize_le(Record r,
                         uint8_t *buf) { // defines serialize_le(): helper
                                         // function used by this example
  buf[0] =
      (uint8_t)(r.id); // => co-15: least-significant byte of id, written FIRST
  buf[1] = (uint8_t)(r.id >> 8);  // assigns buf[1]
  buf[2] = (uint8_t)(r.id >> 16); // assigns buf[2]
  buf[3] = (uint8_t)(r.id >>
                     24); // => co-15: most-significant byte of id, written LAST
...
static Record
deserialize_le(const uint8_t *buf) { // defines deserialize_le(): helper
                                     // function used by this example
  Record r;                          // declares r
  r.id = (uint32_t)buf[0] | ((uint32_t)buf[1] << 8) | ((uint32_t)buf[2] << 16) |
         ((uint32_t)buf[3] << 24); // assigns r.id
  uint16_t s =
      (uint16_t)((uint16_t)buf[4] | ((uint16_t)buf[5] << 8)); // declares s
  r.score = (int16_t)s;                                       // assigns r.score
  r.flag = buf[6];                                            // assigns r.flag
  return r; // returns the computed result
}
...
static int records_equal(
  ...
  return a.id == b.id && a.score == b.score &&
         a.flag == b.flag; // => field-by-field, not memcmp of
...
int main(void) { // program entry point
  Record original = {
      0x01020304u, -12345,
      0xAB}; // => co-15: id's bytes are all distinct, easy to eyeball
  ...
  printf("original: id=0x%08X score=%d flag=0x%02X\n", original.id,
         original.score, original.flag); // prints a report line
  printf(
      "BE wire bytes: %02X %02X %02X %02X %02X %02X %02X  (MSB of id FIRST)\n",
      wire_be[0], wire_be[1], // prints a report line
      wire_be[2], wire_be[3], wire_be[4], wire_be[5],
      wire_be[6]); // continues the printf(...) call above
  printf(
      "LE wire bytes: %02X %02X %02X %02X %02X %02X %02X  (LSB of id FIRST)\n",
  ...
  int be_differs_from_le =
      memcmp(wire_be, wire_le, WIRE_BYTES) != 0; // declares be_differs_from_le

  Record round_be = deserialize_be(wire_be); // declares round_be
  Record round_le = deserialize_le(wire_le); // declares round_le
  int be_roundtrip_ok =
      records_equal(original, round_be); // declares be_roundtrip_ok
  int le_roundtrip_ok =
      records_equal(original, round_le); // declares le_roundtrip_ok
  ...
  int pass =
      be_roundtrip_ok && le_roundtrip_ok && be_differs_from_le; // declares pass
