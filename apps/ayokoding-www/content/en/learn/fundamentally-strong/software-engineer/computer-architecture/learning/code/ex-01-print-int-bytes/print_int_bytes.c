...
int main(void) { // => co-11: single-file, no args -- fully self-contained
  int32_t value =
      0x11223344; // => co-11: a value with 4 visibly DIFFERENT byte values
  // => co-11: 0x11, 0x22, 0x33, 0x44 -- chosen so byte ORDER is unambiguous in
  // the printed dump

  unsigned char *bytes =
      (unsigned char
           *)&value; // => co-11: reinterpret the 4-byte int as 4 raw bytes
  ...
  int little_endian_order = // => co-15: the assertion this program exists to
      ...
      (bytes[0] == 0x44) &&
      (bytes[1] ==
       0x33) && // => co-15: 0x44 first, 0x33 second (reversed order)
      (bytes[2] == 0x22) &&
      (bytes[3] == 0x11); // => co-15: 0x22 third, 0x11 last -- fully reversed
  ...
  return little_endian_order
             ? 0
             : 1; // => co-11: nonzero exit signals a failed assertion
}
