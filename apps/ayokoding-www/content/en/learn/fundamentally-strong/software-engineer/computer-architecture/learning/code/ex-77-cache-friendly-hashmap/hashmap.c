...
static unsigned
hash_u32(unsigned x) { // => co-03: cheap multiplicative hash (Knuth)
  x ^= x >> 16;        // updates x
...
static void oa_insert(
    OaSlot *table, unsigned cap,
    int key) { // defines oa_insert(): helper function used by this example
  unsigned idx = hash_u32((unsigned)key) & (cap - 1); // declares idx
  while (
      table[idx]
          .used) { // => co-03: linear probe -- walks FORWARD through the array
    idx = (idx + 1) &
          (cap - 1); // co-06: wraps at the table boundary (power-of-2 mask)
  }
  table[idx].key = key; // assigns table[idx].key
  table[idx].used = 1;  // assigns table[idx].used
}

static int oa_lookup(
    const OaSlot *table, unsigned cap,
...
static void chain_insert(
    ChainNode **buckets, unsigned cap,
    int key) { // defines chain_insert(): helper function used by this example
  unsigned idx = hash_u32((unsigned)key) & (cap - 1); // declares idx
  ChainNode *node =
      &node_pool[node_pool_perm[node_pool_next++]]; // => co-03: a RANDOM pool
...
static int chain_lookup(
    ChainNode *const *buckets, unsigned cap,
    int key) { // defines chain_lookup(): helper function used by this example
  unsigned idx = hash_u32((unsigned)key) & (cap - 1); // declares idx
  for (ChainNode *n = buckets[idx]; n != NULL;
       n = n->next) { // => co-03: pointer chase through the chain
    if (n->key == key)
...
int main(void) { // program entry point
  OaSlot *oa_table =
      calloc(TABLE_CAP, sizeof(OaSlot)); // heap-allocates memory for oa_table
  ChainNode **chain_table = calloc(
      TABLE_CAP, sizeof(ChainNode *)); // heap-allocates memory for chain_table
  node_pool =
      malloc(sizeof(ChainNode) * N_KEYS); // heap-allocates memory for node_pool
  node_pool_perm =
      malloc(sizeof(int) * N_KEYS); // heap-allocates memory for node_pool_perm
  if (!oa_table || !chain_table || !node_pool ||
      !node_pool_perm) {               // conditional check
  ...
  unsigned pseed =
      31u; // => co-03: Fisher-Yates -- scatters pool-slot assignment
  for (int i = N_KEYS - 1; i > 0;
       i--) { // order independent of insertion (hash-bucket) order
  ...
  int *queries =
      malloc(sizeof(int) * N_QUERIES); // heap-allocates memory for queries
  unsigned seed = 21u;                 // declares seed
  for (int i = 0; i < N_QUERIES;
       i++) { // loop header controlling the sweep below
    seed = seed * 1103515245u + 12345u; // assigns seed
    queries[i] =
        keys[seed % (unsigned)N_KEYS]; // => guaranteed hit, every query
  }
  ...
  double t_oa = (t1.tv_sec - t0.tv_sec) +
                (t1.tv_nsec - t0.tv_nsec) / 1e9; // declares t_oa

  clock_gettime(CLOCK_MONOTONIC, &t0); // calls clock_gettime(...)
  long chain_hits = 0;                 // declares chain_hits
  for (int i = 0; i < N_QUERIES; i++)
    chain_hits +=
        chain_lookup(chain_table, TABLE_CAP,
                     queries[i]); // loop header controlling the sweep below
  clock_gettime(CLOCK_MONOTONIC, &t1); // calls clock_gettime(...)
  double t_chain = (t1.tv_sec - t0.tv_sec) +
                   (t1.tv_nsec - t0.tv_nsec) / 1e9; // declares t_chain
  ...
  printf("separate chaining: %.4f s, %ld hits (expect %d)\n", t_chain,
         chain_hits, N_QUERIES); // prints a report line
  ...
  int pass = (oa_hits == N_QUERIES) && (chain_hits == N_QUERIES) &&
             (speedup > 1.05); // declares pass
