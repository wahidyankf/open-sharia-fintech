...
#define INCREMENTS_PER_THREAD                                                  \
  20000000L // => co-24: 20M increments/thread = 80M total per benchmark
...
static pthread_mutex_t counter_mutex =
    PTHREAD_MUTEX_INITIALIZER; // declares counter_mutex

static void *worker_mutex(
    void *arg) { // defines worker_mutex(): helper function used by this example
  long n = (long)(intptr_t)arg;  // declares n
  for (long i = 0; i < n; i++) { // loop header controlling the sweep below
...
int main(void) {                                         // program entry point
  ...
  double t_atomic =
      run_threads(worker_atomic, INCREMENTS_PER_THREAD); // declares t_atomic
  long final_atomic = atomic_load(&atomic_counter); // declares final_atomic

  double t_mutex =
      run_threads(worker_mutex, INCREMENTS_PER_THREAD); // declares t_mutex
  long final_mutex = mutex_counter;                     // declares final_mutex

  double atomic_ops_per_sec =
      (double)expected / t_atomic; // declares atomic_ops_per_sec
  double mutex_ops_per_sec =
      (double)expected / t_mutex; // declares mutex_ops_per_sec
  double speedup =
      mutex_ops_per_sec > 0 ? t_mutex / t_atomic : 0; // declares speedup

  printf(
      "THREADS=%d, %ld increments/thread, %ld total increments per benchmark\n",
  ...
  printf("mutex  counter: %.4f s, %.1fM ops/sec (final=%ld, correct=%s)\n",
         ...
         final_mutex == expected
             ? "yes"
             : "NO -- BUG"); // continues the printf(...) call above
  printf("atomic speedup over mutex: %.2fx\n", speedup); // prints a report line
  int pass = (final_atomic == expected) && (final_mutex == expected) &&
