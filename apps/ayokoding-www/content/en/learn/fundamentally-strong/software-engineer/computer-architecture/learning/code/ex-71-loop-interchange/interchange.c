...
static void matmul_ijk(
  ...
  for (int i = 0; i < n; i++) {   // loop header controlling the sweep below
    for (int j = 0; j < n; j++) { // loop header controlling the sweep below
      ...
      for (int k = 0; k < n;
           k++) { // => co-03: innermost loop strides B by N floats -- BAD
...
static void matmul_ikj(
    const float *a, const float *b, float *c,
    int n) { // defines matmul_ikj(): helper function used by this example
  for (int i = 0; i < n; i++) { // loop header controlling the sweep below
    for (int j = 0; j < n; j++)
      c[i * n + j] =
          0.0f; // => must zero-init: this order accumulates INTO c across k
    for (int k = 0; k < n; k++) { // loop header controlling the sweep below
      ...
      for (int j = 0; j < n;
           j++) { // => co-03: innermost loop strides B AND C by 1 float -- GOOD
        c[i * n + j] +=
            a_ik * b[k * n + j]; // supporting statement for this example
      }
    }
  }
}
...
static double time_matmul(void (*fn)(const float *, const float *, float *,
                                     int),
...
int main(void) { // program entry point
  float *a =
      malloc(sizeof(float) * (size_t)N * N); // heap-allocates memory for a
  float *b =
      malloc(sizeof(float) * (size_t)N * N); // heap-allocates memory for b
  float *c_ijk =
      malloc(sizeof(float) * (size_t)N * N); // heap-allocates memory for c_ijk
  float *c_ikj =
      malloc(sizeof(float) * (size_t)N * N); // heap-allocates memory for c_ikj
  if (!a || !b || !c_ijk || !c_ikj) {
  ...
  double max_diff =
      0.0; // => co-17: both orders compute the SAME mathematical result
  for (int i = 0; i < N * N; i++) { // loop header controlling the sweep below
  ...
  printf("matmul: %dx%d float matrices (%.2f MB each)\n", N, N,
         (double)(sizeof(float) * N * N) /
             (1024.0 * 1024.0)); // prints a report line
