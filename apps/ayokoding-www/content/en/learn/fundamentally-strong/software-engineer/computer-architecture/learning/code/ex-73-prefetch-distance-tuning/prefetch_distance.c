...
static int *make_permutation(
    int n) { // defines make_permutation(): helper function used by this example
  ...
  for (int i = n - 1; i > 0;
       i--) { // => co-03: standard Fisher-Yates -- unbiased shuffle
...
int main(void) {                       // program entry point
  ...
  int n_dist =
      (int)(sizeof(distances) / sizeof(distances[0])); // declares n_dist
...
#define PASSES 4 // constant PASSES = 4
  double best_per_distance[sizeof(distances) /
                           sizeof(distances[0])]; // supporting statement for
                                                  // this example
  ...
  for (int pass = 0; pass < PASSES;
       pass++) { // loop header controlling the sweep below
    for (int d = 0; d < n_dist;
         d++) { // loop header controlling the sweep below
      long sum; // declares sum
      double secs =
          timed_walk(data, perm, N, distances[d], &sum); // declares secs
      if (reference_sum < 0)
  ...
  for (int d = 1; d < n_dist;
       d++) { // => skip index 0 (distance=0, the "off" baseline)
    if (best_positive_time < 0 ||
        best_per_distance[d] < best_positive_time) { // conditional check
  ...
  int interior_optimum =
      (best_positive_distance > distances[1]) && // declares interior_optimum
      (best_positive_distance <
       distances[n_dist - 1]) && // supporting statement for this example
      (best_positive_time <
       smallest_positive_time) && // supporting statement for this example
      (best_positive_time <
       largest_positive_time); // supporting statement for this example
  printf("\nnote: on THIS machine, no-prefetch (%.4f s) is %s the best "
         "enabled-prefetch distance\n",
         t_none, // prints a report line
         t_none <= best_positive_time
             ? "actually AT LEAST AS FAST AS"
             : "slower than"); // continues the printf(...) call above
