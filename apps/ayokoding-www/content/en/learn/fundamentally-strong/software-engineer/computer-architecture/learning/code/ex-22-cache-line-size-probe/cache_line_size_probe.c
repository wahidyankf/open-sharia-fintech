...
static double
now_seconds(void) {   // => co-25: one CLOCK_MONOTONIC read, in seconds
...
// to actually happen
static double sweep_nanoseconds_per_touch(
    const uint8_t *buf,
    size_t stride) { // defines sweep_nanoseconds_per_touch(): helper function
                     // used by this example
  size_t touches =
      BUF_BYTES /
      stride;          // => co-02: fewer, WIDER-spaced touches as stride grows
  double best = 1e300; // => co-25: tracks the fastest of REPEATS sweeps
  for (int r = 0; r < REPEATS;
       r++) { // => co-25: repeat to damp scheduler noise
    volatile uint8_t sink =
        0; // => co-02: volatile -- defeats dead-code elimination of the loop
    double t0 = now_seconds(); // => co-25: start this sweep's timer
    for (size_t i = 0; i < touches;
         i++) { // => co-02: exactly `touches` reads, `stride` bytes apart
      sink ^= buf[i * stride]; // => co-02: THE probed access -- one byte per
                               // stride step
    }
    double elapsed =
        now_seconds() - t0; // => co-25: total time for this whole sweep
    (void)sink; // => co-02: silences "unused" -- volatile already forced the
                // reads
    if (elapsed < best)
      best = elapsed; // => co-25: keep the fastest sweep for this stride
    double per_touch = (best / (double)touches) *
                       1e9; // => co-02: nanoseconds per individual touch
    if (r == REPEATS - 1)
...
int main(void) { // program entry point
  uint8_t *buf =
      malloc(BUF_BYTES); // => co-02: the oversized probe buffer, allocated once
  if (!buf) {
  ...
  size_t strides[] = {8,   16,  32, 64,
                      128, 256, 512}; // => co-02: powers of two spanning well
  ...
  for (size_t k = 0; k < n; k++) { // => co-02: one measured row per stride
    per_touch_ns[k] = sweep_nanoseconds_per_touch(
        buf, strides[k]); // => co-02: the actual timed measurement
    printf("%-10zu %.3f\n", strides[k],
  ...
  double best_jump =
      -1e300; // => co-02: tracks the largest per-touch delta seen so far
  for (size_t k = 1; k < n; k++) { // => co-02: compares every adjacent pair
    double jump =
        per_touch_ns[k] -
        per_touch_ns[k - 1]; // => co-02: this pair's measured increase
  ...
  size_t detected_line_size =
      strides[best_jump_index]; // => co-02: the MEASURED answer -- not
                                // hardcoded 64 or 128
  ...
  sysctlbyname(
      "hw.cachelinesize", &sysctl_line_size, &len, NULL,
  ...
  // truth
  int correct = (detected_line_size ==
                 128); // => co-02: this machine's real, documented line size
  printf(
      "%s\n",
