...
static int try_apple_superpage(void) { // defines try_apple_superpage(): helper
  ...
  size_t sz =
      2 * 1024 * 1024; // => 2 MB -- the size Apple's superpage flag requests
  kern_return_t kr =   // supporting statement for this example
      vm_allocate(mach_task_self(), &addr, sz,
                  VM_FLAGS_ANYWHERE |
                      VM_FLAGS_SUPERPAGE_SIZE_2MB); // calls vm_allocate(...)
...
#define PAGE_INTS                                                              \
  1024 // => 4096 bytes / sizeof(int) = 1024 ints per "page-sized" stride
...
#define COMPACT_PAGES                                                          \
  16 // => co-09: 16 page-sized strides = 64 KB -- trivially TLB-resident
#define REPEATS                                                                \
...
static long
walk_scattered(volatile int *buf, long pages,
...
static long walk_compact(volatile int *buf, long pages, long page_stride_ints,
  ...
  for (long r = 0; r < repeats;
    ...
    for (long p = 0; p < pages;
         p++) { // loop header controlling the sweep below
...
static double time_call(long (*fn)(volatile int *, long, long),
                        volatile int *buf, long pages,
  ...
  return (t1.tv_sec - t0.tv_sec) +
         (t1.tv_nsec - t0.tv_nsec) / 1e9; // returns the computed result
}
...
int main(void) {                              // program entry point
  ...
  int *buf =
      malloc(sizeof(int) * (size_t)buf_ints); // heap-allocates memory for buf
  if (!buf) {
  ...
  (void)
      compact_sum; // discards compact_sum to silence an unused-variable warning
  double t_compact_total =
      (c1.tv_sec - c0.tv_sec) +
      (c1.tv_nsec - c0.tv_nsec) / 1e9; // declares t_compact_total

  long scattered_accesses = SCATTER_PAGES; // declares scattered_accesses
  long compact_accesses =
      (long)COMPACT_PAGES * REPEATS; // declares compact_accesses
  double ns_per_access_scattered = (t_scattered / scattered_accesses) *
                                   1e9; // declares ns_per_access_scattered
  double ns_per_access_compact = (t_compact_total / compact_accesses) *
                                 1e9; // declares ns_per_access_compact
  ...
  double ratio =
      ns_per_access_scattered / ns_per_access_compact; // declares ratio
