...
#define MAKE_CHAIN(                                                              \
    NAME, K) /* macro MAKE_CHAIN(...): expands inline at compile time */         \
  __attribute__((noinline)) static int64_t NAME(                                 \
...
static double time_it(int64_t (*fn)(long), long n,
  ...
  int64_t (*volatile fn_indirect)(long) =
      fn;                 // declares function pointer fn_indirect
...
int main(void) {                         // program entry point
  ...
  double rate1 = (double)N / t1v, rate2 = (double)N / t2v,
         rate8 = (double)N / t8v, rate16 = (double)N / t16v; // declares rate1
  ...
  printf("K=2  chains: %.4f s, %.1fM divs/s (%.2fx of ideal 2x)\n", t2v,
         rate2 / 1e6, ratio2); // prints a report line
  printf("K=8  chains: %.4f s, %.1fM divs/s (%.2fx of ideal 8x)\n", t8v,
         rate8 / 1e6, ratio8); // prints a report line
  printf("K=16 chains: %.4f s, %.1fM divs/s (%.2fx of ideal 16x)\n", t16v,
         rate16 / 1e6, ratio16); // prints a report line
  ...
  int pass =
      (ratio2 >
       1.5) && // low concurrency: genuinely scales (rules out a trivially
               // "everything always serializes" measurement bug)
      (ratio16 <
       8.0); // high concurrency: achieves LESS THAN HALF of the naive ideal 16x
