...
#define BLOCK                                                                  \
  64 // => co-03: tile edge -- a 64x64 double tile is 32 KiB, fits comfortably
     // in L1d
#define REPEATS                                                                \
  3 // => co-25: best-of-3 -- shared-machine noise smoothing (DD-20 step 2)

static double now_seconds(
    void) { // => co-25: same portable clock_gettime timer used across this tier
  ...
  return (double)ts.tv_sec +
         (double)ts.tv_nsec /
             1e9; // => co-25: combined into one double-seconds value
}
...
static void matmul_naive(
  ...
  for (int i = 0; i < n; i++) {   // => co-03: rows of A/C
    for (int j = 0; j < n; j++) { // => co-03: columns of B/C
      ...
      for (int k = 0; k < n;
           k++) { // => co-03: dot product of A's row i and B's column j
  ...
  for (int i = 0; i < n; i++) // => co-03: C must still be zeroed -- blocking
    ...
    for (int j = 0; j < n; j++)
      c[i * n + j] = 0.0; // => co-03: start every output cell at zero before
                          // accumulating tiles
  for (int ii = 0; ii < n;
       ii += BLOCK) { // => co-03: outer tile loop over row-blocks of A/C
    for (int jj = 0; jj < n;
         jj += BLOCK) { // => co-03: outer tile loop over column-blocks of B/C
      for (int kk = 0; kk < n; kk += BLOCK) { // => co-03: outer tile loop over
        ...
        // cached
        for (int i = ii; i < ii + BLOCK;
             i++) { // => co-03: within-tile row -- BLOCK=64 rows per outer tile
                    // step
          for (int j = jj; j < jj + BLOCK;
               j++) { // => co-03: within-tile column -- BLOCK=64 columns per
                      // outer tile step
            double sum =
                c[i * n +
                  j]; // => co-03: resume accumulating THIS cell across k-tiles
            for (int k = kk; k < kk + BLOCK;
                 k++) { // => co-03: within-tile reduction -- only BLOCK terms
                        // per (ii,jj,kk) pass
              sum += a[i * n + k] *
                     b[k * n + j]; // => co-03: identical arithmetic to the
                                   // naive kernel, tile-local
            } // => co-03: this k-tile's contribution to c[i][j] is now folded
              // in
            c[i * n + j] =
                sum; // => co-03: write back the partial sum for this k-tile
...
int main(
    void) { // => co-25: single self-contained process -- both kernels run in it
  double *a =
      malloc((size_t)N * N * sizeof(double)); // => co-03: 2 MiB input matrix A
  double *b =
      malloc((size_t)N * N * sizeof(double)); // => co-03: 2 MiB input matrix B
  double *c_naive =
      malloc((size_t)N * N *
             sizeof(double)); // => co-03: 2 MiB output for the naive kernel
  double *c_blocked =
      malloc((size_t)N * N *
             sizeof(double)); // => co-03: 2 MiB output for the blocked kernel
  ...
  for (int i = 0; i < N * N;
       i++) { // => co-01: fill A and B once with deterministic values
    a[i] =
        (double)((i * 7) % 13) -
        6.0; // => co-01: arbitrary but deterministic -- content doesn't matter
    b[i] =
        (double)((i * 11) % 17) - 8.0; // => co-01: different formula so A != B
  } // => co-01: both matrices fully touched once, before any timed pass

  double best_naive = 1e18,
         best_blocked =
             1e18; // => co-25: track the FASTEST of REPEATS runs per kernel
  for (int r = 0; r < REPEATS;
       r++) { // => co-25: re-run to confirm the result reproduces
    ...
    double t2 =
        now_seconds(); // => co-25: start the clock for this blocked pass
  ...
  double max_diff = 0.0; // => co-03: largest per-cell difference between the
                         // two kernels' outputs
  for (int i = 0; i < N * N;
       i++) { // => co-03: scan every one of the N*N result cells
    double diff =
        c_naive[i] - c_blocked[i]; // => co-03: exact double arithmetic --
                                   // blocking changes ORDER, not values here
    if (diff < 0)
      diff = -diff; // => co-03: absolute difference
    if (diff > max_diff)
      max_diff =
          diff; // => co-03: track the worst-case mismatch across all cells
  ...
  printf(
      "blocked is %.2fx faster -> %s\n",
      speedup, // => co-25: prints the ratio the reader should verify against
               // below
      (correct && speedup > 1.1)
          ? // => co-03: correctness (near-zero diff) AND speed both gate the
            // PASS
  ...
  return (correct && speedup > 1.1)
             ? 0
             : 1; // => co-25: real process exit code reflecting the PASS/FAIL
}
