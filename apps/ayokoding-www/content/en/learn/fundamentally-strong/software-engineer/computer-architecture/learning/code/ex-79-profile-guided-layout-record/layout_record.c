...
static long scan_next_task_aos(const TaskRecord *tasks,
  ...
  for (int i = 0; i < n;
       i++) { // => co-17: one full 256 B record per iteration...
    long score = tasks[i].priority * 1000L -
                 tasks[i].deadline; // ...to read 8 of those 256 bytes
    if (score > best_score) {
      best_score = score;
      best_idx = i;
    } // conditional check
...
static long scan_next_task_soa(const int *priority, const int *deadline,
  ...
  long best_idx = -1;                   // declares best_idx
  for (int i = 0; i < n;
       i++) { // => co-03: dense reads -- 32 useful ints per 128 B line
...
static double best_of(long (*fn)(const void *, const void *, int),
                      const void *a, const void *b,
                      int n) { // declares function pointer fn
  long (*volatile fn_v)(const void *, const void *, int) =
      fn;                            // => co-25: see ex-74/ex-75/ex-78 for
...
static long wrap_aos(const void *a, const void *b, int n) {
  (void)b;
  return scan_next_task_aos((const TaskRecord *)a, n);
} // performs several bookkeeping updates in one line
static long wrap_soa(const void *a, const void *b, int n) {
  return scan_next_task_soa((const int *)a, (const int *)b, n);
} // supporting statement for this example

int main(void) { // program entry point
  TaskRecord *aos =
      malloc(sizeof(TaskRecord) * N);      // heap-allocates memory for aos
  ...
  for (int i = 0; i < N; i++) { // loop header controlling the sweep below
    ...
    aos[i].priority = priority[i] =
        (int)(seed % 100u);             // assigns aos[i].priority
    seed = seed * 1103515245u + 12345u; // assigns seed
    aos[i].deadline = deadline[i] =
        (int)(seed % 1000u);     // assigns aos[i].deadline
  ...
  double footprint_aos_mb = (double)(sizeof(TaskRecord) * N) /
                            (1024.0 * 1024.0); // declares footprint_aos_mb
  double footprint_soa_mb = (double)(sizeof(int) * 2 * N) /
                            (1024.0 * 1024.0); // declares footprint_soa_mb
