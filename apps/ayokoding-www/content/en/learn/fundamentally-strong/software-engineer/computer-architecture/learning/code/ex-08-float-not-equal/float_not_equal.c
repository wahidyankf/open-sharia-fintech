...
int main(void) {  // program entry point
  ...
  double b =
      0.2; // => co-13: same problem -- both are ROUNDED to nearest double
  double sum =
      a +
      b; // => co-14: the rounded 0.1 plus the rounded 0.2, then rounded again
  double target =
      0.3; // => co-13: 0.3 is ALSO rounded, but to a DIFFERENT nearby double
  ...
  int correct =
      (sum != target) &&
      (sum_bits !=
       target_bits); // => co-14: both the value AND the bits genuinely differ
  printf(
      "%s\n",
