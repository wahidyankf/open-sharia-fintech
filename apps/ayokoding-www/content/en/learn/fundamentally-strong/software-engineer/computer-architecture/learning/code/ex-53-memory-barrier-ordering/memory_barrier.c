...
static void *producer(
    void *arg) { // defines producer(): helper function used by this example
  ...
  for (long i = 1; i <= ITERS; i++) { // loop header controlling the sweep below
    ...
    while (atomic_load_explicit(&hs.flag, memory_order_acquire) !=
           0) { // loop header controlling the sweep below
...
static void *consumer(
    void *arg) { // defines consumer(): helper function used by this example
  ...
  for (long i = 0; i < ITERS; i++) { // loop header controlling the sweep below
    ...
    long observed =
        hs.data; // => co-24: per the C11 model, this MUST see the
...
static void *relaxed_producer(void *arg) { // defines relaxed_producer(): helper
  ...
  for (long i = 1; i <= ITERS; i++) { // loop header controlling the sweep below
    ...
    while (atomic_load_explicit(&rhs.flag, memory_order_relaxed) !=
           0) { // loop header controlling the sweep below
    }
  }
  return NULL; // returns the computed result
...
static void *relaxed_consumer(void *arg) { // defines relaxed_consumer(): helper
  ...
  for (long i = 0; i < ITERS; i++) { // loop header controlling the sweep below
    ...
    expected++;        // supporting statement for this example
    atomic_store_explicit(
        &rhs.flag, 0, memory_order_relaxed); // calls atomic_store_explicit(...)
  }
  return NULL; // returns the computed result
}
...
int main(void) {                                   // program entry point
  ...
  printf("[release/acquire] ITERS=%ld, ordering violations: %ld -> %s\n", ITERS,
         ...
         (violations == 0) ? "PASS (release/acquire handshake held under load)"
  ...
  printf(
      "[relaxed, for comparison] ITERS=%ld, ordering violations: %ld -> %s\n",
  ...
  return (violations == 0)
             ? 0
             : 1; // => co-24: THIS example's hard gate is the CORRECT
