...
static long sum_branchy_max(
  ...
  for (int i = 0; i < n; i++) { // loop header controlling the sweep below
    if (a[i] > b[i]) { // => co-21: a REAL conditional branch (cmp+b.le) --
      ...
      __asm__ volatile(
          ""
          : "+r"(v)); //    them back into a single conditional-select
...
#pragma clang loop vectorize(disable)                                          \
  ...
  for (int i = 0; i < n; i++) { // loop header controlling the sweep below
    ...
    int m =
        a[i] - (diff & mask); // => co-21: mask=0 -> m=a[i]-0=a[i]; mask=-1 ->
...
int main(void) { // program entry point
  ...
  for (int i = 0; i < N;
       i++) { // => uniformly random pairs -- no exploitable pattern
  ...
  double speedup =
      best_branchy /
      best_branchless; // => co-21: how much the branchless rewrite recovered
  printf("branchless is %.2fx faster -> %s\n", speedup, // prints a report line
         (sum_branchy == sum_branchless &&
  ...
  return (sum_branchy == sum_branchless && speedup > 1.1)
             ? 0
