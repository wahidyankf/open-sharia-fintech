...
#define ITERS                                                                  \
  100000000L // => co-24: 100M increments/thread -- large enough for a stable
             // timing signal
#define STRIDE_LONGS                                                           \
  32 // => co-02: 32 * 8 B = 256 B -- guarantees two DIFFERENT 128 B cache lines
...
static double now_seconds(
  ...
  return (double)ts.tv_sec +
         (double)ts.tv_nsec / 1e9; // => combined into one double-seconds value
}
...
static double run_pair(volatile long *a, volatile long *b) {
  ...
  double t0 =
      now_seconds(); // => co-25: start the clock before either thread exists
...
int main(void) {
  ...
  long *buf =
      calloc(1024, sizeof(long)); // => 8 KB buffer, zero-initialized counters
  if (!buf) {
  ...
  // line
  double t_adjacent =
      run_pair((volatile long *)&buf[0], (volatile long *)&buf[1]);
  // co-02: offsets 0 and STRIDE_LONGS are 256 bytes apart -- two DIFFERENT 128
  // B lines
  double t_separate =
      run_pair((volatile long *)&buf[0], (volatile long *)&buf[STRIDE_LONGS]);
  ...
  double slowdown =
      t_adjacent /
      t_separate; // => co-24: how many times slower sharing a line was
  printf("false-sharing slowdown: %.2fx -> %s\n", slowdown,
         slowdown > 1.3 ? "PASS (adjacent measurably slower)"
                        : "FAIL"); // => co-25: program judges its own claim
