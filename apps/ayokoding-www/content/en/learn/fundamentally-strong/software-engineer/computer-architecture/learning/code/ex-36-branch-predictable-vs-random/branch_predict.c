...
static long sum_if_positive(
  ...
  for (int i = 0; i < n; i++) { // loop header controlling the sweep below
    if (arr[i] >= 0) { // => co-21: the data-dependent branch under test --
      ...
      __asm__ volatile(
          ""
          : "+r"(v)); //    them back into a single bic/asr bit-trick
...
static long
sum_if_positive_repeated(const int *arr, int n,
  ...
  for (int p = 0; p < passes; p++) { // loop header controlling the sweep below
    total += sum_if_positive(
        arr, n); // => co-21: identical work every pass, same data
  }
  return total; // returns the computed result
}
...
int main(void) { // program entry point
  ...
  int *sorted =
      malloc((size_t)N * sizeof(int)); // heap-allocates memory for sorted
  int *shuffled =
      malloc((size_t)N * sizeof(int)); // heap-allocates memory for shuffled
  if (!sorted || !shuffled) {
  ...
  for (int i = 0; i < N; i++) { // loop header controlling the sweep below
    int v = (int)(rand() % 2001) -
            1000;    // => values in [-1000, 1000] -- roughly half negative
  ...
  for (int t = 0; t < TRIALS;
    ...
    sum_sorted =
        sum_if_positive_repeated(sorted, N, PASSES); // assigns sum_sorted
    ...
    sum_shuffled =
        sum_if_positive_repeated(shuffled, N, PASSES); // assigns sum_shuffled
  ...
  double speedup =
      best_shuffled /
      best_sorted; // => co-21: how much slower the mispredicting pass was
  printf(
      "sorted is %.2fx faster -> %s\n", speedup, // prints a report line
      (sum_sorted == sum_shuffled &&
       speedup > 1.2) // continues the printf(...) call above
  ...
  return (sum_sorted == sum_shuffled && speedup > 1.2)
             ? 0
             : 1; // returns the computed result
}
