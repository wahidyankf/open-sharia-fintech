...
static long sum_hot_soa(const int *hot,
  ...
  long total =
      0; // => co-17: accumulator over the SAME hot values, different layout
  for (int i = 0; i < n;
       i++) { // => co-17: one iteration per element -- N total, identical work
...
int main(
    void) { // => co-25: single self-contained process -- both layouts run in it
  ...
  AosRecord *aos =
      malloc((size_t)N * sizeof(AosRecord)); // => co-17: 512 MiB -- N structs,
                                             // hot+cold interleaved per record
  int *soa_hot =
      malloc((size_t)N * sizeof(int)); // => co-17: 32 MiB -- N ints, ONLY the
  ...
  for (int i = 0; i < N;
       i++) { // => co-01: fill both layouts with the SAME hot values once
    int value = (i * 7919) % 1009; // => co-01: arbitrary deterministic value --
                                   // content doesn't matter
    aos[i].hot =
        value; // => co-17: same value written into the AoS record's hot field
    soa_hot[i] =
        value; // => co-17: same value written into the SoA hot-only array
  }

  double best_aos = 1e18,
         best_soa =
             1e18; // => co-25: track the FASTEST of REPEATS runs per layout
  ...
  printf(
      ...
      speedup, // => co-25: prints the ratio the reader should verify against
               // below
      (aos_sum == soa_sum && speedup > 1.2)
          ? // => co-17: correctness (equal sums) AND speed both gate the PASS
  ...
  return (aos_sum == soa_sum && speedup > 1.2)
             ? 0
