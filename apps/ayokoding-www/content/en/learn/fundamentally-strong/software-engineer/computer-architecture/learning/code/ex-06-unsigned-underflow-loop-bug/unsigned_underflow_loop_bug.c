...
// unsigned type).
static void trace_buggy_condition(
    size_t n) { // => co-12: n=3 traces a non-empty case; n=0 traces empty
  printf("n = %zu: i = n - 1 starts at %zu\n", n,
         n - 1); // => co-12: n=0 underflows HERE, before any loop body runs
  size_t i =
      n - 1; // => co-12: for n=0 this is already SIZE_MAX -- immediate danger
  for (int watchdog = 0; watchdog < 6;
       watchdog++) { // => co-12: SAFETY CAP -- the real bug has no such cap
    int condition_true =
        (i >= 0); // => co-12: always 1 for size_t -- this IS the bug
...
// makes `0 > 0` false)
static long sum_reverse_fixed(
    const int *array, size_t n,
  ...
  for (size_t i = n;
       i-- >
       0;) { // => co-12: THE FIX -- no i >= 0 tautology, terminates always
...
int main(void) {                                  // program entry point
  ...
  trace_buggy_condition(
      3); // => co-12: shows i=2,1,0 then wraps to SIZE_MAX and beyond
  printf("\n--- bug trace: empty (n=0) ---\n"); // => co-12: section header for
                                                // the empty-array bug trace
  trace_buggy_condition(
      0); // => co-12: shows the underflow happens on the VERY FIRST line
  ...
  printf(
      "\n--- fix: non-empty array {10, 20, 30} ---\n"); // => co-12: section
                                                        ...
                                                        // non-empty case
  int data[3] = {10, 20,
                 30};        // => co-12: expected sum 60, expected iterations 3
  size_t iters_nonempty = 0; // => co-12: filled in by sum_reverse_fixed
  long sum_nonempty = sum_reverse_fixed(
      data, 3, &iters_nonempty); // => co-12: runs the FIXED idiom for real
  ...
  long sum_empty = sum_reverse_fixed(
      data, 0,
  ...
  int correct = // => co-12: the assertion this whole example checks
      sum_nonempty == 60 &&
      iters_nonempty ==
          3 && // => co-12: non-empty fix ran exactly 3 times, summed 60
      sum_empty == 0 &&
      iters_empty ==
          0; // => co-12: empty fix ran ZERO times -- proves it terminates
  printf(
      "\n%s\n",
