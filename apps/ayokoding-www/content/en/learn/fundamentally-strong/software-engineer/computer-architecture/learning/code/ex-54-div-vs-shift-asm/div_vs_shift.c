...
__attribute__((noinline)) int divide_by_two_signed(int x) {
  return x / 2;
} // calls __attribute__(...)
...
// correction.
__attribute__((noinline)) unsigned divide_by_two_unsigned(unsigned x) {
  return x / 2u;
} // calls __attribute__(...)
...
// below).
__attribute__((noinline)) int shift_by_one(int x) {
  return x >> 1;
} // calls __attribute__(...)
...
int main(void) {                   // program entry point
  ...
  for (int x = -1000; x <= 1000;
       x++) { // loop header controlling the sweep below
  ...
  // handle
  int neg7_div =
      divide_by_two_signed(-7); // => C's / truncates toward zero: -7/2 == -3
  int neg7_shift =
      shift_by_one(-7); // => plain >> rounds toward -infinity: -7>>1 == -4
  unsigned u7 =
      divide_by_two_unsigned(7u); // => unsigned: 7/2 == 3, same as 7u>>1
  ...
  printf(
      "negative-x mismatches (x/2 != x>>1): %d out of 1000 (expected >0 -- "
  ...
  printf("non-negative-x matches: %s\n",
         matches_for_nonnegative ? "all agree (expected)"
                                 : "MISMATCH -- BUG"); // prints a report line
  int pass =
      (neg7_div == -3) && (neg7_shift == -4) && (u7 == 3) &&
      matches_for_nonnegative &&     // declares pass
