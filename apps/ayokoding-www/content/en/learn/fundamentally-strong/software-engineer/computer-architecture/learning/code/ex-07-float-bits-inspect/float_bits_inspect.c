...
int main(void) {
  float value =
      1.0f; // => co-13: the simplest nontrivial IEEE-754 value to decode
  unsigned int
      bits; // => co-13: destination for value's raw 32-bit representation
  memcpy(&bits, &value, sizeof bits); // => co-13: memcpy avoids strict-aliasing
                                      // UB that a union/cast risks

  unsigned int sign =
      (bits >> 31) & 0x1u; // => co-13: bit 31 -- 0 means positive
  unsigned int exponent =
      (bits >> 23) & 0xFFu; // => co-13: bits 30..23, 8 bits, BIASED by +127
  unsigned int mantissa =
      bits &
      0x7FFFFFu; // => co-13: bits 22..0, 23 bits, the fractional significand
  ...
  int correct =
      sign == 0 && exponent == 127 &&
      mantissa == 0;     // => co-13: the exact known encoding, checked directly
