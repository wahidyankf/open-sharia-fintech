...
// ---------------------------------------------------------------------------
static void
print_int_bytes(const char *label,
  ...
  printf("%-28s value=%-12d bits=0x%08x bytes(low->high)=%02x %02x %02x %02x\n",
         label, value, (unsigned)(uint32_t)value, bytes[0], bytes[1], bytes[2],
         bytes[3]);
...
static void
print_float_bits(const char *label,
  ...
  unsigned exponent =
      (bits >> 23) & 0xFFu; // => co-13: bits 30..23, biased by 127
  unsigned mantissa =
      bits & 0x7FFFFFu; // => co-13: bits 22..0, the fractional significand
...
// ---------------------------------------------------------------------------
static int checked_records_bytes(int32_t n_records, int32_t record_size,
                                 size_t *out) {
...
static void demonstrate_overflow_hazard(void) {
  ...
  int32_t record_size =
      32; // => bytes per SensorRecord-shaped struct (see cache.c)
  int64_t true_product =
      (int64_t)hostile_n *
      (int64_t)record_size; // => the CORRECT 64-bit answer for comparison
  int32_t naive_product =
      hostile_n *
      record_size; // => co-12: UB in ISO C (signed overflow) --
...
static void demonstrate_endianness(void) {
  uint32_t sensor_id_host =
      0x0A2B3C4Du; // => a plausible packed sensor-id field, in HOST byte order
  uint32_t sensor_id_wire =
      htonl(sensor_id_host); // => co-15: convert to network (big-endian) byte
  ...
  printf("sensor_id_host = 0x%08x  bytes = %02x %02x %02x %02x\n",
         sensor_id_host, host_bytes[0], host_bytes[1], host_bytes[2],
         host_bytes[3]);
  printf("sensor_id_wire = 0x%08x  bytes = %02x %02x %02x %02x  "
         "(network/big-endian order)\n",
         sensor_id_wire, wire_bytes[0], wire_bytes[1], wire_bytes[2],
         wire_bytes[3]);
  printf("roundtrip      = 0x%08x\n", roundtrip);
  printf("PASS: %s (host bytes %s wire bytes on this little-endian machine)\n",
         (roundtrip == sensor_id_host) ? "ntohl(htonl(x)) == x"
                                       : "ROUNDTRIP FAILED",
         memcmp(host_bytes, wire_bytes, 4) != 0 ? "differ from" : "match");
}
