...
#pragma clang loop vectorize(disable)                                          \
    unroll(disable) // supporting statement for this example
  for (int i = 0; i < n;
       i += 8) {       // => co-20: ONE cmp+branch+increment per 8 elements
...
int main(void) {                              // program entry point
  ...
  double speedup =
      best_rolled /
      best_unrolled; // => co-20: how much the reduced loop overhead bought
  printf(
      "unrolled is %.2fx faster -> %s\n", speedup, // prints a report line
  ...
  return (sum_r == sum_u && speedup > 1.05) ? 0
