...
def build_index(
    docs: dict[int, list[str]], *, stem: bool
) -> dict[
    str, set[int]
    ...
    for doc_id, tokens in docs.items():  # => iterates one item at a time
        terms: list[str] = (
            [porter_stem(t) for t in tokens] if stem else list(tokens)
        )  # => co-09: the ONLY difference
...
def main() -> None:  # => defines main
    ...
    raw_index: dict[str, set[int]] = build_index(
        docs, stem=False
    )  # => co-11: no normalization
    stemmed_index: dict[str, set[int]] = build_index(
        docs, stem=True
    )  # => co-09: every token stemmed first
    print(
        f"raw distinct terms:     {len(raw_index)} -> {sorted(raw_index)}"
    )  # => shows raw distinct terms
    print(
        f"stemmed distinct terms: {len(stemmed_index)} -> {sorted(stemmed_index)}"
    )  # => shows stemmed distinct terms

    query_hits: set[int] = stemmed_index.get(
        porter_stem("run"), set()
    )  # => co-11: query "run" through the SAME stemmer
    print(
        f"query 'run' (stemmed index) hits: {sorted(query_hits)}"
    )  # => shows query 'run' (stemmed index) hits

    assert len(stemmed_index) < len(raw_index), (
        "stemming must shrink the distinct-term count"
    )  # => stemming must shrink the distinct-term count
    assert 0 in query_hits, (
        "querying 'run' must retrieve doc 0 ('running'), which the raw index cannot do"
    )  # => querying 'run' must retrieve doc 0 ('running'), which the raw index cannot do
    print(
        f"MATCH: distinct terms fell from {len(raw_index)} to {len(stemmed_index)}, and 'run' now retrieves doc 0"
    )  # => shows MATCH: distinct terms fell from
