...
def stemming_filter(
    ...
    ]  # => returns [porter_stem(t) for t in tokens]


def main() -> None:  # => defines main
    text: str = "Running and jumping quickly"  # => plural-ish/ing forms to make stemming's effect visible

    without_stemming = Analyzer(
        tokenizer=str.split, token_filters=[lowercase_filter]
    )  # => co-25: baseline pipeline
    with_stemming = Analyzer(
        tokenizer=str.split, token_filters=[lowercase_filter, stemming_filter]
    )  # => co-25: ONE filter added

    result_without: list[str] = without_stemming.analyze(
        text
    )  # => the SAME text, no stemming
    result_with: list[str] = with_stemming.analyze(
        text
    )  # => the SAME text, WITH stemming
    ...
    assert result_without != result_with, (
        "adding the stemming filter must CHANGE the emitted terms"
    )  # => adding the stemming filter must CHANGE the emitted terms
    assert result_with[0] == "run", (
        "the stemmed first token must be 'run', not 'running'"
    )  # => the stemmed first token must be 'run', not 'running'
    assert len(result_without) == len(result_with), (
        "swapping a token filter must NOT change the token COUNT, only their form"
    )  # => swapping a token filter must NOT change the token COUNT, only their form
    print(
        f"MATCH: adding one token filter changed the terms ({result_without[0]!r} -> {result_with[0]!r}) without changing their count"
    )  # => shows MATCH: adding one token filter changed the terms (
