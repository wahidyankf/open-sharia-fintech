...
def main() -> None:  # => defines main
    words: list[str] = [
        ...
        "runs",
        "runner",
    ]  # => the three forms ex-17 exercises
    stems: dict[str, str] = {
        w: porter_stem(w) for w in words
    }  # => co-09: each word's own Porter stem
    for w in words:  # => iterates one item at a time
        print(f"porter_stem({w!r}) = {stems[w]!r}")  # => shows porter_stem(

    assert stems["running"] == "run", (
        "'running' must stem to 'run' (ING removed, double-consonant cleanup)"
    )  # => 'running' must stem to 'run' (ING removed, double-consonant cleanup)
    assert stems["runs"] == "run", (
        "'runs' must stem to 'run' (the plain S rule)"
    )  # => 'runs' must stem to 'run' (the plain S rule)
    assert stems["running"] == stems["runs"], (
        "'running' and 'runs' must share the stem 'run'"
    )  # => 'running' and 'runs' must share the stem 'run'
    print(
        f"MATCH: 'running' and 'runs' both stem to {stems['running']!r}"
    )  # => shows MATCH: 'running' and 'runs' both stem to
    ...
    assert stems["runner"] == "runner", (
        "the real Porter algorithm leaves 'runner' unstemmed (m == 1, not > 1)"
    )  # => the real Porter algorithm leaves 'runner' unstemmed (m == 1, not > 1)
    print(
        f"NOTE: {stems['runner']!r} does NOT reduce to 'run' -- Step 4's -ER rule needs measure > 1, and"
    )  # => shows NOTE
    print(
        "      measure('runn') == 1, so the rule's own gate blocks it. Suffix stripping is not lemmatization."
    )  # => shows measure('runn') == 1, so the rule's own gate blocks it. Suffix stripping is not lemmatization.
