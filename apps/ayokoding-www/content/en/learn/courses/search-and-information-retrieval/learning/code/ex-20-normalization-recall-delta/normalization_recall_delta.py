...
def build_index(
...
) -> dict[str, set[int]]:  # => defines build index
    ...
    for doc_id, tokens in docs.items():  # => iterates one item at a time
        ...
        )  # => terms = [porter_stem(t) for t in tokens] if stem else l...
...
def main() -> None:  # => defines main
    ...
    stemmed_index: dict[str, set[int]] = build_index(
        docs, stem=True
    )  # => co-09: every token stemmed

    raw_hits: set[int] = raw_index.get(
        "run", set()
    )  # => an EXACT-match query against the raw index
    stemmed_hits: set[int] = stemmed_index.get(
        porter_stem("run"), set()
    )  # => the SAME query, through the stemmer
    print(
        f"query 'run' against RAW index:     {sorted(raw_hits)} ({len(raw_hits)} hits)"
    )  # => shows query 'run' against RAW index
    print(
        f"query 'run' against STEMMED index: {sorted(stemmed_hits)} ({len(stemmed_hits)} hits)"
    )  # => shows query 'run' against STEMMED index

    assert len(stemmed_hits) > len(raw_hits), (
        "stemming must strictly INCREASE hit count for this query"
    )  # => stemming must strictly INCREASE hit count for this query
    assert stemmed_hits.issuperset(raw_hits), (
        "every raw hit must still be a hit after stemming (recall only grows)"
    )  # => every raw hit must still be a hit after stemming (recall only grows)
