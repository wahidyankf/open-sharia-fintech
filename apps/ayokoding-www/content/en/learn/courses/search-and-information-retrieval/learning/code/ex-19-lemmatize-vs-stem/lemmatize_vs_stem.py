...
LEMMA_DICTIONARY: dict[
    str, str
] = {  # => co-10: a small, HAND-CURATED irregular-form dictionary
...
def lemmatize(
    word: str,
) -> str:  # => look up word's dictionary lemma; words not in the dictionary pass through unchanged
    """Look up word's dictionary lemma; words not in the dictionary pass through unchanged."""
    return LEMMA_DICTIONARY.get(
        word, word
    )  # => co-10: dictionary-based, not rule-based
...
def main() -> None:  # => defines main
    ...
    stem: str = porter_stem(
        word
    )  # => co-09: pure suffix stripping has no suffix to strip here
    ...
    assert lemma == "good", (
        "lemmatize('better') must return the dictionary's irregular mapping 'good'"
    )  # => lemmatize('better') must return the dictionary's irregular mapping 'good'
    assert stem == "better", (
        "porter_stem('better') has no suffix rule to apply and leaves the word unchanged"
    )  # => porter_stem('better') has no suffix rule to apply and leaves the word unchanged
    assert lemma != stem, (
        "the lemma and the stem must DIFFER -- this is the whole point of the contrast"
    )  # => the lemma and the stem must DIFFER -- this is the whole point of the contrast
    print(
        f"MATCH: lemma ({lemma!r}) differs from stem ({stem!r}) -- only a dictionary can catch an irregular form"
    )  # => shows MATCH: lemma (
