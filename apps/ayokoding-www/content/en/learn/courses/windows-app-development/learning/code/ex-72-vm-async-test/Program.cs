// Example 72: host-level verification is Windows-specific, so this executable validator inspects the real target artifact rather than simulating it.
...
var required = new[] { "async Task", "await viewModel.LoadAsync" };
var missing = required.Where(token => !source.Contains(token, StringComparison.Ordinal)).ToArray();
if (missing.Length != 0)
    throw new InvalidOperationException($"Example 72 is missing: {string.Join(", ", missing)}");
Console.WriteLine($"Verified Example 72 in {Path.GetFileName(artifact)}.");
