// Example 28: host-level verification is Windows-specific, so this executable validator inspects the real target artifact rather than simulating it.
...
var required = new[] { "Text=\"{Binding Filter", "UpdateSourceTrigger=PropertyChanged" };
var missing = required.Where(token => !source.Contains(token, StringComparison.Ordinal)).ToArray();
if (missing.Length != 0)
    throw new InvalidOperationException($"Example 28 is missing: {string.Join(", ", missing)}");
Console.WriteLine($"Verified Example 28 in {Path.GetFileName(artifact)}.");
