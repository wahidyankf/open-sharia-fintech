// Example 12: host-level verification is Windows-specific, so this executable validator inspects the real target artifact rather than simulating it.
...
var required = new[] { "<TextBox", "Text=\"{Binding Filter" };
var missing = required.Where(token => !source.Contains(token, StringComparison.Ordinal)).ToArray();
if (missing.Length != 0)
    throw new InvalidOperationException($"Example 12 is missing: {string.Join(", ", missing)}");
Console.WriteLine($"Verified Example 12 in {Path.GetFileName(artifact)}.");
