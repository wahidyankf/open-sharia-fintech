// Example 3: host-level verification is Windows-specific, so this executable validator inspects the real target artifact rather than simulating it.
...
var required = new[] { "OutputType>WinExe", "UseWPF", "TargetFramework>net10.0-windows" };
var missing = required.Where(token => !source.Contains(token, StringComparison.Ordinal)).ToArray();
if (missing.Length != 0)
    throw new InvalidOperationException($"Example 3 is missing: {string.Join(", ", missing)}");
Console.WriteLine($"Verified Example 3 in {Path.GetFileName(artifact)}.");
