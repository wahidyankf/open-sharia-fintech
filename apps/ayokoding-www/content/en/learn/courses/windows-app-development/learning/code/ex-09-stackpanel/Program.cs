// Example 9: host-level verification is Windows-specific, so this executable validator inspects the real target artifact rather than simulating it.
...
var required = new[] { "<StackPanel", "Orientation=\"Horizontal\"" };
var missing = required.Where(token => !source.Contains(token, StringComparison.Ordinal)).ToArray();
if (missing.Length != 0)
    throw new InvalidOperationException($"Example 9 is missing: {string.Join(", ", missing)}");
Console.WriteLine($"Verified Example 9 in {Path.GetFileName(artifact)}.");
