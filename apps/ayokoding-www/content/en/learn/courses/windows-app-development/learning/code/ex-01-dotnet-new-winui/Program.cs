// Example 1: host-level verification is Windows-specific, so this executable validator inspects the real target artifact rather than simulating it.
...
var required = new[] { "net10.0-windows10.0.19041.0", "UseWinUI", "Microsoft.WindowsAppSDK" };
var missing = required.Where(token => !source.Contains(token, StringComparison.Ordinal)).ToArray();
if (missing.Length != 0)
    throw new InvalidOperationException($"Example 1 is missing: {string.Join(", ", missing)}");
Console.WriteLine($"Verified Example 1 in {Path.GetFileName(artifact)}.");
