// Example 52: host-level verification is Windows-specific, so this executable validator inspects the real target artifact rather than simulating it.
...
var required = new[] { "catch (Exception ex)", "ErrorMessage" };
var missing = required.Where(token => !source.Contains(token, StringComparison.Ordinal)).ToArray();
if (missing.Length != 0)
    throw new InvalidOperationException($"Example 52 is missing: {string.Join(", ", missing)}");
Console.WriteLine($"Verified Example 52 in {Path.GetFileName(artifact)}.");
