// Example 11: host-level verification is Windows-specific, so this executable validator inspects the real target artifact rather than simulating it.
...
if (missing.Length != 0)
    throw new InvalidOperationException($"Example 11 is missing: {string.Join(", ", missing)}");
Console.WriteLine($"Verified Example 11 in {Path.GetFileName(artifact)}.");
