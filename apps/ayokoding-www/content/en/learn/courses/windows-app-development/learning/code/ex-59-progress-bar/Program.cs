// Example 59: host-level verification is Windows-specific, so this executable validator inspects the real target artifact rather than simulating it.
...
var required = new[] { "ProgressBar", "Value=\"{Binding Progress}\"" };
var missing = required.Where(token => !source.Contains(token, StringComparison.Ordinal)).ToArray();
if (missing.Length != 0)
    throw new InvalidOperationException($"Example 59 is missing: {string.Join(", ", missing)}");
Console.WriteLine($"Verified Example 59 in {Path.GetFileName(artifact)}.");
