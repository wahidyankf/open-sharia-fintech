// Example 14: host-level verification is Windows-specific, so this executable validator inspects the real target artifact rather than simulating it.
...
var required = new[] { "Text=\"{Binding ErrorMessage}\"" };
var missing = required.Where(token => !source.Contains(token, StringComparison.Ordinal)).ToArray();
if (missing.Length != 0)
    throw new InvalidOperationException($"Example 14 is missing: {string.Join(", ", missing)}");
Console.WriteLine($"Verified Example 14 in {Path.GetFileName(artifact)}.");
