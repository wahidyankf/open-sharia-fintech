module OseBe.Tests.Integration.Steps.EnvTierSteps
...
let private beginScenario () =
    tempDir <- Path.Combine(Path.GetTempPath(), $"ose-be-env-integration-{Guid.NewGuid():N}")
...
let ``the files ".env.local" and ".env.stag" both exist at the app's composition root`` () =
    ...
    for tier in [ "local"; "test"; "stag"; "prod" ] do
        let key = $"OSE_BE_ENV_{tier.ToUpperInvariant()}"
...
let ``configuration values are read from "\.env\.([^"]+)"`` (tier: string) =
    ...
    Assert.Equal($"from-{tier}", Environment.GetEnvironmentVariable($"OSE_BE_ENV_{tier.ToUpperInvariant()}"))

[<Then>]
let ``no value is read from any other env file`` () =
    for tier in [ "local"; "test"; "stag"; "prod" ] |> List.filter ((<>) selectedTier) do
        Assert.Null(Environment.GetEnvironmentVariable($"OSE_BE_ENV_{tier.ToUpperInvariant()}"))


[<Given>]
let ``a tier file at the app's composition root sets a variable to a file value`` () =
    beginScenario ()
    File.WriteAllText(Path.Combine(tempDir, ".env.local"), "OSE_BE_PRECEDENCE=file-value\n")
    touchedKeys <- [ "OSE_BE_PRECEDENCE" ]
    previousValues <- previousValues.Add("OSE_BE_PRECEDENCE", Environment.GetEnvironmentVariable "OSE_BE_PRECEDENCE")
...
let ``the process starts with that variable already set in the process environment`` () =
    ...
    Environment.SetEnvironmentVariable("OSE_BE_PRECEDENCE", "process-value")
    loadEnvTier ()

[<Then>]
let ``the process environment value is used`` () =
    Assert.Equal("process-value", Environment.GetEnvironmentVariable("OSE_BE_PRECEDENCE"))
...
let ``no tier file exists at the app's composition root for the selected tier`` () =
    beginScenario ()
    Environment.SetEnvironmentVariable("OSE_BE_EXISTING", "preserved")
    touchedKeys <- [ "OSE_BE_EXISTING" ]
    previousValues <- previousValues.Add("OSE_BE_EXISTING", Environment.GetEnvironmentVariable "OSE_BE_EXISTING")
...
let ``configuration proceeds using whatever the process environment already supplies`` () =
    Assert.Equal("preserved", Environment.GetEnvironmentVariable("OSE_BE_EXISTING"))
