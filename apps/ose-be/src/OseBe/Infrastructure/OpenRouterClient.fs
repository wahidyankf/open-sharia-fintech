...
let private envOr (readEnvironment: string -> string) (name: string) (fallback: string) : string =
    match readEnvironment name with
    | null
    | "" -> fallback
    | value -> value
...
let loadConfigWith (readEnvironment: string -> string) : OpenRouterConfig =
    { ApiKey = envOr readEnvironment "OSE_BE_OPENROUTER_API_KEY" ""
      Model = envOr readEnvironment "OSE_BE_OPENROUTER_MODEL" DefaultModel
      BaseUrl = envOr readEnvironment "OSE_BE_OPENROUTER_BASE_URL" DefaultBaseUrl }

/// Real process-environment adapter retained for production callers.
[<ExcludeFromCodeCoverage>]
let loadConfig () : OpenRouterConfig =
    loadConfigWith Environment.GetEnvironmentVariable
