...
///
/// Mirrors the legacy `apps/rhino-cli/tests/doctor.rs` BDD-world
...
module RhinoCli.Tests.Integration.Steps.DoctorToolCheckResourceSteps
...
type DoctorToolCheckResourceSteps() =
    let repoRoot =
        let dir =
...
module private FeatureRunner =
    ...
    let run (scenarioTitle: string) : unit =
        ...
        let definitions = StepDefinitions([| typeof<DoctorToolCheckResourceSteps> |])
...
let ``Full scope is the default behaviour`` () =
    FeatureRunner.run "Full scope is the default behaviour"
