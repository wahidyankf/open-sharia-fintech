...
module RhinoCli.Tests.E2E.Steps.GateEnumerationProcessSteps
...
type GateEnumerationSteps() =
    ...
    let list (surface: string) (format: OutputFormat) (byGroup: bool) =
        let formatText = if format = Json then "json" else "text"

        let arguments =
            [ "gate"
              "list"
              sprintf "--surface=%s" surface
              sprintf "--format=%s" formatText ]
            @ (if byGroup then [ "--by-group" ] else [])

        let exitCode, rendered = runCli root arguments
        succeeded <- Some(exitCode = 0)
        output <- rendered

        jsonOutput <-
            if exitCode = 0 && format = Json then
