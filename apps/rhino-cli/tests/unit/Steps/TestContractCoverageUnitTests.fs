...
let ``the fixture root is pinned`` () =
    Assert.Equal("apps/rhino-cli/tests/unit/Fixtures/TestContract/Coverage", TestContractCoverage.FixtureRoot)
...
// ---------------------------------------------------------------------------
// Keys entirely absent, not merely blank
// ---------------------------------------------------------------------------

[<Fact>]
let ``a document missing a required string key entirely is misuse`` () =
    let text = validDocumentJson.Replace("\"project\":\"widget-app\",", "")
    Assert.Contains("\"project\" is required", misuseMessage text)

[<Fact>]
let ``a document missing the reason key entirely is misuse`` () =
    let text = validDocumentJson.Replace("\"reason\":null,", "")
    Assert.Contains("\"reason\" is required", misuseMessage text)

[<Fact>]
let ``a blank reason string is misuse`` () =
    let text = validDocumentJson.Replace("\"reason\":null", "\"reason\":\"\"")
    Assert.Contains("\"reason\" must not be blank", misuseMessage text)

[<Fact>]
let ``a threshold missing its value key entirely is misuse`` () =
    let text =
        documentJson "unit" "measured" "null" [ "{\"source\":\"project.json\"}" ] nativeCommand [] [ fullSlice ]

    Assert.Contains("\"value\" is required", misuseMessage text)

[<Fact>]
let ``a threshold value that is not representable as an integer is misuse`` () =
    let text =
        documentJson
            "unit"
            "measured"
            "null"
            [ "{\"source\":\"project.json\",\"value\":1.5}" ]
            nativeCommand
            []
            [ fullSlice ]

    Assert.Contains("\"value\" must be an integer", misuseMessage text)

[<Fact>]
let ``a slice missing its applicable key entirely is misuse`` () =
    let text =
        documentJson
            "unit"
            "measured"
            ...
            nativeCommand
            []
            [ "{\"path\":\"src/widget/core.fs\",\"measured\":true,\"covered\":100,\"total\":100,\"output\":\"coverage/unit/core\"}" ]

    Assert.Contains("\"applicable\" is required", misuseMessage text)

[<Fact>]
let ``a slice applicable value that is not a boolean is misuse`` () =
    let text =
        documentJson
            "unit"
            "measured"
            ...
            nativeCommand
            []
            [ "{\"path\":\"src/widget/core.fs\",\"applicable\":\"yes\",\"measured\":true,\"covered\":100,\"total\":100,\"output\":\"coverage/unit/core\"}" ]

    Assert.Contains("\"applicable\" must be a boolean", misuseMessage text)

[<Fact>]
let ``a document missing the exclusions key entirely is misuse`` () =
    let text = validDocumentJson.Replace("\"exclusions\":[],", "")
    Assert.Contains("\"exclusions\" is required", misuseMessage text)

[<Fact>]
let ``a document missing the target key entirely is misuse`` () =
    let text =
        sprintf
            "{\"schema\":\"%s\",\"case\":\"inline\",\"project\":\"widget-app\",\"owner\":\"widget-app\",\"adapter\":\"unit\",\"profile\":\"measured\",\"reason\":null,\"thresholds\":[%s],\"exclusions\":[],\"slices\":[%s]}"
            TestContractCoverage.SchemaVersion
            standardThreshold
            fullSlice

    Assert.Contains("\"target\" is required", misuseMessage text)
...
[<Fact>]
let ``a document that is a JSON array rather than an object is misuse`` () =
    match TestContractCoverage.parseDocument "[]" with
    | Error(TestContract.Misuse message) -> Assert.Contains("the fixture must be a JSON object", message)
    | other -> failwith (sprintf "a non-object root must fail, found %A" other)

// ---------------------------------------------------------------------------
// Negative totals and exclusion pattern shapes
// ---------------------------------------------------------------------------

[<Fact>]
let ``a negative total count is misuse`` () =
    let text =
        documentJson
            "unit"
            "measured"
            ...
            nativeCommand
            []
            [ sliceJson "src/widget/core.fs" true true 0 -5 "coverage/unit/core" ]

    Assert.Contains("\"total\"", misuseMessage text)
    Assert.Contains("must not be negative", misuseMessage text)

[<Fact>]
let ``an exclusion entry that is not a string is misuse`` () =
    let text = validDocumentJson.Replace("\"exclusions\":[]", "\"exclusions\":[1]")
    Assert.Contains("\"exclusions\" must contain only strings", misuseMessage text)

[<Fact>]
let ``an exclusion entry that is blank is misuse`` () =
    let text = validDocumentJson.Replace("\"exclusions\":[]", "\"exclusions\":[\"\"]")
    Assert.Contains("\"exclusions\" must not contain a blank entry", misuseMessage text)

[<Fact>]
let ``an exclusion pattern ending in a bare double-star excludes any path sharing its prefix`` () =
    let text =
        documentJson
            "unit"
            "measured"
            ...
            [ "src/widget/co**" ]
            [ sliceJson "src/widget/core.fs" true true 100 100 "coverage/unit/core"
              sliceJson "src/widget/shell.fs" true true 50 50 "coverage/unit/shell" ]

    match validateInline text with
    | Ok report ->
        Assert.Equal(50, report.Covered)
        Assert.Equal(50, report.Total)
    | Error(TestContract.ContractFailure message) -> failwith ("core.fs must be excluded, not fail: " + message)
    | Error(TestContract.Misuse message) -> failwith ("unexpected misuse: " + message)

[<Fact>]
let ``an exclusion pattern with no wildcard excludes only its exact path`` () =
    let text =
        documentJson
            "unit"
            "measured"
            ...
            [ "src/widget/core.fs" ]
            [ sliceJson "src/widget/core.fs" true true 100 100 "coverage/unit/core"
              sliceJson "src/widget/core.fs.bak" true true 20 20 "coverage/unit/corebak" ]

    match validateInline text with
    | Ok report ->
        Assert.Equal(20, report.Covered)
        Assert.Equal(20, report.Total)
    | Error(TestContract.ContractFailure message) -> failwith ("only the exact path must be excluded: " + message)
    | Error(TestContract.Misuse message) -> failwith ("unexpected misuse: " + message)
