...
[<Fact>]
let ``a file that repeats a target inside its own selectedBy list is rejected`` () =
    let body =
        "\"case\":\"c\",\"project\":\"p\",\"owner\":\"o\",\"root\":\"apps/p\",\"ownedLayers\":[\"unit\"],\"directories\":[],"
        + "\"files\":[{\"path\":\"apps/p/tests/unit/dup.test.ts\",\"executable\":true,\"selectedBy\":[\"test:unit\",\"test:unit\"]}]"

    Assert.Contains("repeats the target test:unit", misused (minimal body))

[<Fact>]
let ``a repeated file path across the files list is rejected`` () =
    let body =
        "\"case\":\"c\",\"project\":\"p\",\"owner\":\"o\",\"root\":\"apps/p\",\"ownedLayers\":[\"unit\"],\"directories\":[],"
        + "\"files\":[{\"path\":\"apps/p/tests/unit/a.test.ts\",\"executable\":true,\"selectedBy\":[\"test:unit\"]},"
        + "{\"path\":\"apps/p/tests/unit/a.test.ts\",\"executable\":false,\"selectedBy\":[\"test:integration\"]}]"

    Assert.Contains("\"files\" repeats apps/p/tests/unit/a.test.ts", misused (minimal body))
...
[<Fact>]
let ``a fixture name absent from the layout corpus is misuse`` () =
    match TestContractLayout.loadDocument "/repo-without-this-fixture" "does-not-exist.json" with
    | Error(TestContract.Misuse message) ->
        Assert.Contains("does-not-exist.json", message)
...
let ``the fixture root is the layout corpus`` () =
    Assert.Equal("apps/rhino-cli/tests/unit/Fixtures/TestContract/Layout", TestContractLayout.FixtureRoot)
...
[<Fact>]
let ``a file whose path never sits under the project root is still measured against its own literal path`` () =
    // The file path does not start with "apps/p/", so the forbidden-directory
    // check falls back to the file's own literal path instead of a path
    // relative to the project root, and the file is then reported outside
    // the (unrelated) project's tests root.
    let body =
        "\"case\":\"c\",\"project\":\"p\",\"owner\":\"o\",\"root\":\"apps/p\",\"ownedLayers\":[\"unit\"],\"directories\":[],"
        + "\"files\":[{\"path\":\"apps/other/tests/unit/main.test.ts\",\"executable\":true,\"selectedBy\":[\"test:unit\"]}]"

    match TestContractLayout.parseDocument (minimal body) with
    | Ok document ->
        match TestContractLayout.validateDocument document with
        | Error(TestContract.ContractFailure message) ->
            Assert.StartsWith("layout-test-outside-tests-root", message)
            Assert.Contains("item=apps/other/tests/unit/main.test.ts", message)
        | other -> failwith (sprintf "expected a contract failure, got %A" other)
    | other -> failwith (sprintf "expected the document to parse, got %A" other)

[<Fact>]
let ``a directory outside tests, an unnamed tests directory, and a nested tests directory are all accepted`` () =
    // Three directory shapes that never trigger a placeholder finding:
    // one outside "tests/" entirely, one directly under "tests/" whose name
    // is not a known layer, and one nested more than one level under
    // "tests/". None of the three is the empty owned-layer placeholder the
    // rule actually polices.
    let body =
        "\"case\":\"c\",\"project\":\"p\",\"owner\":\"o\",\"root\":\"apps/p\",\"ownedLayers\":[\"unit\"],"
        + "\"directories\":[\"apps/p/src\",\"apps/p/tests/legacy\",\"apps/p/tests/unit/helpers\"],"
        + "\"files\":[{\"path\":\"apps/p/tests/unit/main.test.ts\",\"executable\":true,\"selectedBy\":[\"test:unit\"]}]"
