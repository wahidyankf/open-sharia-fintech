...
let ``the fixture root is the unit-test fixture directory`` () =
    Assert.Equal("apps/rhino-cli/tests/unit/Fixtures/TestContract/Bdd", TestContractBdd.FixtureRoot)
...
[<Fact>]
let ``a document omitting the driver key entirely is misuse`` () =
    let head = requiredUnitHead.Replace(sprintf ",\"driver\":\"%s\"" unitDriver, "")

    Assert.Contains("driver", misuse (documentJson head oneFeatureBody))

[<Fact>]
let ``a blank driver is misuse`` () =
    let head =
        requiredUnitHead.Replace(sprintf "\"driver\":\"%s\"" unitDriver, "\"driver\":\"   \"")
...
[<Fact>]
let ``a feature missing the scenarios key entirely is misuse`` () =
    let feature = sprintf "{\"path\":\"%s\"}" checkoutFeature
    let body = explicitBody [ feature ] []
    Assert.Contains("scenarios", misuse (documentJson requiredUnitHead body))
...
[<Fact>]
let ``a scenario missing the examples key entirely is misuse`` () =
    let scenario =
        sprintf "{\"name\":\"%s\",\"steps\":[%s]}" checkoutScenario (quoted checkoutSteps)

    let body = explicitBody [ featureJson checkoutFeature [ scenario ] ] []
    Assert.Contains("examples", misuse (documentJson requiredUnitHead body))

[<Fact>]
let ``a non-integer examples value is misuse`` () =
    let scenario =
        sprintf "{\"name\":\"%s\",\"examples\":1.5,\"steps\":[%s]}" checkoutScenario (quoted checkoutSteps)
...
[<Fact>]
let ``a non-string step is misuse`` () =
    let scenario =
        sprintf "{\"name\":\"%s\",\"examples\":1,\"steps\":[1,2,3]}" checkoutScenario
...
[<Fact>]
let ``a non-object synthetic value is misuse`` () =
    Assert.Contains("synthetic", misuse (documentJson requiredUnitHead "\"synthetic\":123"))
...
[<Fact>]
let ``a non-object document root is misuse`` () = Assert.Contains("object", misuse "[]")
...
[<Fact>]
let ``a blank fixture path is misuse`` () =
    match TestContractBdd.loadDocument "/repo" "   " with
    | Error(TestContract.Misuse message) -> Assert.Contains("--fixture", message)
...
let ``a fixture outside the BDD fixture root is misuse`` () =
    match
        TestContractBdd.loadDocument "/repo" "apps/rhino-cli/tests/unit/Fixtures/TestContract/Layout/src-root.json"
    with
