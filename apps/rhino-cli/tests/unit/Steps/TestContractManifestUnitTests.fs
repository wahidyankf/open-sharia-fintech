...
[<Fact>]
let ``a fixture name absent from the manifest corpus is misuse`` () =
    match TestContractManifest.loadDocument "/repo-without-this-fixture" "does-not-exist.json" with
    | Error(TestContract.Misuse message) ->
...
let ``the fixture root is the manifest corpus`` () =
    Assert.Equal("apps/rhino-cli/tests/unit/Fixtures/TestContract/Manifest", TestContractManifest.FixtureRoot)
