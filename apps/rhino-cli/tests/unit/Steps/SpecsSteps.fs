...
// ---------------------------------------------------------------------------
// Direct unit-style [<Fact>]s for edge cases with no dedicated Gherkin
// scenario — mirrors `TestCoverageUnitTests.fs`'s split rationale from its
// own Steps file. Mostly private-helper edge cases reached only by crafting
// inputs against the nearest public entry point.
// ---------------------------------------------------------------------------

[<Fact>]
let ``capturedTitle recognizes double-quoted and backtick-quoted step titles`` () =
    let js =
        String.Join(
            "\n",
            [ "test.fixme(\"Double quoted title\", () => {});"
              "test.fixme(`Backtick quoted title`, () => {});"
              "" ]
        )

    let titles = scanFixmeTitles js

    Assert.Contains("Double quoted title", titles)
    Assert.Contains("Backtick quoted title", titles)

[<Fact>]
let ``scanUnboundDescribeTitles returns nothing for empty content, and scanFeatureContent tolerates CRLF line endings``
    ()
    =
    Assert.Empty(scanUnboundDescribeTitles "")

    let crlfContent =
        "Feature: F\r\n\r\n  Scenario: First\r\n    Given a\r\n    When b\r\n    When c\r\n"

    let findings = scanFeatureContent "example.feature" crlfContent

    Assert.Equal(1, List.length findings)
    Assert.Equal("When", findings.[0].Keyword)

[<Fact>]
let ``scanSkipOrFixmeDescribeTitles collects a nested describe title inside a skip block`` () =
    let js =
        String.Join(
            "\n",
            [ "test.describe.skip('Outer block', () => {"
              ""
    ...
    let titles = scanSkipOrFixmeDescribeTitles js

    Assert.Contains("Outer block", titles)
    Assert.Contains("Inner block", titles)
    Assert.Contains("Leaf test", titles)

[<Fact>]
let ``isUnboundOrAbsent treats an unresolved feature path as unbound`` () =
    Assert.True(isUnboundOrAbsent "specs/example/no-mirror.feature" "Some scenario" Map.empty)

[<Fact>]
let ``scanFixmeDir treats an undecodable spec.js file as carrying no titles`` () =
    let dir =
        Path.Combine(Path.GetTempPath(), "rhino-cli-fixme-badutf8-" + Guid.NewGuid().ToString("N"))

    Directory.CreateDirectory dir |> ignore
    File.WriteAllBytes(Path.Combine(dir, "broken.spec.js"), [| 0xFFuy; 0xFEuy; 0x00uy |])

    let byFile = scanFixmeDir dir |> Result.defaultWith (fun e -> failwith e)

    Assert.Empty(byFile.["broken"].Unbound)
    Assert.Empty(byFile.["broken"].Rendered)

[<Fact>]
let ``loadBaseline defaults project and allowedUnbound when the manifest has neither field`` () =
    let path = Path.GetTempFileName()
    File.WriteAllText(path, "{}")

    let manifest = loadBaseline path |> Result.defaultWith (fun e -> failwith e)

    Assert.Equal("", manifest.Project)
    Assert.Empty(manifest.AllowedUnbound)

[<Fact>]
let ``saveBaseline reports an error when the destination cannot be written`` () =
    let dir =
        Path.Combine(Path.GetTempPath(), "rhino-cli-savebaseline-" + Guid.NewGuid().ToString("N"))
    ...
    match saveBaseline dir manifest with
    | Error _ -> ()
    | Ok() -> failwith "expected writing to an existing directory to fail"

[<Fact>]
let ``formatGapJson renders new-gap and stale entries in the JSON envelope`` () =
    ...
    let json = formatGapJson report

    Assert.Contains("\"a.feature\"", json)
    Assert.Contains("\"A\"", json)
    Assert.Contains("\"b.feature\"", json)
    Assert.Contains("\"B\"", json)

[<Fact>]
let ``walkFeatureFiles and walkMdFiles return no results for a missing directory`` () =
    let missing =
        Path.Combine(Path.GetTempPath(), "rhino-cli-specs-missing-" + Guid.NewGuid().ToString("N"))

    Assert.Empty(walkFeatureFiles missing)
    Assert.Empty(walkMdFiles missing)

[<Fact>]
let ``countNonReadmeMdFiles counts feature and non-README markdown files, and returns zero for a missing directory``
    ()
    =
    let dir =
        Path.Combine(Path.GetTempPath(), "rhino-cli-specs-count-" + Guid.NewGuid().ToString("N"))

    Directory.CreateDirectory dir |> ignore
    File.WriteAllText(Path.Combine(dir, "example.feature"), "Feature: X\n")
    File.WriteAllText(Path.Combine(dir, "notes.md"), "# Notes\n")
    File.WriteAllText(Path.Combine(dir, "README.md"), "# Index\n")

    Assert.Equal(2, countNonReadmeMdFiles dir)
    Assert.Equal(0, countNonReadmeMdFiles (dir + "-missing"))

[<Fact>]
let ``validateOwnerCorpus reports a missing architecture.md`` () =
    let repoRoot =
        Path.Combine(Path.GetTempPath(), "rhino-cli-specs-owner-" + Guid.NewGuid().ToString("N"))

    let ownerRel = "specs/apps/example/owner"

    let ownerDir =
        Path.Combine(repoRoot, ownerRel.Replace('/', Path.DirectorySeparatorChar))
    ...
    File.WriteAllText(Path.Combine(behaviorsDir, "example.feature"), "Feature: X\n\n  Scenario: S\n    Given a thing\n")

    let findings = validateOwnerCorpus repoRoot ownerRel

    Assert.Contains(
        findings,
        fun (f: SpecFinding) -> f.Category = "tree-shape" && f.File = ownerRel + "/" + ArchitectureFileName
    )

[<Fact>]
let ``validateSpecCounts measures an owner corpus addressed by an absolute rooted path`` () =
    let repoRoot =
        Path.Combine(Path.GetTempPath(), "rhino-cli-specs-counts-" + Guid.NewGuid().ToString("N"))

    let ownerDir = Path.Combine(repoRoot, "specs", "apps", "example", "owner")
    Directory.CreateDirectory ownerDir |> ignore
    File.WriteAllText(Path.Combine(ownerDir, ArchitectureFileName), "# Architecture\n")

    // README.md is intentionally omitted so the rooted-path result carries a
    // finding whose File field proves ownerRel was correctly relativized.
    let behaviorsDir = Path.Combine(ownerDir, "behaviors")
    Directory.CreateDirectory behaviorsDir |> ignore
    File.WriteAllText(Path.Combine(behaviorsDir, "README.md"), "# Index\n")
    ...
    let findings = validateSpecCounts repoRoot ownerDir

    Assert.Contains(findings, fun (f: SpecFinding) -> f.File = "specs/apps/example/owner/README.md")

[<Fact>]
let ``auditGherkinKeywordCardinality reports no findings for a directory that does not exist`` () =
    let missing =
        Path.Combine(Path.GetTempPath(), "rhino-cli-specs-cardinality-missing-" + Guid.NewGuid().ToString("N"))

    match auditGherkinKeywordCardinality [ missing ] with
    | Ok findings -> Assert.Empty(findings)
    | Error e -> failwith e

[<Fact>]
let ``scanFeatureContent ignores primary-keyword-shaped text inside a doc string`` () =
    let content =
        String.Join(
            "\n",
            [ "Feature: F"
              ...
              "    Given a step"
              "    \"\"\""
              "    Given a fake line inside the doc string"
              "    When another fake line"
              "    \"\"\""
    ...
    let findings = scanFeatureContent "example.feature" content

    Assert.Empty(findings)

[<Fact>]
let ``auditGherkinKeywordCardinality sorts violations within one file by ascending line number`` () =
    let dir =
        Path.Combine(Path.GetTempPath(), "rhino-cli-specs-cardinality-" + Guid.NewGuid().ToString("N"))
    ...
    let findings =
        auditGherkinKeywordCardinality [ dir ]
        |> Result.defaultWith (fun e -> failwith e)

    Assert.Equal(2, List.length findings)
    Assert.True(findings.[0].Line < findings.[1].Line)
    Assert.Equal("First", findings.[0].Scenario)
    Assert.Equal("Second", findings.[1].Scenario)

[<Fact>]
let ``runSpecsAudit reports failures from a failing member`` () =
    let runMember (name: string) : Result<unit, string> =
        if name = "validate-links" then
    ...
    Assert.Contains("SPECS AUDIT FAILED", outcome.Summary)
    Assert.Equal<string list>([ "validate-links: broken link found" ], outcome.Failures)

[<Fact>]
let ``validateSpecLinks resolves an absolute rooted folder path directly`` () =
    let dir =
        Path.Combine(Path.GetTempPath(), "rhino-cli-specs-links-" + Guid.NewGuid().ToString("N"))

    Directory.CreateDirectory dir |> ignore
    File.WriteAllText(Path.Combine(dir, "README.md"), "# No links here\n")

    let findings = validateSpecLinks dir dir

    Assert.Empty(findings)

[<Fact>]
let ``parseFeatureContent expands a Scenario Outline step across every Examples row`` () =
    let content =
        String.Join(
            "\n",
            [ "Feature: F"
    ...
    let scenarios, expandedSteps = parseFeatureContent content

    Assert.Equal(1, List.length scenarios)
    Assert.Equal<string list>([ "a value of 1"; "a value of 2" ], expandedSteps)

[<Fact>]
let ``coverageWalkFeatureFiles returns no results for a missing directory`` () =
    let missing =
        Path.Combine(Path.GetTempPath(), "rhino-cli-coverage-missing-" + Guid.NewGuid().ToString("N"))

    Assert.Empty(coverageWalkFeatureFiles missing [])

[<Fact>]
let ``checkAll's relTo strips a RepoRoot prefix, tolerates a trailing separator, and falls back to the raw path when RepoRoot is unrelated``
    ()
    =
    let root =
        Path.Combine(Path.GetTempPath(), "rhino-cli-relto-" + Guid.NewGuid().ToString("N"))
    ...
    let featurePath = Path.Combine(specsDir, "widget.feature")
    File.WriteAllText(featurePath, "Feature: F\n\n  Scenario: S\n    Given a thing\n")

    let optsWith (repoRoot: string) : ScanOptions =
        { RepoRoot = repoRoot
          SpecsDir = specsDir
          SpecsDirs = []
          AppDir = appDir
    ...
    let gapSpecFile (repoRoot: string) : string =
        let result = checkAll (optsWith repoRoot)
        Assert.Equal(1, List.length result.Gaps)
        result.Gaps.[0].SpecFile

    // An empty RepoRoot leaves the absolute path untouched.
    Assert.Equal(featurePath, gapSpecFile "")

    // A RepoRoot that already ends with a separator is used as-is.
    let withSeparator = specsDir + string Path.DirectorySeparatorChar
    Assert.Equal("widget.feature", gapSpecFile withSeparator)

    // A RepoRoot that is not a prefix of the scanned path falls back unchanged.
    Assert.Equal(featurePath, gapSpecFile "/completely/unrelated/root")

[<Fact>]
let ``checkOneToOne recognizes an extensionless test file, C# and F# Tests/Steps suffixes, and a dart _test file, while rejecting an unrecognized extension``
    ()
    =
    let root =
        Path.Combine(Path.GetTempPath(), "rhino-cli-testfile-kinds-" + Guid.NewGuid().ToString("N"))
    ...
    let writeSpec (stem: string) : unit =
        File.WriteAllText(
            Path.Combine(specsDir, stem + ".feature"),
            String.Join("\n", [ sprintf "Feature: %s" stem; ""; "  Scenario: Works"; "    Given a thing"; "" ])
        )

    for stem in [ "Alpha"; "Bravo"; "Charlie"; "Delta"; "Echo"; "Foxtrot" ] do
        writeSpec stem

    // Alpha: extensionless test file — isTestFile's `ext = ""` case is always
    // true, but extractScenarioTitles has no case for it, so the scenario
    // still reports as a title gap despite the file itself being found.
    File.WriteAllText(Path.Combine(appDir, "Alpha"), "irrelevant content")

    // Bravo: a Tests.cs file outside any test/tests directory — reaches the
    // Tests.cs suffix check only after Steps.cs and isInTestDir both fail.
    File.WriteAllText(Path.Combine(appDir, "BravoTests.cs"), "// Scenario: Works\n")

    // Charlie: a Steps.fs file — F# carries no scenario-title extraction case
    // either, so it reports a title gap the same way Alpha does.
    File.WriteAllText(Path.Combine(appDir, "CharlieSteps.fs"), "irrelevant content")

    // Delta: a Tests.fs file, reached only after Steps.cs/Tests.cs/Steps.fs
    // all fail.
    File.WriteAllText(Path.Combine(appDir, "DeltaTests.fs"), "irrelevant content")

    // Echo: a dart file recognized purely by its _test.dart suffix.
    File.WriteAllText(Path.Combine(appDir, "Echo_test.dart"), "// Scenario: Works\n")

    // Foxtrot: matches the stem but carries an extension isTestFile's
    // fallback arm rejects outright.
    File.WriteAllText(Path.Combine(appDir, "Foxtrot.py"), "# Scenario: Works\n")
    ...
    let gapStems = result.Gaps |> List.map (fun g -> g.Stem)
    let scenarioGapSpecs = result.ScenarioGaps |> List.map (fun sg -> sg.SpecFile)

    // Every recognized test file counts as found — only Foxtrot is missing.
    Assert.Equal<string list>([ "Foxtrot" ], gapStems)

    // Alpha (extensionless) and Charlie/Delta (.fs — F# carries no
    // scenario-title extraction case at all) were found but their titles
    // could not be extracted, so all three still report a title gap.
    Assert.True(scenarioGapSpecs |> List.exists (fun s -> s.EndsWith("Alpha.feature")))
    Assert.True(scenarioGapSpecs |> List.exists (fun s -> s.EndsWith("Charlie.feature")))
    Assert.True(scenarioGapSpecs |> List.exists (fun s -> s.EndsWith("Delta.feature")))
    Assert.False(scenarioGapSpecs |> List.exists (fun s -> s.EndsWith("Bravo.feature")))
    Assert.False(scenarioGapSpecs |> List.exists (fun s -> s.EndsWith("Echo.feature")))

[<Fact>]
let ``checkOneToOne recognizes a .integration. and an _test. TypeScript test file naming convention`` () =
    let root =
        Path.Combine(Path.GetTempPath(), "rhino-cli-testfile-ts-" + Guid.NewGuid().ToString("N"))
    ...
    for stem in [ "Golf"; "Hotel" ] do
        File.WriteAllText(
            Path.Combine(specsDir, stem + ".feature"),
            String.Join("\n", [ sprintf "Feature: %s" stem; ""; "  Scenario: Works"; "    Given a thing"; "" ])
        )

    File.WriteAllText(Path.Combine(appDir, "Golf.integration.ts"), "Scenario(\"Works\", () => {});\n")
    File.WriteAllText(Path.Combine(appDir, "Hotel_test.ts"), "Scenario(\"Works\", () => {});\n")
...
[<Fact>]
let ``extractTsStepTexts drops a step definition whose regex fails to compile`` () =
    let path = Path.GetTempFileName() + ".ts"
    File.WriteAllText(path, "Given(/[unclosed/, () => {});\n")
    let sm = StepMatcher()
    extractTsStepTexts path sm
    Assert.True(sm.IsEmpty)

[<Fact>]
let ``extractRustStepTexts drops a step definition whose regex fails to compile`` () =
    let path = Path.GetTempFileName() + ".rs"
    File.WriteAllText(path, "#[given(regex = r#\"[unclosed\"#)]\n")
    let sm = StepMatcher()
    extractRustStepTexts path sm
    Assert.True(sm.IsEmpty)

[<Fact>]
let ``extractFsharpStepTexts drops a backtick step pattern whose regex fails to compile`` () =
    let path = Path.GetTempFileName() + ".fs"

    File.WriteAllText(path, String.Join("\n", [ "[<Given>]"; "let ``a step with [unclosed bracket`` () = ()"; "" ]))

    let sm = StepMatcher()
    extractFsharpStepTexts path sm
    Assert.True(sm.IsEmpty)

[<Fact>]
let ``checkOneToOne's Scenario Outline coverage is satisfied when every expanded variant matches`` () =
    let root =
        Path.Combine(Path.GetTempPath(), "rhino-cli-outline-cov-" + Guid.NewGuid().ToString("N"))
    ...
    File.WriteAllText(
        Path.Combine(specsDir, "outline.feature"),
        String.Join(
            "\n",
            [ "Feature: F"
              ""
    ...
    File.WriteAllText(
        Path.Combine(appDir, "outline.steps.ts"),
        String.Join(
            "\n",
            [ "Given(\"a value of 1\", () => {});"
              "Given(\"a value of 2\", () => {});"
...
[<Fact>]
let ``checkSharedSteps reports an orphan for an unmatched exact step definition`` () =
    let root =
        Path.Combine(Path.GetTempPath(), "rhino-cli-orphan-exact-" + Guid.NewGuid().ToString("N"))
    ...
    File.WriteAllText(
        Path.Combine(specsDir, "widget.feature"),
        String.Join("\n", [ "Feature: F"; ""; "  Scenario: S"; "    Given a used step"; "" ])
    )

    File.WriteAllText(
        Path.Combine(appDir, "widget.steps.ts"),
        String.Join(
            "\n",
            [ "Given(\"a used step\", () => {});"
              "Given(\"a completely unused step\", () => {});"
              "" ]
        )
    )
    ...
    Assert.Contains(
        ...
        fun (o: OrphanStepImpl) -> o.MatcherKind = "exact" && o.MatcherText = "a completely unused step"
    )

[<Fact>]
let ``checkAll finds no specs when both SpecsDir and SpecsDirs are empty`` () =
    let appDir =
        Path.Combine(Path.GetTempPath(), "rhino-cli-nospecs-" + Guid.NewGuid().ToString("N"))
    ...
    Assert.Equal(0, result.TotalSpecs)

[<Fact>]
let ``formatCoverageText separates the orphan section from a preceding gap section`` () =
    let result: CheckResult =
        { TotalSpecs = 1
          TotalScenarios = 1
          TotalSteps = 1
          Gaps =
            [ { SpecFile = "widget.feature"
                Stem = "widget" } ]
          ScenarioGaps = []
          StepGaps = []
          OrphanStepImpls =
            [ { File = "app/orphan.steps.ts"
                MatcherKind = "exact"
                MatcherText = "an orphan step" } ] }

    let text = formatCoverageText result false

    Assert.Contains("Missing test files (1):", text)
    Assert.Contains("Orphan step implementations (1)", text)

[<Fact>]
let ``testLevelName renders e2e`` () = Assert.Equal("e2e", testLevelName E2e)

[<Fact>]
let ``extractCoversMarkers relativizes marker source files against RepoRoot`` () =
    let dir =
        Path.Combine(Path.GetTempPath(), "rhino-cli-covers-relto-" + Guid.NewGuid().ToString("N"))

    Directory.CreateDirectory dir |> ignore
    let filePath = Path.Combine(dir, "test.rs")
    File.WriteAllText(filePath, "// @covers specs/example.feature:A scenario\n")

    // An empty RepoRoot leaves the absolute path untouched.
    let markersEmptyRoot = extractCoversMarkers dir Unit ""
    Assert.Equal(1, List.length markersEmptyRoot)
    Assert.Equal(filePath, markersEmptyRoot.[0].SourceFile)

    // A RepoRoot that already ends with a separator is used as-is.
    let withSeparator = dir + string Path.DirectorySeparatorChar
    let markersTrailingSep = extractCoversMarkers dir Unit withSeparator
    Assert.Equal("test.rs", markersTrailingSep.[0].SourceFile)

    // A RepoRoot that is not a prefix of the scanned file falls back unchanged.
    let markersUnrelated = extractCoversMarkers dir Unit "/completely/unrelated/root"
    Assert.Equal(filePath, markersUnrelated.[0].SourceFile)

[<Fact>]
let ``extractScenarioSpecs recognizes the e2e-tagged level tag`` () =
    let path = Path.GetTempFileName()
    File.WriteAllText(path, String.Join("\n", [ "@e2e"; "Scenario: Works end to end"; "  Given a thing"; "" ]))

    let specs = extractScenarioSpecs path "specs/example.feature"

    Assert.Equal(1, List.length specs)
    Assert.Contains(E2e, specs.[0].LevelTags)

[<Fact>]
let ``extractScenarioSpecs recognizes the integration-tagged level tag`` () =
    let path = Path.GetTempFileName()

    File.WriteAllText(
        path,
        String.Join("\n", [ "@integration"; "Scenario: Works across services"; "  Given a thing"; "" ])
    )

    let specs = extractScenarioSpecs path "specs/example.feature"

    Assert.Equal(1, List.length specs)
    Assert.Contains(Integration, specs.[0].LevelTags)

// ---- Additional coverage-gap-closing tests: walkFeatureFiles/
// auditGherkinKeywordCardinality's per-directory read-failure handlers,
// auditGherkinKeywordCardinality's outer per-file read-failure handler,
// parseFeatureContent's Examples-with-no-outline no-op, formatCoverageText's
// short-circuit-avoiding all-prior-sections-empty case, and
// extractCoversMarkers's per-file read-failure handler. ----

[<Fact>]
let ``walkFeatureFiles silently skips a subdirectory that cannot be listed`` () =
    let root =
        Path.Combine(Path.GetTempPath(), "rhino-cli-specs-walk-unreadable-" + Guid.NewGuid().ToString("N"))

    let subdir = Path.Combine(root, "sub")
    Directory.CreateDirectory subdir |> ignore
    File.WriteAllText(Path.Combine(subdir, "hidden.feature"), "Feature: X\n")
    File.WriteAllText(Path.Combine(root, "top.feature"), "Feature: Y\n")

    try
        File.SetUnixFileMode(subdir, UnixFileMode.None)

        let found = walkFeatureFiles root
        Assert.Equal<string list>([ Path.Combine(root, "top.feature") ], found)
    finally
        File.SetUnixFileMode(subdir, UnixFileMode.UserRead ||| UnixFileMode.UserWrite ||| UnixFileMode.UserExecute)
        Directory.Delete(root, true)

[<Fact>]
let ``auditGherkinKeywordCardinality silently skips a subdirectory that cannot be listed`` () =
    let root =
        Path.Combine(Path.GetTempPath(), "rhino-cli-specs-cardinality-unreadable-dir-" + Guid.NewGuid().ToString("N"))
    ...
    try
        ...
        | Ok findings ->
            Assert.Equal(1, List.length findings)
            Assert.Equal(Path.Combine(root, "top.feature"), findings.[0].File)
        | Error e -> failwith e
    finally
        File.SetUnixFileMode(subdir, UnixFileMode.UserRead ||| UnixFileMode.UserWrite ||| UnixFileMode.UserExecute)
        Directory.Delete(root, true)
...
[<Fact>]
let ``auditGherkinKeywordCardinality reports an error when a feature file cannot be read`` () =
    let root =
        Path.Combine(Path.GetTempPath(), "rhino-cli-specs-cardinality-unreadable-file-" + Guid.NewGuid().ToString("N"))

    Directory.CreateDirectory root |> ignore
    let featurePath = Path.Combine(root, "bad.feature")
    File.WriteAllText(featurePath, "Feature: X\n\n  Scenario: S\n    Given a\n    Given b\n")

    try
        File.SetUnixFileMode(featurePath, UnixFileMode.None)

        match auditGherkinKeywordCardinality [ root ] with
        | Error message -> Assert.Contains("gherkin keyword cardinality audit failed", message)
        | Ok _ -> failwith "expected an error for an unreadable feature file"
    finally
        File.SetUnixFileMode(featurePath, UnixFileMode.UserRead ||| UnixFileMode.UserWrite)
        Directory.Delete(root, true)

[<Fact>]
let ``parseFeatureContent ignores an Examples table with no preceding Scenario Outline`` () =
    let content = "Examples:\n  | a |\n  | 1 |\n"

    let scenarios, expandedSteps = parseFeatureContent content

    Assert.Empty(scenarios)
    Assert.Empty(expandedSteps)

[<Fact>]
let ``formatCoverageText skips the pre-orphan blank line when no other gap section precedes it`` () =
    let result: CheckResult =
        { TotalSpecs = 1
          TotalScenarios = 1
          TotalSteps = 1
    ...
    Assert.Contains("Orphan step implementations (1)", text)
    Assert.DoesNotContain("Missing test files", text)
    Assert.DoesNotContain("Missing scenarios", text)
    Assert.DoesNotContain("Missing steps", text)

[<Fact>]
let ``extractCoversMarkers silently skips a source file that cannot be read`` () =
    let dir =
        Path.Combine(Path.GetTempPath(), "rhino-cli-covers-unreadable-" + Guid.NewGuid().ToString("N"))
    ...
    try
        ...
        let markers = extractCoversMarkers dir Unit dir
