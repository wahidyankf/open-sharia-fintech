...
module RhinoCli.Tests.Unit.Steps.ConventionUnitTests
...
// ---- runAuditMember: unreachable-via-runConventionAudit branch, tested
// directly (internal + InternalsVisibleTo) the same way Rust tests
// `run_member` directly in `convention_audit.rs::tests` ----

[<Fact>]
let ``runAuditMember rejects an unknown validator name`` () =
    match runAuditMember "/irrelevant" "bogus" with
    | Error message -> Assert.Contains("unknown convention validator", message)
    | Ok() -> Assert.Fail("expected an error for an unknown validator name")
...
[<Fact>]
let ``runAuditMember reports an error naming the finding count when the emoji validator fails`` () =
    let root = newTempDir ()

    try
        writeFile root "src/example.ts" "\U0001F680\n"

        match runAuditMember root "emoji" with
        | Error message -> Assert.Contains("emoji finding(s) found", message)
        | Ok() -> Assert.Fail("expected an error when the emoji validator finds a codepoint")
    finally
        Directory.Delete(root, true)
