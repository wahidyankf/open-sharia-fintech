...
let private prebuiltFsharpCli: Lazy<string> =
    lazy
        (let binary =
            Path.Combine(repoRoot, "apps", "rhino-cli", "src-fsharp", "dist", "rhino-cli-fsharp")
         ...
         if File.Exists binary then
             ...
             for a in
                 [ "publish"
                   "apps/rhino-cli/src-fsharp/RhinoCli.Program/RhinoCli.Program.fsproj"
                   "-c"
                   "Release"
                   "--self-contained"
                   "true"
                   "--use-current-runtime"
                   "-o"
                   "apps/rhino-cli/src-fsharp/dist" ] do
...
type ParityManifestSteps() =
    ...
    member _.``a tracked parity test file is edited``() =
        write "apps/rhino-cli/tests/parity.rs" "#[test] fn changed_parity() {}\n"

    [<When>]
    member _.``an untracked test fixture is created``() =
        write "apps/rhino-cli/tests/fixtures/local.env" "SECRET=not-read\n"
    ...
    member _.``the parity gate names the edited test``() =
        ...
        Assert.Contains("apps/rhino-cli/tests/parity.rs", output)
