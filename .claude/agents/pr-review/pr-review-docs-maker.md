...
- Heading-hierarchy, linking, Mermaid-validity, or naming issues already caught by this repo's
  mechanical gates (markdownlint-cli2, Prettier, `rhino-cli md mermaid validate`,
  `md links validate`, `md heading-hierarchy validate`) — governance's territory per ruling (f).
