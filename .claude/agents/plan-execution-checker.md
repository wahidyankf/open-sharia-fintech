...
2. **Post-Push CI Passed**
   - Check if every GitHub Actions workflow triggered by the latest commits passed on the plan's delivery target — the PR branch under `*-to-pr` modes, `main` under the direct-push modes (resolve the mode first; do not assume `main`)
