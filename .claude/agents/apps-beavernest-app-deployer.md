...
- No build required (the workflow's `fe-lint`/`fe-widget`/`e2e` jobs handle validation)
...
`beavernest-app` Flutter Web surface — **not** to promote anything to staging or production, since neither
exists yet.
...
A passing run means `fe-lint`, `fe-widget`, and the browser half of `e2e` are green. It does **not**
...
container image** (F# API + Flutter Web client, same origin, port 19300) built by
`apps/beavernest-be/Dockerfile`.
...
The workflow itself enforces the safety gate: `fe-lint`, `be-integration`, `fe-widget`, `e2e`,
...
- Confirming `beavernest-app` still passes Flutter lint, widget, and browser E2E checks against
...
- Making changes to application content or code
...
- `beavernest-app` - Flutter Web client built into the combined runtime
