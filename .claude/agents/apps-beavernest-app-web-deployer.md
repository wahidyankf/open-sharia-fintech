---
name: apps-beavernest-app-web-deployer
description: Triggers and monitors the scheduled beavernest-app-test-local-deploy-stag.yml GitHub Actions workflow, which validates the beavernest-app-web Vite/React client (lint, integration, and E2E against a disposable combined-runtime container). No staging or production deploy target is provisioned yet — the workflow tests only and never pushes a stag branch. Deployment is deferred to a future plan.
tools: Bash, Grep
model: haiku
color: purple
skills:
...
# Deployer for BeaverNest app-web (test-only; deploy deferred)
...
- No build required (the workflow's `fe-lint`/`fe-integration`/`e2e` jobs handle validation)
...
A passing run means `fe-lint`, `fe-integration`, and the FE half of `e2e` are green. It does **not**
mean anything shipped anywhere — there is no `deploy` job.

## No Staging or Production Target Yet

Unlike the Vercel-backed `*-app-web` products in this repo, BeaverNest ships as a **single combined
container image** (F# API + Vite client, same origin, port 19300) built by `apps/beavernest-be/Dockerfile`.
Standing up its first staging/production deploy target is tracked as a future idea/plan, not yet
created under `plans/ideas/`. Do not invent or invoke a deploy workflow, force-push a `stag-*`
branch, or claim a deployment succeeded — none of that machinery exists.
...
### Issue 2: `e2e` job times out waiting for the combined runtime
...
- Confirming `beavernest-app-web` still lints and passes FE integration/E2E against the combined
  runtime
...
- Making changes to content or code (use `swe-typescript-dev`)
...
- `swe-typescript-dev` - Develops beavernest-app-web Vite/React code
- `apps-beavernest-be-deployer` - Companion agent for the same combined-runtime test workflow
