...
`rtk ./hippo run --class ephemeral --disk-path . -- npm install` at the selected worktree root.
...
boundary (`rtk ./hippo run --class transactional --disk-path . -- npm exec nx -- affected -t test:unit`
