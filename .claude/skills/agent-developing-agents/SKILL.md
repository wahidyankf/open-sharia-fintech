...
This applies to `.claude/agents/*.md`, `.claude/skills/*/SKILL.md` and `reference/*.md`, and `.opencode/agents/*.md` mirrors. After editing `.claude/agents/` sources, run `npm run generate:bindings` so `.opencode/agents/` stays aligned — the regenerated mirrors are part of your change and MUST land in the **same commit** as the `.claude/` source. Verify with `npm run validate:sync`; never hand-edit a mirror. See [File-Touch Discipline](../../../repo-governance/development/practice/file-touch-discipline.md).
