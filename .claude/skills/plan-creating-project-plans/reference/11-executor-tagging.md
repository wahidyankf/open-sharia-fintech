...
**Git-mechanical steps are `[AI]` (HARD RULE)**: provision the worktree (`git worktree add …`), commit, push (to `origin main` for `*-to-origin-main` modes, or to the PR branch for `*-to-pr` modes), and remove the worktree (`git worktree remove …`) are git-mechanical steps the agent performs directly — always tag them `[AI]`, never `[HUMAN]`. For the default `worktree-to-pr` mode, do **not** author a `[HUMAN]` "review the diff and approve push" gate for the push itself — pushing to the PR branch is `[AI]`, and so is the final PR merge to `main`, once the hardened preconditions hold and the PR-Review Maker→Fixer Cycle has completed (per [09-delivery-mode.md](09-delivery-mode.md)). Author a `[HUMAN]` merge step only where the plan explicitly opts into that gate. See [Git Push Default Convention](../../../../repo-governance/development/workflow/git-push-default.md).
...
See [Plans Organization Convention §Executor Tagging](../../../../repo-governance/conventions/structure/plans/17-executor-tagging-tags-and-bias.md#executor-tagging--ai-vs-human-hard-rule) for the authoritative rule.
