...
**Identity note**: post under the existing `gh` CLI identity with an explicit AI-attribution
footer — a `---` rule, blank line, then `Generated by AI`, as the last thing in every comment —
until a dedicated bot/App identity is
provisioned, mirroring the retired monolith's own temporary posture.
