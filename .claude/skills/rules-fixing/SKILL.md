---
name: rules-fixing
description: Mechanics for rules-fixer — applying validated fixes from a rules-checker audit report across agent-skill duplication, skills coverage gaps, rules governance, licensing convention, and software-documentation findings.
when_to_use: When implementing or maintaining rules-fixer, or any agent that applies validated fixes from a repository-wide rules audit report.
---
...
`rules-fixer` reads a `rules-checker` audit report and applies fixes across six
...
`rules-checker` (produces the audit report this agent consumes), `rules-maker` (creates
