...
- **Cost/latency per review, tracked per risk-tier** — measured separately for `trivial`, `lite`, and
  `full` per the [risk-tier fan-out (D12)](./08-cost-control-noise-control-mechanics-risk-tier-fan-out.md), never as one blended average
...
- **Human-override rate** — the share of PRs where a human explicitly dismisses or overrides a
  ...
  [human-dismissal-respect re-review rule](./11-cost-control-noise-control-mechanics-human-dismissal-and-boundary-tag-strip.md) already tracks
