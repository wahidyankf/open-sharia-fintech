...
**Continued from** [Nine Reviewer Disciplines: Table (1)](./04-the-nine-reviewer-disciplines-table-part-1.md) — same table, columns unchanged.
...
| Type-soundness        | `pr-review-types-maker`       | Static type-system soundness across languages — TypeScript `any`/unsafe casts/`@ts-ignore`/`@ts-expect-error` misuse, Rust `unsafe` blocks/unchecked casts/`unwrap()`/`expect()` panics on fallible paths with a documented error type available, F# type-erasure/`obj` misuse/non-exhaustive `match` relying on a silent default, Dart null-assertion (`!`) misuse — anywhere a type escape hatch or an unhandled fallible path defeats the compiler's own soundness guarantee | Whether the code compiles → CI/build concern, not a review finding; general logic correctness unrelated to type escape hatches → logic; whether a new type/module boundary should exist at all → architecture |
...
[The Boundary Tie-Breaker Rule](./06-the-boundary-tie-breaker-rule.md) below for the coordinator's
