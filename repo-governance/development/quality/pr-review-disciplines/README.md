...
- [Cost/Noise Control: Plans-Only Review Route](./cost-control-noise-control-mechanics-plans-only-route.md) — The fresh-per-cycle plans-only specialist route, primary secrets probe, and implementation suppression. Use when a PR contains only plan documents and required indexes.
...
- [Quality Gates: CRITICAL-Reproduction and Cycle Cap](./quality-gate-enhancements-critical-reproduction-and-seven-cycle-maximum.md) — Requiring reproduction for CRITICAL, and the seven-cycle cap. Use when a CRITICAL finding lacks reproduction steps.
