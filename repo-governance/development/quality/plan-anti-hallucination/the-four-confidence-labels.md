...
- **`[Unverified]`** — author flagged the claim as needing verification but proceeded under time pressure. `plan-checker` admits `[Unverified]` claims to the ledger; the gate's repair pass either verifies and re-labels them or blocks the row for manual resolution.
