...
- **CLI apps**: Unit + integration mandatory; both levels cover the behaviour in the Gherkin specs; `--lib` unit tests mock all I/O via injected dependencies, but the `tests/*.rs` cucumber binaries (also enumerated in `test:unit` for `rhino-cli`) drive the real command pipeline against real or `/tmp`-fixture filesystems — see [Per-Backend and CLI App Implementation Patterns](./per-backend-and-cli-app-implementation-patterns.md); `test:integration` re-runs the identical `tests/*.rs` binaries and is cacheable but not CI-wired
