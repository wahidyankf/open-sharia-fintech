...
`lint` (including static a11y checks via oxlint jsx-a11y plugin for TypeScript UI projects) runs in all three enforcement gates: the pre-push hook, the PR quality gate, and Test CI workflows. `specs:coverage` runs in the pre-push hook and all Test CI workflows, ensuring every Gherkin step has a matching step definition. The pre-push hook intentionally omits the `test:integration`/`test:e2e` **Nx targets** named in the table above. These tests require Docker infrastructure (PostgreSQL, running servers) and are too slow and environment-dependent to run on every push. This is a statement about those two targets, not a blanket claim that no real-filesystem or command-pipeline test ever runs pre-push: `rhino-cli`'s `tests/*.rs` cucumber binaries drive the real command pipeline and are enumerated directly in `test:unit`, so they run in the pre-push hook's `Via test:quick` column above — see [Per-Backend and CLI App Implementation Patterns](./per-backend-and-cli-app-implementation-patterns.md). The PR quality gate omits `specs:coverage` because it targets only the fast `test:quick` path used for merge checks. The scheduled `test-and-deploy-*.yml` workflows cover integration, E2E, and specs:coverage on a regular cadence for production apps.
