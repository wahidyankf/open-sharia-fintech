...
- [The Rule](./the-rule.md) — The rule mandating preexisting CI blockers be root-caused, never bypassed. Use for the exact wording of the CI-blocker-resolution rule.
- [Forbidden Actions](./forbidden-actions.md) — Actions forbidden when resolving a CI blocker. Use before skipping, disabling, or bypassing a CI check.
- [The Investigation Process (Steps 1-4)](./the-investigation-process-steps-1-4.md) — Steps 1-4: read the error, blast radius, reproduce, trace to root cause. Use when starting to investigate a CI blocker.
- [The Investigation Process (Steps 5-7)](./the-investigation-process-steps-5-7.md) — Steps 5-7: apply the fix, verify, audit an existing mitigation. Use when applying and verifying a fix for a CI blocker.
- [Commit Separation](./commit-separation.md) — Why a CI-blocker fix must be its own commit. Use when a CI-blocker fix is bundled with unrelated changes.
