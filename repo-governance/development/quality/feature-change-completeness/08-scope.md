...
- `plans/` -- plan documents are intentions, not implementation artifacts, so this convention does not treat the plan files themselves as artifacts to keep in sync. **However**, per the [Two Paths](./03-two-paths-with-a-plan-and-without-a-plan.md) section, a plan whose scope touches `apps/`, `libs/`, or `specs/` MUST plan the companion specs/Gherkin work in its delivery checklist; a plan that omits those steps is incomplete (enforced by `plan-maker` and `plan-checker`).
