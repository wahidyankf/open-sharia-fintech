...
- [Layer Definitions](./hexagonal-architecture/03-layer-definitions.md) — What belongs and what is forbidden in each hexagonal layer - domain, application, infrastructure, and inbound adapters. Use when deciding whether a piece of code belongs in the domain, application, infrastructure, or inbound-adapter layer.
- [Dependency Rule, App-Type Specializations, and Related](./hexagonal-architecture/04-dependency-rule-app-type-specializations-and-related.md) — The inward-only dependency rule diagram, links to per-app-type specializations, and related pattern documentation. Use when verifying the dependency-rule direction, or finding the CLI/backend specialization for a given app type.
