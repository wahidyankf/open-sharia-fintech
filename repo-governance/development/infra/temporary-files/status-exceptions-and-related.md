...
- **Manual-verification evidence** - Screenshots and exported reports proving a plan's manual checks are **committed**, not temporary, and belong in that plan's own `evidence/` subfolder — see [The Rule](../../quality/evidence-capture/the-rule.md). A root-anchored `/evidence/` entry in `.gitignore` catches the repo-root misplacement, so evidence written there silently never stages
...
- [Evidence Capture — The Rule](../../quality/evidence-capture/the-rule.md) - Where committed manual-verification artifacts live, and the `.gitignore` guard behind it
