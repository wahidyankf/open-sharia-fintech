...
- [Mandatory Report Generation for Checker Agents](./mandatory-report-generation.md) — The requirement that `*-checker` agents write reports to their per-family local-tmp/ directory with required tools. Use when building or reviewing a `*-checker` agent.
...
- [Directory Purposes — generated-reports/ and Progressive Writing Requirement](./generated-reports-and-progressive-writing.md) — What generated-reports/ is for under the intent test, and why checker agents must write progressively. Use when deciding whether an artifact belongs in generated-reports/ or local-tmp/.
...
- [Report File Naming Standard — Repository Audit and Link Validation Reports](./report-file-naming-early-report-types.md) — Filename pattern and retention for rules-checker and docs-link-checker reports. Use when naming a rules-checker or docs-link-checker report.
...
- [`local-tmp/`](./local-tmp-directory.md) — What local-tmp/ is for, its per-family layout, and the predicates for reclaiming anything inside it. Use when deciding if a file belongs in local-tmp/.
