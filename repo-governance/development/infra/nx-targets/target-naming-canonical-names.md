...
| `test:specs`              | Aggregate of every `specs:*` validator for the project (`specs:structure-validation`, `specs:behavior:coverage`; `echo` where no specs); present on all projects; runs inside `test:quick` — replaces the separate specs-structural gate job                              | All projects (echo where no specs) |
