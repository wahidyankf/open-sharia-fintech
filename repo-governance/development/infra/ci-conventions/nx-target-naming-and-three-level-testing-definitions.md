...
| `test:integration` | Execute real isolated local-resource tests with no network path                               |
...
Setup and assertions count when classifying a test. Integration forbids loopback and local servers,
not only external network. E2E requires public-boundary observation, not merely permission to use
