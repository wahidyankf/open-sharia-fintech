...
- [Quality Gates](./quality-gates.md) — The local and CI gates an eligible or noneligible PR must pass, the universal secret check, and the no-bypass-without-permission rule. Use when confirming which gates a PR must pass before merge, or when a secret exposure is suspected in a PR diff.
