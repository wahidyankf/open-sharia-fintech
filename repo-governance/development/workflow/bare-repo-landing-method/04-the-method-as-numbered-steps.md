...
1. **Verify topology first** (see [Verify Topology First](./03-verify-topology-first.md#verify-topology-first) above).
...
7. `git worktree remove <path>` — remove the worktree non-destructively, never with `--force` and
   ...
   **before** running this step — see
   [Remote-Branch Cleanup in a Bare Repository](./09-remote-branch-cleanup-in-a-bare-repository.md#remote-branch-cleanup-in-a-bare-repository). This
...
8. **Reconcile local `main`** — the step most often missing in practice. See
   [Terminal Reconcile](./05-terminal-reconcile.md#terminal-reconcile) for the exact command, keyed by topology.
