...
- [Environment Configuration](./environment-configuration.md) — Summary pointer to the .env template pattern, the no-secrets-in-git iron rule, and the env backup/restore toolchain. Use when locating the full env/secrets standards, or recalling the .env.example pattern and backup/restore commands.
