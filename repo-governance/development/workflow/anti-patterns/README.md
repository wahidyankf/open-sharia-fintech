...
- [Anti-Pattern: Skipping Feature Flags for Incomplete Work](./skipping-feature-flags-for-incomplete-work.md) — Keeping work on a long-lived branch instead of integrating an internally complete-and-inert increment behind a temporary production-disabled flag. Use when incomplete behavior would otherwise be held back from `main`.
