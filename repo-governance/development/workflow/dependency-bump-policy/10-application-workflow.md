...
5. For Path C: document the waiver per the
   [Path C template](./03-three-path-decision-tree.md#path-c--security-override-waiver)
...
10. Run security re-audit: `npm audit --audit-level=moderate`, `govulncheck ./...`, optionally `mvn org.owasp:dependency-check-maven:check`
    ...
    scheduling ([EPSS Escalation](./04-kev-fast-track-and-epss-escalation.md#epss-escalation--soft-urgency-signal) applies).
