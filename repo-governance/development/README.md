...
- [Git Identity From Global Config Convention](./workflow/git-identity-from-global-config.md) - Prohibits `[user]` overrides in any subrepo's `.git/config`; git author identity must come exclusively from the developer's global `~/.gitconfig`. Enforced by the `AGENTS.md` Git Identity Guardrail (behavioral, agent-side) — the former `scripts/git-identity-check.sh` pre-commit script was removed. Includes `includeIf` and `GIT_AUTHOR_*` guidance for multi-identity workflows
- [Git Hook Lifecycle](./workflow/git-hook-lifecycle.md) - Canonical reference for the three Husky registry shims (commit-msg, pre-commit, pre-push): discovery with `gate list`, conformance with `gate validate`, failure behavior, and the CI matrix relationship
...
- [Mechanize Cross-File Invariants](./practice/mechanize-cross-file-invariants.md) - When a rule must hold across more than one file, generate the dependent file(s) from a single declared source and validate the result, rather than stating the rule in prose and trusting hand-sync
- [Trustworthy Measurement](./practice/trustworthy-measurement.md) - Before a number justifies a decision, prove the command ran, prove it measured the path that actually executes, and prove the metric can respond to the change; covers false-zero timing harnesses, isolated-vs-batched benchmarks, and critical-path reasoning for max-type metrics
