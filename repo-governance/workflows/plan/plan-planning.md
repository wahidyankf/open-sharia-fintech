...
produces changes** — lands as **its own PR**. The mapping is strict and one-to-one: **one worktree →
one branch → one PR → one node**. Never open two PRs from one worktree, and never drive one PR from
two worktrees.
