...
**Continues** [Execution Mode](./01-execution-mode.md).
...
plans/backlog/<slug>/ plans/in-progress/<slug>/` (no date prefix; `in-progress/` uses the same
   ...
   `plans/backlog/`. See [Execute Plan from Backlog](./44-example-usage-and-iteration-example.md#execute-plan-from-backlog) below.
