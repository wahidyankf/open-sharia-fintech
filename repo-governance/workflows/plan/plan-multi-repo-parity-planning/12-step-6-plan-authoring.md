...
7. Delivery mode (from `mode` input) — this governs how the **plan document** itself is delivered
   (see [Modes](./04-invocation-point-and-modes-overview.md) above), distinct from the plan's own `## Delivery Mode` declaration below
8. **Instruction for `plan-maker`** to declare this repo's own
   ...
   [Modes](./04-invocation-point-and-modes-overview.md) above places on this workflow's own 3-value vocabulary. A repo whose plan
...
**Continues in** [Step 6 — Plan Authoring (Required Contents)](./13-step-6-plan-authoring-required-contents.md).
