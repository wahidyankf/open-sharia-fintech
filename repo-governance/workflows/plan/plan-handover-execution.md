...
- **`plan-path`** (string, required) — Path to the plan folder (in plans/backlog/, plans/in-progress/, or plans/done/) in the current repo, or a bare plan-identifier slug.
- **`date`** (string, optional) — ISO date (YYYY-MM-DD) to stamp the handover filename with. Defaults to the current date.

## Outputs

- **`handover-doc`** (file, pattern `local-tmp/handovers/*__*-implementation.md`) — The written handover document. Default filename/folder is `local-tmp/handovers/<date>__<plan-identifier>-implementation.md` — the exact same default `plan-takeover-execution.md`'s Phase A0.5 looks up (see its frontmatter `inputs`/discovery-path note); keep both in sync if this default ever changes.
