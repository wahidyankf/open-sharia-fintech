...
| [Plan Handover Execution](plan/README.md)                                    | Write a structured handover document (`local-tmp/handovers/<date>__<plan-identifier>-implementation.md`) capturing an in-progress plan's per-repo state, concrete next step, and non-obvious gotchas learned this session, for a different agent or session to resume from. Local, gitignored, single-machine artifact; read by plan-take-over-execution as a lead, never a substitute for its own verification                                                                                                                                                                     | none — direct orchestration                                                                                                                                             | Low        |
| [Plan Take-Over Execution](plan/README.md)                                   | Given a plan path, discover its true execution state across the current repo and its siblings (`ose-primer`, `ose-private` at minimum) — worktrees, local/remote branches, PRs — reconcile findings, take over any live in-flight work, clean up confirmed-stale leftover worktrees/branches/build artifacts, then hand off to plan-execution                                                                                                                                                                                                                                       | none — direct orchestration, hands off to plan-execution-checker                                                                                                        | Medium     |
...
- **pr-review-quality-gate**: Strictly sequential N-cycle (default 3) reviewer pipeline
  (`pr-review-scout-maker` → nine specialists → `pr-review-synthesis-maker`) → `pr-review-fixer`
  loop against a PR — line-anchored
