...
Every port-binding app in this repository — the six Next.js sites and the two F# backends alike —
...
2. The app's **prefixed variable** (`OSE_WWW_PORT`, `AYOKODING_WWW_PORT`, `OSE_BE_PORT`, …),
   formed by the per-app prefix rule above.
...
at once; the prefixed name is what lets one shell hold all eight ports. The frameworks are bridged,
