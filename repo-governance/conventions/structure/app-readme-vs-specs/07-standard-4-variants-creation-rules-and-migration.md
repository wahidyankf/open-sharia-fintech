...
| Multi-CLI (e.g., `crane`)         | Same as CLI-only, with `components/cli/` + `behavior/<product>-cli/gherkin/` alongside web layers if applicable                                                            | Nothing additional omitted — same shape, more populated |
