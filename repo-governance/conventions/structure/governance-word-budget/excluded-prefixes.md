...
description: The seven path prefixes the word-budget gate excludes, and why.
...
The `governance-word-budget` gate in `repo-config.yml` registers seven `args.exclude` entries, which
`governance word-budget validate` also folds into a bare CLI run. They are repo-relative path
...
| `.opencode/skills/`   | Nx-generated wholesale, with no `.claude/` source of truth   |
| `.opencode/commands/` | Same                                                         |

The exclusions exist because `**/README.md` matches every README in the repository, including trees
this convention never claimed. The two `.opencode/` entries are a narrower class than `.fvm/`: they
are git-tracked and present in every clone, excluded not because they are absent but because
nothing in this repository's governance system produces or can regenerate them. Both are
**tree-level** prefixes, so a future Nx regeneration adding another skill or command directory stays
excluded without an edit here.
