...
2. A plan's delivery checklist references an existing corpus by relative link (for example, a
   ...
   plan is a **consumer** (see the [Custody Rule](./11-custody-rule.md)), not learning-bearing in
   its own right.

Only plans meeting this trigger carry the folder layout, template-derived shape, and the
[Corpus Disposition](./10-corpus-disposition.md) declaration; every other plan is exempt from this
