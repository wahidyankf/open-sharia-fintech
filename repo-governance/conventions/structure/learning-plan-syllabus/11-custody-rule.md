...
1. **Exactly one custodian per corpus.** The custodian is named in the corpus's own
   ...
   declaration from [`## Corpus Disposition`](./10-corpus-disposition.md), which only the owning
   (custodian) plan carries. A consumer plan is not learning-bearing in its own right (see the
   [Learning-Bearing Trigger](./04-learning-bearing-trigger.md)'s negative example 2) and so never
