...
- [Principles and Purpose](./no-date-metadata/01-principles-and-purpose.md) — why manual date
  metadata was banned in favor of git history
- [Scope](./no-date-metadata/02-scope.md) — which files this applies to, which website content is
  exempt, and what stays unaffected
- [Standards 1-3](./no-date-metadata/03-standards-1-to-3.md) — no `updated:` frontmatter, no
  `**Last Updated**` footer blocks, no misplaced mid-body `**Last Updated**` lines
- [Standards 4-5](./no-date-metadata/04-standards-4-to-5.md) — no inline date annotation lines, and
  how to find the authoritative change date via git
- [Examples and Migration](./no-date-metadata/05-examples-and-migration.md) — worked before/after
  comparisons and the cleanup checklist
- [Tools and References](./no-date-metadata/06-tools-and-references.md) — enforcing agents and
  related conventions
