...
1. Create a new plan folder in `backlog/` with `[project-identifier]/` format (no date prefix) —
   default to the five-document multi-file layout (see [Structure Decision](./08-structure-decision.md#structure-decision)).
...
The [Knowledge Capture phase](./13-the-knowledge-capture-phase.md#the-knowledge-capture-phase-final-phase-before-archival) routes some
