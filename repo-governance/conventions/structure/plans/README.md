...
- [Folder Structure](./02-folder-structure.md) — Deciding which top-level plans/ subfolder a document belongs in.
...
- [Two-Page Discipline](./05-two-page-discipline-and-difference-from-backlog.md) — A two-pager is growing too detailed or when deciding whether an idea is ready to become a full plan.
...
- [Placement, Legend](./19-executor-tagging-placement-legend-and-execution-semantics.md) — Adding the executor-tag legend to a delivery.md file or handling a [HUMAN] stop during execution.
...
- [Delivery Checklists Express a DAG](./21-delivery-checklists-express-a-dag.md) — Writing a plan's Parallelization Model.
- [Delivery Units](./22-delivery-checklists-express-a-dag-continued.md) — Mapping a plan's DAG nodes onto delivery units, branches, and PRs.
...
- [Baseline Artifacts](./24-phase-0-opens-no-pr-rationale-and-enforcement.md) — A Phase 0 step writes an evidence artifact.
...
- [Rules 5-7](./26-prs-open-at-delivery-boundaries-rules-continued.md) — Deciding whether independent work may share a PR, or whether an already-open PR may wait for a later merge.
...
- [Delivery Boundaries Declaration](./28-delivery-boundaries-and-applicability.md) — Writing a plan's Delivery Boundaries table or checking whether a grandfathered in-progress plan must retrofit gates.
...
- [One Worktree Per Repository Per Plan](./31-worktree-cap.md) — A plan produces more than one delivery unit in the same repository.
- [Delivery Mode](./32-delivery-mode-the-four-modes.md) — Identifying which of the four delivery modes a plan's work location and integration target correspond to.
...
- [Merge Authority](./34-delivery-mode-merge-authority-and-precedence.md) — Determining which delivery mode actually applies to a plan.
...
- [Key Differences from Documentation](./37-key-differences-and-creating-plans.md) — Deciding whether new content belongs in plans/ or docs/, or when starting to author a new plan.
...
- [Infra-Apply Gate](./39-infra-apply-gate-and-plan-index-files.md) — Deciding whether a plan with a terraform apply or similar step is ready to archive.
...
- [Related Documentation](./43-related-documentation.md) — Looking for a related convention or guide that this plans convention builds on or links out to.
