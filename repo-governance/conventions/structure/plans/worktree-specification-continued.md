...
3. **Immediate cleanup**: when every delivery unit this plan places in a repo is confirmed delivered,
   resolve the exact path and branches from its file-touch ledger. Remove the exact, self-created
   worktree without another prompt only after it is clean and idle, has no unpushed commits, and each
   PR is confirmed merged (never infer this from ancestry after a squash merge). Use non-force
...
`origin/main` when missing, syncs with `origin/main` before implementing, and removes the exact
ledger-owned path immediately after delivery when its mandatory safety checks pass.
````
