...
Authoritative split between `brd.md` and `prd.md`. These rules are normative for `plan-maker` / `plan-checker` / `plan-quality-gate` — the agents share one definition to avoid drift.
